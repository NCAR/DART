...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_hcho_thinner.f90'
...
!
   ...
   integer                         :: i,j
!
! TROPOMI HCHO variable declarations
...
!  
   type observation_data
      ...
      real,allocatable,dimension(:)    :: hcho_trop_col
      real,allocatable,dimension(:)    :: hcho_trop_col_err
...
!
   ...
   do while (ios == 0)
      ...
      if(tropomi_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tropomi_data(i_min,j_min)%hcho_trop_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%hcho_trop_col_err(max_num_obs))
      endif
!
      ...
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%hcho_trop_col(icnt), &
         tropomi_data(i_min,j_min)%hcho_trop_col_err(icnt)
      read(fileid,*,iostat=ios) &
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            call median_code(tropomi_data(i,j)%median_col,tropomi_data(i,j)%hcho_trop_col(1:tropomi_data(i,j)%icnt), &
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.20) then
            call mode_code(tropomi_data(i,j)%mode_col,tropomi_data(i,j)%hcho_trop_col(1:tropomi_data(i,j)%icnt), &
            tropomi_data(i,j)%icnt)
         endif
      enddo
   enddo
      ...
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%mean_col=tropomi_data(i,j)%mean_col+tropomi_data(i,j)%hcho_trop_col(icnt)
               tropomi_data(i,j)%mean_err=tropomi_data(i,j)%mean_err+tropomi_data(i,j)%hcho_trop_col_err(icnt)
            enddo
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%stdv_col=tropomi_data(i,j)%stdv_col+(tropomi_data(i,j)%hcho_trop_col(icnt)- &
               tropomi_data(i,j)%mean_col)**2.
               tropomi_data(i,j)%stdv_err=tropomi_data(i,j)%stdv_err+(tropomi_data(i,j)%hcho_trop_col_err(icnt)- &
   ...
   do i=1,nx_model
      do j=1,ny_model
         ...
         if(tropomi_data(i,j)%icnt.eq.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               dist=abs(tropomi_data(i,j)%mean_col-tropomi_data(i,j)%hcho_trop_col(icnt))
...
! Write data to file
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%hcho_trop_col(icnt), &
               tropomi_data(i,j)%hcho_trop_col_err(icnt)
            write(fileid,*,iostat=ios) &
...
! Deallocate structure arrays
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            deallocate(tropomi_data(i,j)%hcho_trop_col)
            deallocate(tropomi_data(i,j)%hcho_trop_col_err)
   ...
   deallocate (dist_min)
   deallocate (icnt_min)   
end program tropomi_hcho_thinner
