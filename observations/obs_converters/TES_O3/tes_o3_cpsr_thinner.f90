...
!
program tes_o3_profile_thinner
...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter      :: source   = 'tes_o3_thinner.f90'
...
!  
   type observation_data
      ...
      real,allocatable,dimension(:,:)   :: o3_obs_prior
      ...
      real,allocatable,dimension(:)     :: o3_total_col
      real,allocatable,dimension(:)     :: o3_total_col_prior
      real,allocatable,dimension(:)     :: o3_total_col_err
...
!
   ...
   do while (ios == 0)
      ...
      if(tes_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tes_data(i_min,j_min)%o3_total_col(max_num_obs))
         allocate(tes_data(i_min,j_min)%o3_total_col_prior(max_num_obs))
         allocate(tes_data(i_min,j_min)%o3_total_col_err(max_num_obs))
      endif
!
      ...
      if(tes_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tes_data(i_min,j_min)%o3_obs(max_num_obs,nlay_obs))
         allocate(tes_data(i_min,j_min)%o3_obs_prior(max_num_obs,nlay_obs))
         allocate(tes_data(i_min,j_min)%o3_obs_err(max_num_obs,nlay_obs))
      ...
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_obs_prior(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_obs_err(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
      ...
      enddo   
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_total_col(icnt)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_total_col_prior(icnt)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%o3_total_col_err(icnt)
      read(fileid,*,iostat=ios) data_type, obs_id, &
         i_min, j_min
      tes_data(i_min,j_min)%data_type=data_type
      tes_data(i_min,j_min)%obs_id=obs_id
...
! Write data to file
         if(tes_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_obs(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_obs_prior(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_obs_err(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
            ...
            enddo
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_total_col(icnt)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_total_col_prior(icnt)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%o3_total_col_err(icnt)
         endif
      enddo
   enddo
!               
! Deallocate structure arrays
   do i=1,nx_model
      do j=1,ny_model
         if(tes_data(i,j)%icnt.gt.0) then
            ...
            deallocate(tes_data(i,j)%o3_obs)
            deallocate(tes_data(i,j)%o3_obs_prior)
            deallocate(tes_data(i,j)%o3_obs_err)
            ...
            deallocate(tes_data(i,j)%o3_total_col)
            deallocate(tes_data(i,j)%o3_total_col_prior)
            deallocate(tes_data(i,j)%o3_total_col_err)
         endif
      enddo
   enddo
   deallocate (tes_data)
   deallocate (dist_min)
   deallocate (icnt_min)   
end program tes_o3_profile_thinner
