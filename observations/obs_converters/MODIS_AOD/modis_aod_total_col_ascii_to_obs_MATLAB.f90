...
!
program modis_aod_total_col_ascii_to_obs_matlab
!
!=============================================
! MODIS AOD total column obs
!=============================================
...
!
   ...
   integer                         :: obs_accept,obs_aod_reten_freq
!
...
! Species-specific variables
   ...
   real                            :: prs_total,zenang,obs_sum
   real(r8)                        :: prs_total_r8
   real                            :: prs_loc
   ...
   real,allocatable,dimension(:,:,:)   :: aod_fld,qmr_fld
!
...
! Record the namelist values used for the run ...
   call error_handler(E_MSG,'init_create_modis_obs','create_modis_obs_nml values are',' ',' ',' ')
   write(     *     , nml=create_modis_obs_nml)
!
...
!
   do icopy =1, num_copies
      if (icopy == 1) then
         copy_meta_data='MODIS AOD observation'
      else
...
!   allocate(aod_fld(nx_model,ny_model,nz_model))
...
!   call get_DART_diag_data(file_in,'aod',aod_fld,nx_model,ny_model,nz_model,1)
!   prs_fld(:,:,:)=prs_bas(:,:,:)+prs_prt(:,:,:)
!   tmp_fld(:,:,:)=300.+tmp_prt(:,:,:)
!   aod_fld(:,:,:)=aod_fld(:,:,:)*1.e-6
!
! Open MODIS AOD binary file
   fileid=100
...
! Read MODIS AOD
   ...
   do while (ios == 0)
      ...
      read(fileid,*,iostat=ios) taudb,taudb_best,taudb_err
      lon_obs_r8=lon_obs
      lat_obs_r8=lat_obs
!
! Possible MODIS error
   if(mn_obs.eq.7 .and. dy_obs.eq.0) then
      day = 1
   endif
!
...
! Set data for writing obs_sequence file
      qc_count=qc_count+1
!
! Obs value is the totalospheric slant column
! scd = vcd * amf      
      obs_val(:)=taudb_best
      obs_err_var=(fac_obs_error*taudb_err)**2.
      modis_qc(:)=0
...
!
      ...
      call set_qc(obs, modis_qc, num_qc)
...
!
end program modis_aod_total_col_ascii_to_obs_matlab
