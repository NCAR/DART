...
!
      ...
      if(omi_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(omi_data(i_min,j_min)%prs_obs(max_num_obs,nlay_obs))
      endif
      read(fileid,*,iostat=ios) &
      ...
      read(fileid,*,iostat=ios) &
         omi_data(i_min,j_min)%prs_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) data_type, obs_id, &
         i_min, j_min
      omi_data(i_min,j_min)%data_type=data_type
      omi_data(i_min,j_min)%obs_id=obs_id
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(omi_data(i,j)%icnt.gt.1) then
            call median_code(omi_data(i,j)%median_col,omi_data(i,j)%col_amt_total(1:omi_data(i,j)%icnt), &
            omi_data(i,j)%icnt)
         endif
      enddo
   enddo
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(omi_data(i,j)%icnt.gt.20) then
            call mode_code(omi_data(i,j)%mode_col,omi_data(i,j)%col_amt_total(1:omi_data(i,j)%icnt), &
            omi_data(i,j)%icnt)
         endif
      enddo
   enddo
      ...
      do j=1,ny_model
         if(omi_data(i,j)%icnt.gt.0) then
            ...
            do icnt=1,omi_data(i,j)%icnt
               omi_data(i,j)%mean_col=omi_data(i,j)%mean_col+omi_data(i,j)%col_amt_total(icnt)
               omi_data(i,j)%mean_err=omi_data(i,j)%mean_err+omi_data(i,j)%col_amt_total_err(icnt)
            enddo
...
! Calculate standard deviation   
   do i=1,nx_model
      do j=1,ny_model
         if(omi_data(i,j)%icnt.gt.1) then
            ...
            do icnt=1,omi_data(i,j)%icnt
               omi_data(i,j)%stdv_col=omi_data(i,j)%stdv_col+(omi_data(i,j)%col_amt_total(icnt)- &
               omi_data(i,j)%mean_col)**2.
               omi_data(i,j)%stdv_err=omi_data(i,j)%stdv_err+(omi_data(i,j)%col_amt_total_err(icnt)- &
   ...
   enddo
!
! Find the mean's closest neighbor based on total_col
   dist_min(:,:)=1.e20
   icnt_min(:,:)=0
   rewind(fileid)
   do i=1,nx_model
      do j=1,ny_model
         ...
         if(omi_data(i,j)%icnt.eq.1) then
            ...
            do icnt=1,omi_data(i,j)%icnt
               dist=abs(omi_data(i,j)%mean_col-omi_data(i,j)%col_amt_total(icnt))
...
! Write data to file
         if(omi_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               omi_data(i,j)%prs_obs(icnt,1:nlay_obs)
         endif
      enddo
   enddo
!               
