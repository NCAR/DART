...
use airs_obs_rad_mod, only : make_obs_sequence, create_output_filename
...
if ((l2_files(1) /= '') .and. (l2_file_list /= '')) then
   ...
   call error_handler(E_ERR, progname, msgstring, source, revision, revdate)
endif
...
do
   ...
   call airs_cc_rad_rdr(datafile(1), granule)   
