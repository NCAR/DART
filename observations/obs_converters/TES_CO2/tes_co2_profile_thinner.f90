...
!
program tes_co2_profile_thinner
...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter      :: source   = 'tes_co2_thinner.f90'
...
! TES CO2 variable declarations
   ...
   real,allocatable,dimension(:)    :: co2_lay
   real,allocatable,dimension(:)    :: co2_prior_lay
   real,allocatable,dimension(:)    :: co2_prior_err_lay
...
!  
   type observation_data
      ...
      real,allocatable,dimension(:,:)   :: co2_obs
      real,allocatable,dimension(:,:)   :: co2_obs_prior
      real,allocatable,dimension(:,:)   :: co2_obs_err
      ...
      real,allocatable,dimension(:)     :: co2_total_col
      real,allocatable,dimension(:)     :: co2_total_col_prior
      real,allocatable,dimension(:)     :: co2_total_col_err
...
!
   ...
   do while (ios == 0)
      ...
      if(tes_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tes_data(i_min,j_min)%co2_total_col(max_num_obs))
         allocate(tes_data(i_min,j_min)%co2_total_col_prior(max_num_obs))
         allocate(tes_data(i_min,j_min)%co2_total_col_err(max_num_obs))
      endif
!
      ...
      if(tes_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tes_data(i_min,j_min)%co2_obs(max_num_obs,nlay_obs))
         allocate(tes_data(i_min,j_min)%co2_obs_prior(max_num_obs,nlay_obs))
         allocate(tes_data(i_min,j_min)%co2_obs_err(max_num_obs,nlay_obs))
      ...
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_obs_prior(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_obs_err(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
      ...
      enddo   
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_total_col(icnt)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_total_col_prior(icnt)
      read(fileid,*,iostat=ios) &
         tes_data(i_min,j_min)%co2_total_col_err(icnt)
      read(fileid,*,iostat=ios) data_type, obs_id, &
         i_min, j_min
      tes_data(i_min,j_min)%data_type=data_type
      tes_data(i_min,j_min)%obs_id=obs_id
...
! Write data to file
         if(tes_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_obs(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_obs_prior(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_obs_err(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
            ...
            enddo
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_total_col(icnt)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_total_col_prior(icnt)
            write(fileid,*,iostat=ios) &
               tes_data(i,j)%co2_total_col_err(icnt)
         endif
      enddo
   enddo
!               
! Deallocate structure arrays
   do i=1,nx_model
      do j=1,ny_model
         if(tes_data(i,j)%icnt.gt.0) then
            ...
            deallocate(tes_data(i,j)%co2_obs)
            deallocate(tes_data(i,j)%co2_obs_prior)
            deallocate(tes_data(i,j)%co2_obs_err)
            ...
            deallocate(tes_data(i,j)%co2_total_col)
            deallocate(tes_data(i,j)%co2_total_col_prior)
            deallocate(tes_data(i,j)%co2_total_col_err)
         endif
      enddo
   enddo
   deallocate (tes_data)
   deallocate (dist_min)
   deallocate (icnt_min)   
end program tes_co2_profile_thinner
