...
! Version controlled file description for error handling, do not edit
   character(len=*), parameter     :: source   = 'tropomi_ch4_thinner.f90'
...
!
   ...
   integer                         :: i,j
!
! TROPOMI CH4 variable declarations
...
!  
   type observation_data
      ...
      real,allocatable,dimension(:)    :: alt_sfc
      real,allocatable,dimension(:)    :: prs_sfc
      real,allocatable,dimension(:,:)  :: alt_obs
      real,allocatable,dimension(:,:)  :: avgk_obs
      real,allocatable,dimension(:,:)  :: prior_obs
      real,allocatable,dimension(:)    :: ch4_col
      real,allocatable,dimension(:)    :: ch4_col_err
...
!
   ...
   do while (ios == 0)
      ...
      if(tropomi_data(i_min,j_min)%icnt.eq.1) then
         ...
         allocate(tropomi_data(i_min,j_min)%alt_sfc(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%prs_sfc(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%ch4_col(max_num_obs))
         allocate(tropomi_data(i_min,j_min)%ch4_col_err(max_num_obs))
      endif
!
      ...
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%alt_sfc(icnt), &
         tropomi_data(i_min,j_min)%prs_sfc(icnt)
      if(tropomi_data(i_min,j_min)%icnt.eq.1) then
         allocate(tropomi_data(i_min,j_min)%alt_obs(max_num_obs,nlay_obs))
      ...
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%alt_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%avgk_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%prior_obs(icnt,1:nlay_obs)
      read(fileid,*,iostat=ios) &
         tropomi_data(i_min,j_min)%ch4_col(icnt), &
         tropomi_data(i_min,j_min)%ch4_col_err(icnt)
      read(fileid,*,iostat=ios) data_type, obs_id, &
         i_min, j_min
      tropomi_data(i_min,j_min)%data_type=data_type
      tropomi_data(i_min,j_min)%obs_id=obs_id
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            call median_code(tropomi_data(i,j)%median_col,tropomi_data(i,j)%ch4_col(1:tropomi_data(i,j)%icnt), &
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.20) then
            call mode_code(tropomi_data(i,j)%mode_col,tropomi_data(i,j)%ch4_col(1:tropomi_data(i,j)%icnt), &
            tropomi_data(i,j)%icnt)
         endif
      enddo
   enddo
      ...
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%mean_col=tropomi_data(i,j)%mean_col+tropomi_data(i,j)%ch4_col(icnt)
               tropomi_data(i,j)%mean_err=tropomi_data(i,j)%mean_err+tropomi_data(i,j)%ch4_col_err(icnt)
            enddo
   ...
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               tropomi_data(i,j)%stdv_col=tropomi_data(i,j)%stdv_col+(tropomi_data(i,j)%ch4_col(icnt)- &
               tropomi_data(i,j)%mean_col)**2.
               tropomi_data(i,j)%stdv_err=tropomi_data(i,j)%stdv_err+(tropomi_data(i,j)%ch4_col_err(icnt)- &
   ...
   do i=1,nx_model
      do j=1,ny_model
         ...
         if(tropomi_data(i,j)%icnt.eq.1) then
            ...
            do icnt=1,tropomi_data(i,j)%icnt
               dist=abs(tropomi_data(i,j)%mean_col-tropomi_data(i,j)%ch4_col(icnt))
...
! Write data to file
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%alt_sfc(icnt), &
               tropomi_data(i,j)%prs_sfc(icnt)
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%alt_obs(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%avgk_obs(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%prior_obs(icnt,1:nlay_obs)
            write(fileid,*,iostat=ios) &
               tropomi_data(i,j)%ch4_col(icnt), &
               tropomi_data(i,j)%ch4_col_err(icnt)
         endif
      enddo
   enddo
!               
! Deallocate structure arrays
   do i=1,nx_model
      do j=1,ny_model
         if(tropomi_data(i,j)%icnt.gt.0) then
            ...
            deallocate(tropomi_data(i,j)%alt_sfc)
            deallocate(tropomi_data(i,j)%prs_sfc)
            deallocate(tropomi_data(i,j)%alt_obs)
            deallocate(tropomi_data(i,j)%avgk_obs)
            deallocate(tropomi_data(i,j)%prior_obs)
            deallocate(tropomi_data(i,j)%ch4_col)
            deallocate(tropomi_data(i,j)%ch4_col_err)
         endif
      enddo
   enddo
   ...
   deallocate (dist_min)
   deallocate (icnt_min)   
end program tropomi_ch4_thinner
