...
!-----------------------------------------------------------------------
!> seaice_fraction_to_obs_netcdf - input is a seaice-coverage file that has been
...
program seaice_fraction_icebridge_to_obs_netcdf
...
character(len=*), parameter :: routine = 'seaice_fraction_icebridge_to_obs_netcdf'
...
real(r8), allocatable :: seaice_fraction(:)
...
namelist /seaice_fraction_icebridge_to_obs_nc_nml/  year, doy, cat,terr, &
...
call find_namelist_in_file('input.nml', 'seaice_fraction_icebridge_to_obs_nc_nml', iunit)
read(iunit, nml = seaice_fraction_icebridge_to_obs_nc_nml, iostat = io)
call check_namelist_read(iunit, io, 'seaice_fraction_icebridge_to_obs_nc_nml')

! Record the namelist values used for the run
if (do_nml_file()) write(nmlfileunit, nml=seaice_fraction_icebridge_to_obs_nc_nml)
if (do_nml_term()) write(     *     , nml=seaice_fraction_icebridge_to_obs_nc_nml)
...
allocate(seaice_fraction(axdim))
...
call nc_check( nf90_get_var(ncid, varid, seaice_fraction), &
...
acrossloop: do i = 1, axdim
   ...
   call create_3d_obs(thislat, thislon, cat, VERTISSURFACE, seaice_fraction(i), &
...
deallocate(lon, lat, qc_array)

! end of main program
call finalize_utilities()

end program seaice_fraction_icebridge_to_obs_netcdf
