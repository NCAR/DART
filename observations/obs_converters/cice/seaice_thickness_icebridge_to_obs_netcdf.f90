...
program seaice_thickness_icebridge_to_obs_netcdf
...
character(len=*), parameter :: routine = 'seaice_thickness_icebridge_to_obs_netcdf'
...
namelist /seaice_thickness_icebridge_to_obs_nc_nml/  year, doy, terr, &
...
call find_namelist_in_file('input.nml', 'seaice_thickness_icebridge_to_obs_nc_nml', iunit)
read(iunit, nml = seaice_thickness_icebridge_to_obs_nc_nml, iostat = io)
call check_namelist_read(iunit, io, 'seaice_thickness_icebridge_to_obs_nc_nml')

! Record the namelist values used for the run
if (do_nml_file()) write(nmlfileunit, nml=seaice_thickness_icebridge_to_obs_nc_nml)
if (do_nml_term()) write(     *     , nml=seaice_thickness_icebridge_to_obs_nc_nml)
...
allocate(seaice_thickness(axdim))
...
acrossloop: do i = 1, axdim
   ...
   call create_3d_obs(thislat, thislon, 0.0_r8, VERTISSURFACE, seaice_thickness(i), &
...
deallocate(seaice_thickness, lon, lat, qc_array)

! end of main program
call finalize_utilities()

end program seaice_thickness_icebridge_to_obs_netcdf
