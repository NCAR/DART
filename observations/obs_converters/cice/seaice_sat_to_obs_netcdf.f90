...
character(len=*), parameter :: routine = 'seaice_sat_to_obs_netcdf'
...
alongloop:  do j = 1, aydim
   acrossloop: do i = 1, axdim
      ...
      !  if (seaice_concentr(i,j) < 0.01_r8) cycle acrossloop   !FIXME temporary do not assimilate
      !  if (seaice_concentr(i,j) > 1.00_r8) cycle acrossloop   !FIXME temporary do not assimilate

      thislat = lat(i,j)
      thislon = lon(i,j)

      !>@todo  maybe    thiserr = max(seaice_concentr(i,j)*terr, 0.01_r8)

      thiserr = seaice_concentr(i,j)*terr
      if (seaice_concentr(i,j).eq.0) thiserr = 0.01
