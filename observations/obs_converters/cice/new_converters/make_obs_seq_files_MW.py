...
datelist = pd.date_range(start=datetime(2019,1,2),end=datetime(2019,1,31)).to_pydatetime()
ens_truth = 24
exp = 'validate_dragio'
...
# for t in range(0,1):
  ...
  for p in range(0,unique_hrs.shape[0]):
    ...
    restart = '/glade/scratch/mollyw/'+exp+'/run/'+exp+'.cice_00'+ str(ens_truth)+'.r.{0}-{1}-{2}-{3}.nc'.format(year,mon,day,label)
    path = '/glade/scratch/mollyw/'+exp+'/run/'+exp+'.cice_00'+str(ens_truth)+'.h.{0}-{1}-{2}.nc'.format(year_old,mon_old,day_old)
    print(path)
  ...
  if os.path.exists('obs_seq.'+str(curr_date)[0:4]+'-'+str(curr_date)[4:6]+'-'+str(curr_date)[6:]+'-00000'):
    ...
    os.system(comd)
    comd = 'mv obs_seq.'+str(curr_date)[0:4]+'-'+str(curr_date)[4:6]+'-'+str(curr_date)[6:]+'-00000 ./OBS/validate/multivariate/dragio/mem_00'+str(ens_truth)+'/std1/'
    os.system(comd)
