...
# for t in range(0,1):
  ...
  for p in range(0,unique_hrs.shape[0]):
    ...
    for l in range(0,hold_lat.shape[0]):
      for c in range(0,5):
        count += 1
        new_file.writelines('0\n')
        new_file.writelines('5\n')
        new_file.writelines('1\n')
        new_file.writelines(str(c+1)+'\n')
        ...
        error_hi = 0.1
        new_file.writelines(str(error**2 + error_hi**2)+'\n')
  ...
  if os.path.exists('obs_seq.'+str(curr_date)[0:4]+'-'+str(curr_date)[4:6]+'-'+str(curr_date)[6:]+'-00000'):
    ...
    os.system(comd)
    comd = 'mv obs_seq.'+str(curr_date)[0:4]+'-'+str(curr_date)[4:6]+'-'+str(curr_date)[6:]+'-00000 ./OBS/itd/volume/mem_00'+str(ens_truth)+'/std1/'
    os.system(comd)
