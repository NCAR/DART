...
!-----------------------------------------------------------------------
!> seaice_freeboard_to_obs_netcdf - input is a seaice-coverage file that has been
...
program seaice_freeboard_icesat2_to_obs_netcdf
...
character(len=*), parameter :: routine = 'seaice_freeboard_icesat2_to_obs_netcdf'
...
real(r8), allocatable :: seaice_freeboard(:)
...
namelist /seaice_freeboard_icesat2_to_obs_nc_nml/  year, doy, terr, &
...
call find_namelist_in_file('input.nml', 'seaice_freeboard_icesat2_to_obs_nc_nml', iunit)
read(iunit, nml = seaice_freeboard_icesat2_to_obs_nc_nml, iostat = io)
call check_namelist_read(iunit, io, 'seaice_freeboard_icesat2_to_obs_nc_nml')

! Record the namelist values used for the run
if (do_nml_file()) write(nmlfileunit, nml=seaice_freeboard_icesat2_to_obs_nc_nml)
if (do_nml_term()) write(     *     , nml=seaice_freeboard_icesat2_to_obs_nc_nml)
...
allocate(seaice_freeboard(axdim))
allocate(lon(axdim), lat(axdim))
allocate(qc_array(axdim))

varname = 'fb'
call nc_check( nf90_inq_varid(ncid, varname, varid), &
               routine, 'inquire var '// trim(varname))
call nc_check( nf90_get_var(ncid, varid, seaice_freeboard), &
...
acrossloop: do i = 1, axdim
   ...
   if ( lat(i) >  90.0_r8 .or. lat(i) <   40.0_r8 ) cycle acrossloop
   if ( lon(i) <   0.0_r8 .or. lon(i) >  360.0_r8 ) cycle acrossloop

   ! the actual data values are denser, so inner loop here

   if ( qc_array(i) /= 0) cycle acrossloop  !reserve for future quality control
   ...
   call create_3d_obs(thislat, thislon, 0.0_r8, VERTISSURFACE, seaice_freeboard(i), &
                      SAT_SEAICE_AGREG_FREEBOARD, thiserr, oday, osec, qc, obs)
...
deallocate(seaice_freeboard, lon, lat, qc_array)

! end of main program
call finalize_utilities()

end program seaice_freeboard_icesat2_to_obs_netcdf
