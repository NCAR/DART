...
#
# merge the individual 6 minute 'granule' files into daily files 
# which start at 0Z and end at 23:59:59Z
...
while (( d <= totaldays)) ; do
  ...
  # all of todays data plus the first few N of tomorrows
  ls AIRS.${cyear}.${cmonth}.${cday}.*.out > olist
  ls AIRS.${nyear}.${nmonth}.${nday}.0[01]?.out >> olist
