...
#
# DART $Id: prepbufr.csh 6256 2013-06-12 16:19:10Z thoar $
#
#--------------------------------------------------------------
# DESCRIPTION:
#
...
#set BUFR_dir = /glade/p/cisl/dares/Observations/bufr/raw
set BUFR_dir = ../data
...
while ( $day <= $last )
   ...
   while ( (($h < 30) && ($daily == yes)) || (($h < 24) && ($daily == no)) )
      ...
      if ($daily == 'yes') then
         ...
         if ($zeroZ == 'yes') then
            ...
            set BUFR_out_base = ${BUFR_dir}/${year}${mm}_6Hnew
            set BUFR_out = ${BUFR_out_base}/temp_obs.${year}${mm}${dd}${hh}
         else
            # if 0Z, output named with previous day and 24Z
            set BUFR_out_base = ${BUFR_dir}/${oyear}${omm}_6Hnew
            set BUFR_out = ${BUFR_out_base}/temp_obs.${oyear}${omm}${odd}${ohh}
         endif
         echo "moving output to ${BUFR_out}"
         if (! -d ${BUFR_out_base} ) mkdir ${BUFR_out_base}
   ...
   if ($daily == 'yes') then
      ...
      set BUFR_out_base = ${BUFR_dir}/${oyear}${omm}_new
      set BUFR_out = ${BUFR_out_base}/temp_obs.${oyear}${omm}${odd}
      echo "moving output to ${BUFR_out}"
      if (! -d ${BUFR_out_base} ) mkdir ${BUFR_out_base}
...
# $URL: https://subversion.ucar.edu/DAReS/DART/trunk/observations/NCEP/prep_bufr/work/prepbufr.csh $
# $Revision: 6256 $
