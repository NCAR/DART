...
# we used to separate files in monthly subdirectories because when you
# untar'd them, that was the convention.  however it seems easier to just
# have all the files in the same directory so as we roll over month
# boundaries and need the last 6 hours from the previous month or the
# first 6 hours from the next month, we don't have to construct a
# different directory name.
#
# directory where the BUFR files are located, and where the
# ascii intermediate files should be created.
# the prepbufr filenames have changed conventions over the years
# but currently seem to have the format:  prepqmYYMMDDHH.no_ship_id
# often gzipped.
#
# for example, the 6Z file for nov 27th, 2010 would be:
#   prepqm10112706.no_ship_id.gz
#
# it creates files of the format: 
#   temp_obs.YYYYMMDDHH
#
# if the prepqm filenames do *not* follow this pattern, you will have to edit
...
#set BUFR_dir = ../data
set BUFR_dir  = /glade/p/cisl/dares/Observations/bufr
