...
program aqsmexco_ascii_to_obs

!=============================================
! AQSMEX SURFACE AQ obs
...
!
      use time_manager_mod, only :    set_date,                  &
                                      ...
                                      get_time
!
      use obs_kind_mod, only :        AQSMEX_CO,                 &
...
! version controlled file description for error handling, do not edit
      character(len=*), parameter :: source   = 'aqsmex_co_ascii_to_obs.f90'
...
!
      ...
      character(len=129)           :: qc_meta_data='AQSMEX QC index'
      character(len=129)           :: file_name='aqsmex_obs_seq'
...
!
      namelist /create_aqsmex_obs_nml/beg_year,beg_mon,beg_day, &
...
!
      do icopy =1, num_copies
         if (icopy == 1) then
             copy_meta_data='AQSMEX observation'
         else
...
!
! READ AQSMEX OBSERVATIONS
! Read the namelist entry
      call find_namelist_in_file("create_aqsmex_obs_nml.nl", "create_aqsmex_obs_nml", iunit)
      read(iunit, nml = create_aqsmex_obs_nml, iostat = io)
      call check_namelist_read(iunit, io, "create_aqsmex_obs_nml")
!
      min0=0
      sec0=0
...
! Determine bias correction
      ...
      if(does_file_exist) then
         ...
         do iobs=1,nobs
            ...
            if(trim(obs_list(iobs)).eq.'AQSMEX_CO') then
               exit
            endif
         enddo
         if(correction_old.eq.-999.) then
            print *, 'APM: AQSMEX_CO - Error assigning bias correction'
            stop
         endif
         close(iunit)
      endif
...
!-----------------------------------------------------------------------------
   ...
   end program aqsmex_co_ascii_to_obs
!
