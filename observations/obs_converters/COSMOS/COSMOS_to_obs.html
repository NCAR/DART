...
USE</a>
<h2>Overview</h2>
<h3>COSMOS "Level 2" Text File to DART Converter</h3>
...
<b>Level 2 Data</b> is most suited for use with DART.<br>
<br>
Since each site has a separate input data file, and the metadata
for each site must essentially be hand-input to the converter
program, it is generally easiest to convert the observations for
each site separately and then use the <a href=
...
sites and restrict the DART observation sequence file to contain
just the observations of the timeframe of interest.<br>
<br>
FYI - in DART, the soil moisture profile is converted to expected
neutron counts using the <b>CO</b>smic-ray <b>S</b>oil
<b>M</b>oisture <b>I</b>nteraction <b>C</b>ode (COSMIC), developed
at the University of Arizona by Rafael Rosolem and Jim
Shuttleworth.<br>
<br>
...
"input">&amp;COSMOS_to_obs_nml</em></li>
<li>download the Level 2 Data and prefix the filename with the
station name (or else they all get named <em class=
"file">corcounts.txt</em>) and enter the filename into <em class=
"input">&amp;COSMOS_to_obs_nml</em></li>
...
parameters</a>)</li>
<li>run <em class="program">COSMOS_to_obs</em> to generate a DART
observation sequence file for the station and rename the output
file if necessary (you can explicity name the output file via the
namelist).</li>
...
The data for each station is available from the data portal with
several levels of processing. The metadata for each station
(location, height, etc) is also available from the data portal. The
<b>Level 2 Data</b> is most suited for use with DART. An example of
the Level 2 Data follows:</p>
<pre>
...
<hr>
<h2>PROGRAMS</h2>
<p>The <em class="file">COSMOS_to_obs.f90</em> file is the source
code for the main converter program. At present there is an
...
future. <em class="program">COSMOS_to_obs</em> reads each text line
into a character buffer and then reads from that buffer to parse up
the data items. The items are then combined with the COSMIC
...
parameters to be contained as metadata for each observation.</p>
...
"../../observations/forward_operators/obs_def_land_mod.f90">DART/observations/forward_operators/obs_def_land_mod.f90</a>
and <a href=
"../../observations/forward_operators/obs_def_COSMOS_mod.f90">DART/observations/forward_operators/obs_def_COSMOS_mod.f90</a>.
...
code.</p>
<div class="indent1">
<h3>Guidance on COSMIC parameters</h3>
...
convenient.<br>
<br>
<em class="program">COSMOS_to_obs</em> reads the site name from the
input namelist and the known station information from <em class=
"file">COSMIC_parlist.nc</em>. The simplest way to add a new
station to <em class="file">COSMIC_parlist.nc</em> is probably
to:</p>
<ol>
...
<h3 class="indent1">COSMOS_to_obs namelist</h3>
<pre>
&amp;COSMOS_to_obs_nml
   site_metadata_file = 'COSMIC_parlist.nc',
