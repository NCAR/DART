...
<P>
The list of observation types to be supported by the DART executables is defined
at compile time. The observations DART supports can be changed at  any time by adding or
removing items from the preprocess namelist and rerunning <em>quickbuild.csh</em>.

<p>
<em class="unix">Preprocess</em> takes observation specific code sections from special obs_def files
to generate
...
One of the motivations behind creating <em class=file>obs_def_1d_state_mod.f90</em> was to provide a prototype for people
developing more complicated specialized observation definition modules.

<br /><br />
<em class=file>Obs_def_1d_state_mod.f90</em> is an extended format Fortran 90 module that provides the definition
...
vector that is equally spaced on a 1D cyclic domain.
Observation types include:
<ul>
  <li> RAW_STATE_VARIABLE - A straight linear interpolation to a point on a [0,1] domain.
  <li> RAW_STATE_VAR_POWER - The interpolated RAW_STATE_VARIABLE raised to a real-valued power.
  <li> RAW_STATE_1D_INTEGRAL - An area-weighted 'integral' of the state variable over some part of the cyclic 1D domain.
</ul>

RAW_STATE_VAR_POWER is convenient for studying non-gaussian, non-linear assimilation problems.
RAW_STATE_VAR_POWER can be used to do idealized studies related to remote sensing observations that are
...
Special observation modules like <em class=file>obs_def_1d_state_mod.f90</em> contain Fortran 90 code <em>and</em> additional
specially formatted commented code that is used to guide the preprocess program
in constructing obs_def_mod.f90 and obs_kind_mod.f90. The specially
...
comment line that must be included <em>verbatim</em>.
<br /><br />
...
generic quantities
(see <a href="../../assimilation_code/programs/preprocess/preprocess.html">preprocess</a>
for details on quantity files).
The header line is followed by lines
that have the observation type name (an all caps Fortran 90 identifier) and
their associated generic quantity identifier.
...
There must be a case statement entry for each type of observation, <em>except</em>
for observation types defined with COMMON_CODE.
<pre>
...
obs sequence file.
There must be a case statement entry for each type of observation, <em>except</em>
for observation types defined with COMMON_CODE. If no
special action is required put a <code>continue</code> statement
...
for observation types defined with COMMON_CODE. If no
special action is required put a <code>continue</code> statement
as the body of the case instead of a subroutine call.<pre>
