...
double precision :: table(0:table_size), inverse_table(0:table_size)
double precision :: var_table(0:table_size), inverse_var_table(0:table_size)
...
double precision :: mean(2), c(2, 2), corr, rnum(2, n_mc), sample_correl
double precision :: correl_2
integer :: i, j, k

! Initialize a repeatable random sequence for computing expected correlation
call init_random_seq(r)

! Initialize the table for accumulation
table = 0.0

! Mean is 0 for random sampling
mean = 0.0

do i = 0, table_size
   corr =  1.0 * i / table_size
! Round-off can lead to corr greater than 1.0 which messes stuff up
   if(corr > 1.0) corr = 1.0
! Load the correlation matrix
   c(1, 1) = 1.0; c(2, 2) = 1.0; c(1, 2) = corr; c(2, 1) = corr
! Prepare to accumulate mean of squared correlation
   correl_2 = 0.0
   do j = 1, n_mc
      do k = 1, n
! Loop through to do MC to get expected value of correlation
   ...
   if(table(i) < 0.0) table(i) = 0.0
   if(table(i) > 1.0) table(i) = 1.0
   var_table(i) = (correl_2 - n_mc * table(i)**2) / (n_mc - 1)
   write(*, *) i, real(corr), real(table(i)), real(var_table(i)), real((1 - table(i)**2)**2 / (n - 1))
end do
...
do i = 0, table_size
   sample_correl = 1.0 * i / table_size
   if(sample_correl > 1.0) sample_correl = 1.0
   do j = 1, 100
      if(sample_correl <= table(j)) then
         inverse_table(i) = (sample_correl - table(j - 1)) / (table(j) - table(j - 1)) / table_size &
            + (j - 1.0) / table_size
         if(inverse_table(i) < 0.0) inverse_table(i) = 0.0
         if(inverse_table(i) > 1.0) inverse_table(i) = 1.0
         inverse_var_table(i) = ((sample_correl - table(j - 1)) / (table(j) - table(j - 1))) * &
            (var_table(j) - var_table(j - 1)) + var_table(j - 1)
         if(inverse_var_table(i) < 0.0) inverse_var_table(i) = 0.0
         if(inverse_var_table(i) > 1.0) inverse_var_table(i) = 1.0
         write(*, *) i, real(sample_correl), real(inverse_table(i)), real(inverse_var_table(i)), &
            real((1.0 - inverse_table(i)**2)**2 / (n - 1))
         goto 10
      endif
   end do
10 continue
...
double precision, intent(in) :: sample_correl
double precision, intent(out) :: est_correl, var_correl
double precision :: c, sign, fract, interval
integer :: upper, lower

! There's an intrinsic to do this sign stuff
if(sample_correl >= 0.0) then
   sign = 1.0
else
   sign = -1.0
endif
...
c = dabs(sample_correl)

! Interval in table
interval = 1.0 / (table_size - 1) 
...
double precision, intent(out) :: mean_inc, spread_inc(ens_size)
...
! Need to call init if this is first call
if(first_call) then
...
subroutine corr_update_from_obs_inc(obs, mean_inc, spread_inc, state, ens_size, &
   delta_state, cov_factor)
 
implicit none
 
integer, intent(in) :: ens_size
double precision, intent(in) :: obs(ens_size), mean_inc, spread_inc(ens_size)
double precision, intent(in) :: state(ens_size), cov_factor
double precision, intent(out) :: delta_state(ens_size)
 
double precision :: ens(2, ens_size), cov(2, 2)
double precision :: sum_x, sum_y, sum_xy, sum_x2, reg_coef, creg_coef
double precision :: sum_y2, correl, est_correl, var_correl, varx, sample_factor
double precision :: temp_ens(2, ens_size), temp_correl
...
! TRY MODULATING FIRST
correl = correl * cov_factor



!call get_correct_correlation(cov_factor * correl, ens_size, est_correl, var_correl)
...
!   write(*, *) 'cov, sam, correl', real(cov_factor), real(sample_factor), real(correl)
   delta_state = sample_factor * creg_coef * spread_inc + sample_factor * creg_coef * mean_inc 
...
! Ouput comparison
!if(var_correl <= est_correl **2) then
!   write(*, *) 'old new ', real(cov_factor * reg_coef), real(sample_factor * creg_coef),  real((cov_factor * reg_coef) / (sample_factor * creg_coef)), real(correl)
!else
