ReadASCIIObsSeq <- function(fname='obs_seq.final') {
  ...
  if(!file.exists(fname)) stop(paste0('File does not exist:', fname))

  fid = file(fname, "r")

  ## since we are reading line-by-line, this will save some space and
  ## make it look more like the original code.
  fgetl <- function(con) readLines(con, n=1)
  compws <- function(str) gsub("[[:space:]]+", " ", str)
  ...
  RAW_STATE_1D_INTEGRAL      <- -99
  oned_integral_already_read <-  0
  GPSRO_REFRACTIVITY         <- -99

  aline  <-  fgetl(fid)
  values <-  strsplit(trimws(aline),' ')[[1]][1]
  ##sscanf(aline,'#s #*');   # Just concerned with the first word.

  if(tolower(values)=='obs_sequence') {       # pre-I or newer format

    aline = fgetl(fid)   # skip line containing the word 'obs_kind_definitions'
    aline = fgetl(fid)
    numdefs <- as.integer(aline)

    obskindnumber = 0*(1:numdefs)
    obskindstring = rep('',numdefs)

    for(idef in 1:numdefs) {
      aline = fgetl(fid)
      aline <- compws(trimws(aline))
      timtom <- strsplit(aline,' ')[[1]]
      obskindnumber[idef] = as.integer(timtom[1])
      obskindstring[idef] = timtom[2]

      switch(obskindstring[idef],
             'DOPPLER_RADIAL_VELOCITY' =  {
               DOPPLER_RADIAL_VELOCITY <- obskindnumber[idef]; NA },
             'RAW_STATE_1D_INTEGRAL'   = {
               RAW_STATE_1D_INTEGRAL   <- obskindnumber(idef); NA },
             'GPSRO_REFRACTIVITY'      = {
               GPSRO_REFRACTIVITY      <- obskindnumber(idef); NA } )
    }
    aline      <- fgetl(fid)
    values     <- strsplit(compws(trimws(aline)),' ')
    num_copies <- as.integer(values[[1]][2])
    num_qc     <- as.integer(values[[1]][4])

    a <- list(filename=fname,
              obskindnumber=obskindnumber,
              obskindstring = obskindstring)
  ...
  aline       <- fgetl(fid);
  values      <- strsplit(compws(trimws(aline)),' ')
  num_obs     <- as.integer(values[[1]][2])
  max_num_obs <- as.integer(values[[1]][4])

  cat('num_copies  is ',num_copies,'\n')
  cat('num_qc      is ',num_qc,'\n')
  cat('num_obs     is ',num_obs,'\n')
  cat('max_num_obs is ',max_num_obs,'\n')

  ## Read the metadata
  metadata <- rep('', num_copies)
  for(ii in 1:num_copies) metadata[ii] <- trimws(fgetl(fid))

  ## Read the QC info
  qcdata <- rep('', num_qc)
  for(ii in 1:num_qc) qcdata[ii] <- trimws(fgetl(fid))

  ## Read and parse the information that will help us read the rest of the file.
  aline       <- fgetl(fid);
  values      <- strsplit(compws(trimws(aline)),' ')
  first_time  <- as.integer(values[[1]][2])
  last_time   <- as.integer(values[[1]][4])

  cat('first_time = ',first_time,'\n')
  cat('last_time  = ',last_time ,'\n')
