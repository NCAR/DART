...
public ::                                                                &
   ...
   get_close_maxdist_init, get_close_obs_init, cam_get_close_obs,        &
   cam_sv_to_restart_file, cam_restart_file_to_sv, cam_ens_mean_for_model
...
namelist /cam_model_nml/ output_state_vector, model_version, cam_phis, model_config_file, & 
...
!   call nc_check(nf90_put_var(ncfileid, ncfldid, temp_2d(1:f_dim1, 1:f_dim2),    &
   call nc_check(nf90_put_var(ncfileid, ncfldid, temp_2d(1:f_dim1,1:f_dim2),     &   
                 ...
                 'write_cam_init','put_var '//trim(cflds(ifld)))
!   call nc_check(nf90_put_var(ncfileid, ncfldid, var%vars_2d(1:s_dim_2d(1,i), 1:s_dim_2d(2,i), i),&
...
subroutine cam_model_interpolate(x, location, obs_type, interp_val, istatus)
...
subroutine cam_restart_file_to_sv(filename, state_vector, model_time)
...
! decompose CAM fields into vector
call prog_var_to_vector (var, state_vector)

call end_model_instance(var)

end subroutine cam_restart_file_to_sv

!------------------------------------------------------------------

subroutine cam_sv_to_restart_file(state_vector, filename, statedate)
...
call write_cam_init(filename, var, statedate)

call end_model_instance(var)

end subroutine cam_sv_to_restart_file
...
subroutine cam_get_close_obs(gc, base_obs_loc, base_obs_kind, obs_loc, obs_kind, &
...
if (dart_kind > 0) then
   ...
   call cam_get_state_meta_data(dart_kind, dum_loc, cam_kind)
end if
...
subroutine cam_init_conditions(x)
...
subroutine cam_adv_1step(x, Time)
 real(r8), intent(inout) :: x(:)
