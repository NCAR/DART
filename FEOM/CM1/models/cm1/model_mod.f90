...
use state_structure_mod,   only : add_domain, get_model_variable_indices, &
                                  ...
                                  set_dart_kinds, get_dart_vector_index,  &
                                  set_clamping, set_update_variables,     &
                                  get_num_dims, get_dim_name, get_variable_name, &
                                  get_dart_kind, get_dim_length,            &
                                  get_varid_from_kind,                      &
                                  get_varid_from_kind_string,               &
                                  get_clamping_maxval, get_clamping_minval, &
                                  do_clamping, get_num_variables,  &
                                  get_model_num_dims
...
call parse_variable_list( model_variables, nfields, variable_table )

! determine variable shapes for the state structure

domid = add_domain(cm1_template_file, nfields, variable_table(1:nfields,VT_VARNAME_INDEX))

call set_dart_kinds(domid, variable_table(1:nfields,VT_DARTKIND_INDEX))
call set_update_variables(domid, variable_table(1:nfields,VT_UPDATE_INDEX))
call set_clamping(domid, variable_table(1:nfields,VT_MINVAL_INDEX), &
                         variable_table(1:nfields,VT_MAXVAL_INDEX))
...
! Array to store an axis. Used in model_interpolate_distrib
...
if (present(var_type)) then
   var_type = get_dart_kind(domid, var_id)
endif
...
! add a draw from a random gaussian distribution
do i=1,size(state)
...
subroutine vert_convert_distrib(state_ens_handle, location, obs_kind, vstatus)
...
ndims = get_model_num_dims(domid, varid)
! 2d vs. 3d variable test
if (get_model_num_dims(domid, varid) == 3) then
   nlevs = 2
else ! 2D, surface obs
   nlevs = 1
endif
...
if (debug > 0) print*, 'nlevs: ', nlevs, ', num_dims:', get_model_num_dims(domid, varid)
...
write(construct_file_name_in, '(A, i6.6, A)') TRIM(stub), copy, '.nc'
...
call print_time(base_time, 'base_time')
call print_time(set_time(second, 0))
read_model_time = base_time + set_time(second, days=0)
call print_time(read_model_time, 'analysis_time')
...
do_clamp_or_fail =  do_clamping(dom, var)
...
if ( do_clamping(dom, var_index) ) then
   ...
   if ( maxclamp /= missing_r8 ) then
      if ((debug > 0) .and. do_output()) then
         print*, 'Setting upper bound ', maxclamp, ' on ', trim(varname)
      endif
...
subroutine parse_variable_list( state_variables, ngood, table )
...
character(len=NF90_MAX_NAME) :: update
character(len=NF90_MAX_NAME) :: minstr
character(len=NF90_MAX_NAME) :: maxstr
...
MyLoop : do i = 1, nrows
   ...
   update  = trim(state_variables(ncols*i -2))
   minstr  = trim(state_variables(ncols*i -1))
   maxstr  = trim(state_variables(ncols*i   ))
   ...
   if ((debug > 99) .and. do_output()) then
      ...
      write(*,*) 'dartstr : ', trim(dartstr)
      write(*,*) 'update  : ', trim(update)
      write(*,*) 'minstr  : ', trim(minstr)
      write(*,*) 'maxstr  : ', trim(maxstr)
      write(*,*) ' '
   endif

   table( i, VT_VARNAME_INDEX  ) = trim(varname)
   table( i, VT_DARTKIND_INDEX ) = trim(dartstr)
   table( i, VT_UPDATE_INDEX   ) = trim(update)
   table( i, VT_MINVAL_INDEX   ) = trim(minstr)
   table( i, VT_MAXVAL_INDEX   ) = trim(maxstr)
...
if (ngood == nrows) then
   ...
   call error_handler(E_MSG,'parse_variable_list',string1,source,revision,revdate,text2=string2)
endif
