...
use utilities_mod,        only : nc_check, to_upper
...
public :: static_init_state_type,       &
          ...
          get_dim_length,               &
          get_dart_kind,                &
          get_dart_kind_string,         &
          get_varid_from_kind,          &
          get_varid_from_kind_string,   &
          add_time_unlimited,           &
          set_var_id,                   &
          get_unique_dim_name,          &
          get_num_unique_dims,          &
...
type netcdf_var_type
   ...
   ! dart information
   integer :: dart_kind
   character(len=NF90_MAX_NAME) :: kind_string

   ! determine whether we should update variable or not
   logical :: update_var = .true.
   !
   ! restrict the range of variables
   logical  :: clamping = .false.     ! this variable range restricted
...
type domain_type
   ...
   !> @todo The unlimted dimension could be different by variable.
   !>       Should we have a true/false function rather than checking for 
   !>       unlimDimId = -1?
   !>       Also there can be more than one unlimited dimension = nc_inq_unlimdims
...
!-------------------------------------------------------------------------------
! Total number of dimension for variable not including the unlimited dimension
!-------------------------------------------------------------------------------
function get_model_num_dims(idom, ivar)

integer, intent(in) :: idom
integer, intent(in) :: ivar ! should you access this by index?
integer :: get_model_num_dims

get_model_num_dims = state%domain(idom)%variable(ivar)%numdims

! Subrtract one from the number of dimensions if there is an unlimited
! dimension in the netdf file.
if (get_unlimited_dimid(idom) /= -1) get_model_num_dims = get_model_num_dims -1

end function get_model_num_dims
...
! Don't what to use the unlimited dimension in the 1d, 2d, 3d check
if (get_unlimited_dimid(dom_id) /= -1) ndims = ndims -1
...
!-------------------------------------------------------------------------------
subroutine set_dart_kinds(dom_id, kind_list)

integer, intent(in) :: dom_id ! domain identifier
character(len=*)    :: kind_list(:)
...
if (size(kind_list) /= num_vars) then 
   write(string1,"('size of kind_list [',I4,'] must equal num_vars[',I4,']')") size(kind_list), num_vars
...
do ivar = 1, num_vars
   ...
   dartstr = trim(kind_list(ivar))

   if( get_raw_obs_kind_index(dartstr) < 0 ) then
      write(string1,'(''there is no obs_kind <'',a,''> in obs_kind_mod.f90'')') trim(dartstr)
      call error_handler(E_ERR,'parse_variable_list',string1,source,revision,revdate)
   endif

   state%domain(dom_id)%variable(ivar)%kind_string = dartstr
   state%domain(dom_id)%variable(ivar)%dart_kind   = get_raw_obs_kind_index(dartstr)
...
! Return dart kind
!-------------------------------------------------------------------------------
function get_dart_kind(idom, ivar)

integer, intent(in) :: idom
integer, intent(in) :: ivar ! should you access this by index?
integer  :: get_dart_kind

get_dart_kind = state%domain(idom)%variable(ivar)%dart_kind

end function get_dart_kind

!-------------------------------------------------------------------------------
! Return dart kind string
!-------------------------------------------------------------------------------
function get_dart_kind_string(idom, ivar)

integer, intent(in) :: idom
integer, intent(in) :: ivar ! should you access this by index?
character(len=NF90_MAX_NAME)  :: get_dart_kind_string

get_dart_kind_string = state%domain(idom)%variable(ivar)%kind_string

end function get_dart_kind_string

!-------------------------------------------------------------------------------
! Set whether a variable should be updated when writting restart files
!-------------------------------------------------------------------------------
subroutine set_update_variables(domid, update_table)

integer, intent(in) :: domid
character(len=*), intent(in)    :: update_table(:)

integer :: ivar, num_vars
character(len=NF90_MAX_NAME) :: update_string

num_vars = get_num_variables(domid)

if (size(update_table) /= num_vars) then 
   write(string1,"('size of update table [',I4,'] must equal num_vars[',I4,']')") size(update_table), num_vars
   call error_handler(E_ERR,'set_update_variables',string1,source,revision,revdate)
endif

do ivar = 1, get_num_variables(domid)

   update_string = trim(update_table(ivar))
   call to_upper(update_string)

   if ( update_string == 'UPDATE' ) then 
      state%domain(domid)%variable(ivar)%update_var = .true.
   else
      state%domain(domid)%variable(ivar)%update_var = .false.
   endif

enddo 

end subroutine set_update_variables

!-------------------------------------------------------------------------------
! Determine whether we should write variable to file or not
!-------------------------------------------------------------------------------
function update_variable(idom, var_id)

integer, intent(in) :: idom
integer, intent(in) :: var_id

logical  :: update_variable

update_variable = state%domain(idom)%variable(var_id)%update_var

end function update_variable

!-------------------------------------------------------------------------------
! Set clamping bounds for variables
!-------------------------------------------------------------------------------
subroutine set_clamping(domid, min_clamp, max_clamp)

integer, intent(in) :: domid
character(len=*), intent(in)    :: min_clamp(:)
character(len=*), intent(in)    :: max_clamp(:)
...
if (size(min_clamp) /= num_vars) then 
   write(string1,*) 'size of min_clamp (', size(min_clamp),') ', &
                    'must equal num_vars', num_vars
   call error_handler(E_ERR,'set_clamping',string1,source,revision,revdate)
endif

if (size(max_clamp) /= num_vars) then 
   write(string1,*) 'size of max_clamp (', size(max_clamp),') ', &
                    'must equal num_vars', num_vars
   call error_handler(E_ERR,'set_clamping',string1,source,revision,revdate)
endif

do ivar = 1, num_vars
   read(min_clamp(ivar),*,iostat=ios) minvalue
   if (ios == 0) then 
      state%domain(domid)%variable(ivar)%clamping = .true.   
      state%domain(domid)%variable(ivar)%minvalue = minvalue
   endif

   read(max_clamp(ivar),*,iostat=ios) maxvalue
   if (ios == 0) then 
      state%domain(domid)%variable(ivar)%clamping = .true.   
      state%domain(domid)%variable(ivar)%maxvalue = maxvalue
   endif
enddo
...
!-------------------------------------------------------------------------------
! get variable id from kind
!-------------------------------------------------------------------------------
function get_varid_from_kind_string(dom_id, kind_string)

integer,          intent(in) :: dom_id ! domain identifier
character(len=*), intent(in) :: kind_string
integer :: get_varid_from_kind_string
...
do ivar = 1, num_vars
   if (trim(state%domain(dom_id)%variable(ivar)%kind_string) == trim(kind_string)) then
     get_varid_from_kind_string = ivar
     return
   endif
enddo

! did not find the variable id from kind

write(string1,*) 'can not find variable id for ', trim(kind_string)
call error_handler(E_ERR,'get_varid_from_kind_string',string1,source,revision,revdate)

end function get_varid_from_kind_string

!-------------------------------------------------------------------------------
! get variable id from kind
!-------------------------------------------------------------------------------
function get_varid_from_kind(dom_id, dart_kind)
...
do ivar = 1, num_vars
   if (state%domain(dom_id)%variable(ivar)%dart_kind == dart_kind) then
      get_varid_from_kind = ivar
      return
   endif
enddo

! did not find the variable id from kind
! JH FIXME: need to convert dart_kind to string.  Find the function that does this.

write(string1,*) 'can not find variable id for dart_kind ', dart_kind
call error_handler(E_ERR,'get_varid_from_kind',string1,source,revision,revdate)
...
do ivar = 1, num_vars
 ...
 ! write(*,*) 'units       :', state%domain(dom_id)%variable(ivar)%units
   ...
   write(*,*) 'update_var  :', state%domain(dom_id)%variable(ivar)%update_var
