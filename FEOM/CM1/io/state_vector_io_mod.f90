...
integer                         :: model_size, ens_size, imem
...
if(get_num_domains()==0) then
   call error_handler(E_ERR, 'filter_read_restart_direct', &
             'model needs to call add_domain for direct_netcdf_read = .true.')
endif
...
if(use_time_from_file) then
   ...
   do imem = 1,ens_size
      state_ens_handle%time(imem) = read_model_time(get_input_file(imem,domain=1))
   enddo
else
