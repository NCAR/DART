...
for ivar = 1:plotdat.nvars
   ...
   psfname = sprintf('%s_rank_hist',plotdat.varnames{ivar});
   fprintf('Removing %s.ps from the current directory.\n',psfname)
   system(sprintf('rm %s.ps',psfname));
      ...
      for iregion = 1:plotdat.nregions
         ...
         if ( isempty(plotdat.level_units) )
            ...
            plotdat.title    = sprintf('%s @ %d %s',    ...
               ...
               plotdat.level_units);
            plotdat.title = 'AMSR-E Brightness Temperature';
         end
...
function myplot(plotdat)
 ...
 %  h = subplot(plotdat.nregions,1,plotdat.region);
   h = subplot(2,1,1);
