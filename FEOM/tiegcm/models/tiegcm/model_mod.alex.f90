...
if (vert_is_undef(location)) then !2D fields
...
! T, U, V, O & O2 are defined at midlevel
   ...
   loop_latitude: do j = 1, nlat     
        loop_level: do k = 1, nlev 
            loop_var: do nf = 1, state_num_3d         
                        ...
                        ! for NE
                        elseif (nf==12 .and. x(indx)<1) then
...
!
   ...
   real(r8), dimension(nlon,nlat,nlev)       :: TN, TN_NM, UN, UN_NM, VN, VN_NM  
   real(r8), dimension(nlon,nlat,nlev)       :: O1, O1_NM, O2, O2_NM
   real(r8), dimension(nlon,nlat,nlev)       :: TI, TE
...
!... vTEC
   ...
   ! Convert Z (geoponential height) in cm to m
   Z    = Z * 1.0e-2_r8 
