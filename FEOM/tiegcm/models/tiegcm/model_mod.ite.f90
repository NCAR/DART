...
! TYPE_local_ZG    = 0 1
! TYPE_local_TN    = 1 2
! TYPE_local_TN_NM = 2 3
! TYPE_local_O1    = 3 4
! TYPE_local_O1_NM = 4 5
! TYPE_local_O2    = 5 6
! TYPE_local_O2_NM = 6 7
! TYPE_local_UN    = 7 8
! TYPE_local_UN_NM = 8 9
! TYPE_local_VN    = 9 10
! TYPE_local_VN_NM = 1011
! TYPE_local_NE    = 1112
   ...
   loop_latitude: do j = 1, nlat     
        loop_level: do k = 1, nlev 
            loop_var: do nf = 1, state_num_3d         
                        ...
                        !Setup a minimun value for electron density. By ITL,2010          
                        if(nf==12 .and. x(indx)<1000) then 
                             var%vars_3d(i,j,k,nf) = 1000
                        !for neutral temperature
                        elseif(nf==2 .or. nf==3) then
                               if(x(indx)<0) then
                                  var%vars_3d(i,j,k,nf) = 0
                               elseif(x(indx)>6000) then
                                  var%vars_3d(i,j,k,nf) = 6000
                               else
                                  var%vars_3d(i,j,k,nf) = x(indx)
                               end if

                        !for O&O2 ratio
                        elseif(nf==4 .or. nf==5 .or. nf==6 .or. nf==7) then
                            if(x(indx)<0) then
...
! Input parameters that can be either constant or time-dependent:
     ...
     &  kp,              &! Kp index
     &  phid,            &! plasma daytime flux
     &  phin              ! plasma nighttime flux
...
! Namelist for read:
     ...
     &  imf_ncfile,saber_ncfile,tidi_ncfile,sech_nbyte,phid,phin
