function [dart ucoam] = Check_ud(ucoamfile,dartfile)
%% Check_ud : check ucoam_to_dart.f90 ... the conversion of a UCOAM restart to a DART state vector file.
%
...
ucoam.S     = nc_varget(ucoamfile,  'SALT_CUR');
ucoam.T     = nc_varget(ucoamfile,  'TEMP_CUR');
ucoam.U     = nc_varget(ucoamfile,  'UVEL_CUR');
ucoam.V     = nc_varget(ucoamfile,  'VVEL_CUR');
ucoam.PSURF = nc_varget(ucoamfile, 'PSURF_CUR');

[nz ny nx] = size(ucoam.U);
