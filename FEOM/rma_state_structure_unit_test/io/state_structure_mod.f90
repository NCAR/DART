...
logical, save      :: state_initialized = .true.
...
!-------------------------------------------------------------------------------
!> Initializes state_type
!-------------------------------------------------------------------------------

subroutine static_init_state_type()

!>@ todo review if this is needed by anyone ... can remove the whole static_init_ from several modules and all functions in this module
! TJH REMOVE THIS ROUTINE AND ALL ITS CALLS if(state_initialized) return !?
! TJH REMOVE THIS ROUTINE AND ALL ITS CALLS
! TJH REMOVE THIS ROUTINE AND ALL ITS CALLS state%num_domains = 0
! TJH REMOVE THIS ROUTINE AND ALL ITS CALLS state%model_size  = 0
...
write(string1,*)'get_num_variables:requesting size of variable # ',ivar, ' for domain ',dom_id
...
!>@ todo ... why use get_io_num_dims() instead of (blahblah)%io_info%io_numdims
...
if (state%num_domains + 1 > max_num_domains) then
   ...
   write(string2,*)'maximum number of domains is ',max_num_domains
   write(string3,*)'increase "max_num_domains" and recompile'
...
if (dom_id > num_domains .or. dom_id < 0) then
   write(string1,*)'number of known domains is ',num_domains
