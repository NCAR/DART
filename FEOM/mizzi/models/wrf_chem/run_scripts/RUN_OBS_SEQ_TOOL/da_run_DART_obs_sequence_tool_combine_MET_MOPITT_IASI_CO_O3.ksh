...
# INDEPENDENT DIRECTORIES
  ...
  export RET_MOPITT_OBS_DIR=${DATA_DIR}/obs_MOPITT_CO_DnN_Mig_DA
  export RET_IASI_CO_OBS_DIR=${DATA_DIR}/obs_IASI_CO_DnN_Mig_DA
  export RET_IASI_O3_OBS_DIR=${DATA_DIR}/obs_IASI_O3_DnN_Mig_DA
  export WRITE_OUT_NAME=obs_MOPnIAS_CO_O3_Mig_DA
#
...
# Use obs_sequence_tool to combine multiple obs_seq files
     ...
     if [[ ! -d ${ASIM_DIR}/mdart_to_sdart/${YYYY}${MM} ]]; then mkdir -p ${ASIM_DIR}/mdart_to_sdart/${YYYY}${MM}; fi
        ...
        if [[ -e ${RET_IASI_O3_OBS_DIR}/obs_seq_${L_DATE} ]]; then
           export IAS_O3_FLG=1
           cp ${RET_IASI_CO_OBS_DIR}/obs_seq_${L_DATE} ./obs_seq_IAS_O3_${L_DATE}.out
        fi
#
# No missing files
