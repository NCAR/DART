...
!-----------------------------------------------------------------------
      ...
      subroutine RNEWDATE( date, datesec, day )
!-----------------------------------------------------------------------
! 	... Find the date a specified number of days (possibly negative)
!           from the given date.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      real, intent(in) :: &
         day                  ! number of days to increment date (can be negative)
      integer, intent(inout) :: &
         date                 ! starting date in yyyymmdd format (can be negative)
      integer, intent(out) :: &
         datesec              ! starting seconds in day
...
!-----------------------------------------------------------------------
      ...
      iday = int( day )
      if( type(1:3) == '365' ) then
         date = NEWDATE365( date, iday )
      else if ( type(1:9)  ==  'gregorian' ) then
         date = NEWDATEGRG( date, iday )
      end if
      if( iday >= 0 ) then
        datesec = 86400*(day - int(iday))
      else
        datesec = 86400*(1. + (day - int(iday)))
      endif

      end subroutine RNEWDATE
