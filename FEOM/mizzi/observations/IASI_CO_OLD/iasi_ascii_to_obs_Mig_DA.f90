...
character*129   :: qc_meta_data='BASED ON IASI QUALITY STATEMENT'
...
!print *, 'avg_k ',avg_k
!print *, 'cov_r ',cov_r
!print *, 'co_tot_col, co_tot_err ',co_tot_col, co_tot_err
!
...
!                level = iasilev(kk)*100
                if(kk.lt.nlev) then
                   level=(iasilev(kk)+iasilev(kk+1))/2*100
                else
                   level=(iasilev(kk)+50)/2*100
                endif
...
!print *, 'avg_k ',co_avgker(1:nlvls)
                ...
                if(kk.eq.1) then
                   obs_kind             = IASI_CO_SFC
                else
                   obs_kind             = IASI_CO_PRTL_COL
                endif
!
                ...
                call set_obs_def_iasi_co(qc_count, co_avgker, co_prior, co_psurf, &
                                     nlvls)
