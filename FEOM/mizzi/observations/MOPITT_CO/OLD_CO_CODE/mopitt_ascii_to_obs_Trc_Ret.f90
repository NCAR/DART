...
!
double precision, allocatable, dimension(:) :: wrk,SV_avg_k,SV_cvr
double precision, allocatable, dimension(:) :: w,err2_tr_rr
double precision, allocatable, dimension(:) :: r_x_r,r_x_p,r_avg_xp
double precision, allocatable, dimension(:) :: tr_r_x_r,tr_r_x_p,tr_r_avg_xp
double precision, allocatable, dimension(:) :: tr_rr_x_r,tr_rr_x_p,tr_rr_avg_xp
!
double precision, allocatable, dimension(:,:) :: SV,U_zero,UT_zero
double precision, allocatable, dimension(:,:) :: Z,U_cvr,V_cvr
double precision, allocatable, dimension(:,:) :: UT_cvr,VT_cvr,UT_cva,VT_cva
double precision, allocatable, dimension(:,:) :: U_avg_k,UT_avg_k,V_avg_k,VT_avg_k
double precision, allocatable, dimension(:,:) :: r_avg_k,tr_r_avg_k,tr_rr_avg_k
double precision, allocatable, dimension(:,:) :: r_cov_r,tr_r_cov_r,tr_rr_cov_r
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! APM: Rotate the forward operator with the SVD of avg_k
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Calculate SVD of the rotated averaging kernal
          lwrk=max(4*nlvls,5*nlvls)
          allocate (Z(nlvls,nlvls),SV_avg_k(nlvls))
          allocate (U_avg_k(nlvls,nlvls),VT_avg_k(nlvls,nlvls),wrk(lwrk))
          allocate (UT_avg_k(nlvls,nlvls),r_avg_k(nlvls,nlvls))
          allocate (r_cov_r(nlvls,nlvls),r_x_r(nlvls),r_x_p(nlvls))    
          allocate (r_avg_xp(nlvls))
          allocate (U_zero(nlvls,nlvls),UT_zero(nlvls,nlvls))
          Z(:,:)=avg_k(:,:)
          call dgesvd('A','A',nlvls,nlvls,Z,nlvls,SV_avg_k,U_avg_k,nlvls,VT_avg_k,nlvls,wrk,lwrk,info)
!
! Rotate s_avg_k with UT_s_avg_k  
          call mat_transpose(U_avg_k,UT_avg_k,nlvls,nlvls)
          call mat_prd(UT_avg_k,avg_k,r_avg_k,nlvls,nlvls,nlvls,nlvls)
!
! Rotate remaining terms in the forward operator
          call mat_tri_prd(UT_avg_k,cov_r,U_avg_k,r_cov_r,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_avg_k,x_r,r_x_r,nlvls)
          call lh_mat_vec_prd(UT_avg_k,x_p,r_x_p,nlvls)
!
! Calculate rotated prior term (r_avg_ker*x_p)
          call lh_mat_vec_prd(r_avg_k,x_p,r_avg_xp,nlvls)
!          print *, 'r_x_r ',r_x_r        
!          print *, 'r_x_p ',r_x_p
!          print *, 'r_avg_k ',r_avg_k
!          print *, 'r_cov_r ',r_cov_r
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! APM: Find the number of non-zero singular values and truncate the forward operator
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Find number of non-zero singluar values 
           nrows=0
           do i=1,nlvls
              if(abs(SV_avg_k(i)).gt.eps_tol) then
                 nrows=nrows+1
              endif
           enddo 
!
! Get truncated U matrix (U_zero) for truncated inverse rotation 
          U_zero(:,:)=U_avg_k(:,:)
          UT_zero(:,:)=UT_avg_k(:,:)
          do i=nrows+1,nlvls
             do k=1,nlvls
             ...
             enddo
          enddo 
          deallocate(Z,SV_avg_k,U_avg_k,UT_avg_k,VT_avg_k,wrk)
!
! Get terms of truncated forward operator
! Truncate r_avg_k with UT_s_avg_k  
          allocate(tr_r_avg_k(nlvls,nlvls))
          allocate(tr_r_cov_r(nlvls,nlvls),tr_r_x_r(nlvls),tr_r_x_p(nlvls))
          allocate(tr_r_avg_xp(nlvls))
          call mat_prd(U_zero,r_avg_k,tr_r_avg_k,nlvls,nlvls,nlvls,nlvls)
!
! Truncate remaining terms in the forward operator
          call mat_tri_prd(U_zero,r_cov_r,UT_zero,tr_r_cov_r,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(U_zero,r_x_r,tr_r_x_r,nlvls)
          call lh_mat_vec_prd(U_zero,R_x_p,tr_r_x_p,nlvls)
!
! Truncate prior term (r_avg_ker*x_p)
          call lh_mat_vec_prd(tr_r_avg_k,x_p,tr_r_avg_xp,nlvls)
          deallocate(r_avg_k,r_cov_r,r_x_r,r_x_p,r_avg_xp)
          deallocate(U_zero,UT_zero)
!          print *, 'tr_r_x_r ',tr_r_x_r        
!          print *, 'tr_r_x_p ',tr_r_x_p
!          print *, 'tr_r_avg_k ',tr_r_avg_k
!          print *, 'tr_r_cov_r ',tr_r_cov_r
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! APM: Rotate the truncated forward operator with the SVD of tr_r_cov_r
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Calculate SVD of cov_r (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          allocate(tr_rr_avg_k(nlvls,nlvls),tr_rr_cov_r(nlvls,nlvls))
          allocate(tr_rr_x_r(nlvls),tr_rr_x_p(nlvls))
          allocate(tr_rr_avg_xp(nlvls))
          allocate(err2_tr_rr(nlvls))
          Z(:,:)=tr_r_cov_r(:,:)
...
! Rotate terms in the forward operator       
          call mat_prd(UT_cvr,tr_r_avg_k,tr_rr_avg_k,nlvls,nlvls,nlvls,nlvls)
          call mat_tri_prd(UT_cvr,tr_r_cov_r,U_cvr,tr_rr_cov_r,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_cvr,tr_r_x_r,tr_rr_x_r,nlvls)
          call lh_mat_vec_prd(UT_cvr,tr_r_x_p,tr_rr_x_p,nlvls)
!
! Calculate rotated prior term (r_avg_ker*x_p)
          call lh_mat_vec_prd(tr_rr_avg_k,x_p,tr_rr_avg_xp,nlvls)
!          print *, 'tr_rr_x_r ',tr_rr_x_r        
!          print *, 'tr_rr_x_p ',tr_rr_x_p
!          print *, 'tr_rr_avg_k ',tr_rr_avg_k
!          print *, 'tr_rr_cov_r ',tr_rr_cov_r
!
! Get new errors (check if err2_rs_r < 0 the qstatus=1)
          call diag_vec(tr_rr_cov_r,err2_tr_rr,nlvls)
          qstatus=0.0
          do i=1,nlvls
             if (tr_rr_cov_r(i,i).lt.0. .and. i.le.nrows) then
                qstatus = 1.0
             else
                err2_tr_rr(i)=sqrt(tr_rr_cov_r(i,i))
             endif
          enddo
          deallocate(tr_r_avg_k,tr_r_cov_r,tr_r_x_r,tr_r_x_p,tr_r_avg_xp)
          deallocate(Z,SV,SV_cvr,U_cvr,V_cvr)
          deallocate(UT_cvr,VT_cvr,wrk)
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! APM: Make assignments to Ave's variables
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
          if (qstatus .eq. 0) then
             ...
             do i=1,nlev
! APM: perturbation form not implemented
!                xcomp(i)=tr_rr_x_r(i) - tr_rr_x_p(i)
                xcomp(i)=tr_rr_x_r(i)
                xcomperr(i)=err2_tr_rr(i)
                xapr(i)=tr_rr_x_p(i)-tr_rr_avg_xp(i)
                do j=1,nlvls
                   avgker(i,j)=tr_rr_avg_k(i,j)
                enddo
             enddo
!
! APM clean up allocatables from my code
             deallocate(tr_rr_x_r,err2_tr_rr,tr_rr_x_p,tr_rr_avg_xp,tr_rr_avg_k,tr_rr_cov_r)
       !
       !--------------------------------------------------------
...
999 continue
   close(fileid)
!   close(fileidapm)
   print *, 'APM: Number of MOPITT CO obs ',apm_obs
   print *, 'APM: Number of MOPITT CO obs kept ',apm_keep
!
!-----------------------------------------------------------------------------
! Clean up
!-----------------------------------------------------------------------------
