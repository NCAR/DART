...
double precision, allocatable, dimension(:,:) :: s_avg_k, s_cov_m
...
!
double precision, allocatable, dimension(:,:) :: Z, ZT, U_avk, V_avk, U_cvm, V_cvm
double precision, allocatable, dimension(:,:) :: UT_avk, VT_avk, UT_cvm, VT_cvm
double precision, allocatable, dimension(:,:) :: SV
double precision, allocatable, dimension(:)   :: SV_avk, SV_cvm
double precision, allocatable, dimension(:,:) :: adj_avg_k, s_cvm, rs_avg_k, rs_cov_m
integer   :: apm_count   
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!
          ...
          allocate(SV_avk(nlvls))
          allocate(s_avg_k(nlvls,nlvls),s_cov_m(nlvls,nlvls))
          ...
          call dgesvd('A','A',nlvls,nlvls,Z,nlvls,SV_avk,U_avk,nlvls,VT_avk,nlvls,wrk,lwrk,info)
           nrows=0
           do i=1,nlvls
              if(abs(real(SV_avk(i))).gt.eps_tol) then
                 nrows=nrows+1
              endif
           enddo
!
! Zero the trailing singular vectors
          do i=1,nlvls
             do j=nrows+1,nlvls
                U_avk(i,j)=0.
             enddo
          enddo
!
! Zero the trailing singular values
          do j=nrows+1,nlvls
             SV_avk(j)=0.
          enddo
!
...
! 1st Rotation (terms in the forward operator)       
          ...
          call mat_tri_prd(UT_avk,dble(cov_m),U_avk,s_cov_m,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_avk,dble(x_r),s_x_r,nlvls)
          call lh_mat_vec_prd(UT_avk,dble(x_p),s_x_p,nlvls)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Rotate the forward operator with SVD of s_cov_m  
! NOTE: had used cov_r but based on Joiner and da Silva 1998 and Migliorini et al 2008 should use cov_m   
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Calculate SVD of s_cov_m (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          allocate(SV_cvm(nlvls))
          allocate(rs_avg_k(nlvls,nlvls),rs_cov_m(nlvls,nlvls))
          ...
          nrows=0
          do i=1,nlvls
             if(abs(real(SV_cvm(i))).gt.eps_tol) then
                nrows=nrows+1
             endif
          enddo
!
...
! 2d Rotation (terms in the forward operator) 
          call mat_prd(UT_cvm,dble(s_avg_k),rs_avg_k,nlvls,nlvls,nlvls,nlvls)
          call mat_tri_prd(UT_cvm,dble(s_cov_m),U_cvm,rs_cov_m,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_cvm,dble(s_x_r),rs_x_r,nlvls)
          call lh_mat_vec_prd(UT_cvm,dble(s_x_p),rs_x_p,nlvls)
          call lh_mat_vec_prd(UT_cvm,dble(s_x_qr),rs_x_qr,nlvls)
          deallocate(Z,SV,U_cvm,V_cvm)
          deallocate(UT_cvm,VT_cvm,wrk)
          deallocate(s_avg_k,s_cov_m,s_x_p,s_x_r,s_x_qr)
!print *, 'x_r ',rs_x_r
!print *, 'x_p ',rs_x_p
!print *, 'x_qr ',rs_x_qr
!print *, 'avg_k ',rs_avg_k
!print *, 'cov_m ',rs_cov_m
!
! Get new errors (check if err2_rs_r < 0 the qstatus=1)
          allocate(err2_rs_r(nlvls))
          call diag_vec(rs_cov_m,err2_rs_r,nlvls)
          qstatus=0.0
          do i=1,nlvls
             if (rs_cov_m(i,i) .lt. 0.) then
                qstatus = 1.0
             else
                err2_rs_r(i)=sqrt(rs_cov_m(i,i))
             endif
          enddo
          nrows=0
          do i=1,nlvls
...
!
! APM: temp code for cov_group problem
                if (real(err2_rs_r(i)) .ne. 1.) then
                   print *, 'APM: Obs error variance problem ',err2_rs_r(i)
                   stop
                endif
!
! APM: Set the following to zero to use the "Quasi Optimatl" retrieval forward operator
!                xapr(i)=rs_x_p(i)
                xapr(i)=0.
                do j=1,nlvls
...
! APM: clean up allocatables from my code
             deallocate(rs_x_r,rs_x_p,rs_x_qr,rs_avg_k,rs_cov_m)
       !
       !--------------------------------------------------------
...
!print *, 'press ',co_press(1:nlvlsp)
                if(co_vmr(1).eq.0..or.co_error.eq.0.) then
                   print *, 'cnt, co, err ',qc_count,co_vmr(1),co_error
                   print *, 'lon, lat, level ',longitude,latitude,level
                   stop
                endif
!
                if(kk.eq.1) then
...
!
             ...
             deallocate(rs_cov_m,rs_avg_k,rs_x_r,rs_x_p,rs_x_qr)
             deallocate(err2_rs_r)
          endif    ! qstatus
       endif    ! co_qc(1)
