...
double precision, allocatable, dimension(:,:) :: s_cov_m,s_cov_a,s_avg_k
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Rotate the forward operator with SVD of cov_m 
...
! Calculate SVD of cov_m (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          deallocate(wrk)
!       do i=1,nlvls
!          print *, 'U row ',i, U_cvrm(i,1:nlvls)
!       enddo
!       print *, 'SV ',SV_cvm
!       do i=1,nlvls
!          print *, 'VT row ',i, VT_cvm(i,1:nlvls)
!       enddo
!
! Count number of non-zero singular values
           nrows=0
           do i=1,nlvls
              if(abs(SV_cvm(i)).gt.eps_tol) then
                 nrows=nrows+1
              endif
           enddo
!
! Truncate the singular vectors
           ...
           do j=1,nrows
              ...
              VT_cvm(j,:)=VT_cvm(j,:)/sqrt(SV_cvm(j))
           enddo
!
...
! Rotate terms in the forward operator       
          call mat_prd(UT_cvm,dble(avg_k),rs_avg_k,nlvls,nlvls,nlvls,nlvls)
          call mat_tri_prd(UT_cvm,dble(cov_m),U_cvm,rs_cov_m,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_cvm,dble(adj_x_r),rs_x_r,nlvls)
          call lh_mat_vec_prd(UT_cvm,dble(adj_x_p),rs_x_p,nlvls)
!       do i=1,nlvls
!          print *, 'UT row ',i, UT_cvm(i,1:nlvls)
!       enddo
!        print *, 'rs_x_r ',rs_x_r
!        print *, 'rs_x_r ',rs_x_p
!       do i=1,nlvls
!          print *, 'rs_cov_m ',i, rs_cov_m(i,1:nlvls)
!       enddo
!
          deallocate(Z,SV,UT_cvm,VT_cvm)
...
!             deallocate(s_cvr,s_cov_m,s_avg_k,s_x_r,s_x_p)
             deallocate(rs_cov_m,rs_avg_k,rs_x_r,rs_x_p)
