function pinfo = GetWrfHydroInfo(pinfo_in,fname,routine)
%% GetWRFHYDROInfo   prepares a structure of information needed by the subsequent "routine"
%               The information is gathered via rudimentary "input" routines.
%
% pinfo = GetWRFHYDROInfo(pinfo_in, fname, routine);
%
...
if strcmpi(pinfo.model,'wrfhydro') ~= 1
   error('Not so fast, this is not a WRFHYDRO model.')
end
...
leveldim = find(strncmp('depth',varinfo.Dimension,length('depth')) > 0);
...
%----------------------------------------------------------------------
% ???
% The WRFHYDRO domain is not a simple 1D lat 1D lon domain, so it makes more
% sense to query about lat & lon indices rather than values.

varinfo = nc_getvarinfo(fname, pgvar);
latdim = find( strncmp('iy',varinfo.Dimension,length('iy')) | ...
               strncmp('iyrt',varinfo.Dimension,length('iyrt')) | ...
               strncmp('links',varinfo.Dimension,length('links')) );
londim  = find( strncmp('ix',varinfo.Dimension,length('ix')) | ...
                strncmp('ixrt',varinfo.Dimension,length('ixrt')) ) ;


if (isempty(latdim) &&  isempty(londim))
   error('''%s'' has neither lats nor lons.\n',pgvar)
end
...
if (isempty(londim)) 
    ...
    latmat = nc_varget(fname,'latRt');
    lonmat = nc_varget(fname,'lonRt');
   
    latind = nc_varget(fname,'linkIndX');
    lonind = nc_varget(fname,'linkIndY');
       
   for i = 1:nlat
       lat(i,1) = latmat(latind(i),lonind(i));
       lon(i,1) = lonmat(latind(i),lonind(i));
   end 
   ...
   lonind = round(nlat/2);
    ...
    dum = nc_getvarinfo(fname,'latRt');
    
    if (nlat == dum.Size(1))
        lat = nc_varget(fname,'latRt');
        lon = nc_varget(fname,'lonRt');
    else 
