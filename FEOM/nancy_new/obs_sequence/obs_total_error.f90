...
character(len = 160) :: filename_list = ''
integer              :: lastN = 50;

logical              :: print_only    = .true.
character(len=32)    :: calendar      = 'Gregorian'


namelist /obs_total_error_nml/ &
         filename_in, filename_list, lastN, calendar
...
! and this one counts up, if there is a 'prior ensemble mean' copy,
...
integer            :: size_seq_in
integer            :: i, total_obs
integer            :: this_obs_type, max_obs_types
! max_obs_kinds is a public from obs_kind_mod.f90 and really is
! counting the max number of types, not kinds
integer            :: type_count(max_obs_kinds), identity_count, qc_count(0:7), qcindex
integer            :: trueindex, meanindex, spreadindex, erritems, nextslot, lastcount
real(r8)           :: qcval(1), copyval(1), truth, mean, spread
real(r8)           :: tminusm, spreadsq
logical            :: doerror
real(r8), allocatable :: lasttmm(:), lastssq(:)
...
! use the right name so this code is less confusing
max_obs_types = max_obs_kinds
...
meanindex = get_dartcopy_index(seq_in, filename, 'prior ensemble mean')
spreadindex = get_dartcopy_index(seq_in, filename, 'prior ensemble spread')

doerror = all( (/trueindex, meanindex, spreadindex/) >= 0 )
if (doerror .and. lastN > 0) then
   allocate(lasttmm(lastN), lastssq(lastN))
   nextslot = 0
   lastcount = 0
endif
...
!   if(this_obs_type > 0)print *, 'obs name = ', get_obs_kind_name(this_obs_type)
   if (qcindex > 0) then
...
print *, trueindex, meanindex, spreadindex
   if (doerror .and. goodqc(qcval(1))) then
...
print *, truth, mean, spread
      tminusm = tminusm + (truth - mean)**2.0
      spreadsq = spreadsq + spread**2.0
      erritems = erritems + 1

print *, tminusm, spreadsq
      nextslot = nextslot + 1
      if (nextslot > lastN) nextslot = 1
      if (lastcount < lastN) lastcount = lastcount + 1

      lasttmm(nextslot) = (truth - mean)**2.0
      lastssq(nextslot) = spread**2.0
   endif
...
if (doerror .and. erritems > 1) then
   call error_handler(E_MSG, '', ' ')
   write(msgstring, *) 'Error results: '
   call error_handler(E_MSG, '', msgstring)
   write(msgstring, *) 'All obs, Total  error: ', sqrt(tminusm) / (erritems - 1)
   call error_handler(E_MSG, '', msgstring)
   write(msgstring, *) 'All obs, Total spread: ', sqrt(spreadsq) / (erritems - 1)
   call error_handler(E_MSG, '', msgstring)
   write(msgstring, *) 'All obs         count: ', erritems
   call error_handler(E_MSG, '', msgstring)

   if (lastcount > 1) then
      write(msgstring, *) 'Last ', lastcount, ' obs, Total  error: ', sqrt(sum(lasttmm(1:lastcount))) / (lastcount-1)
      call error_handler(E_MSG, '', msgstring)
      write(msgstring, *) 'Last ', lastcount, ' obs, Total spread: ', sqrt(sum(lastssq(1:lastcount))) / (lastcount-1)
      call error_handler(E_MSG, '', msgstring)
      write(msgstring, *) 'Last obs        count: ', lastcount
      call error_handler(E_MSG, '', msgstring)
   endif
endif
...
if (allocated(lasttmm)) deallocate(lasttmm, lastssq)

end subroutine print_obs_seq_info

!---------------------------------------------------------------------
function goodqc(qcval)

real(r8), intent(in) :: qcval
logical :: goodqc
