...
subroutine initialize_utilities(progname, alternatename, output_flag, standalone_flag)

character(len=*), intent(in), optional :: progname
character(len=*), intent(in), optional :: alternatename
logical,          intent(in), optional :: output_flag
logical,          intent(in), optional :: standalone_flag
...
! if using 'standalone' mode, no log file, no namelists - intended for
! small utilities which just want to use a module or two from the dart
! system, like the time manager or command line argument parsing.

if (present(standalone_flag)) standalone = standalone_flag

if (standalone) then
   do_output_flag = .false.
...
! for the standalone mode, all namelists are ignored.
...
! namelists are ignored in standalone mode
if (standalone) return
