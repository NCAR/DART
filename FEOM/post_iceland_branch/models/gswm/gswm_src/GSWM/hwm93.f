...
C               TO TURN ON AND OFF PARTICULAR VARIATIONS CALL WTSELEC(SW)
...
C
C              To get current values of SW: CALL WTRETRV(SW)
C
...
C      Put identification data into common/datime/
    ...
    1 CONTINUE
      DO 2 I=1,2
        ...
        NAME(I)=NAM(I)
    2 CONTINUE
      IF(ISW.NE.64999) CALL WTSELEC(SV)
      YRD=IYD
      WW(1)=W(1)
      WW(2)=W(2)
C
...
C-----------------------------------------------------------------------
   ...
   10   CONTINUE
        YD1=UGN1(1)*ZDIF
        YD2=UGN1(2)*ZDIF
        CALL WSPLINE(XS,YS,MN,YD1,YD2,Y2OUT)
C      Eq.
        X=(Z-Z1)/ZDIF
C      Eq. 
        CALL WSPLINT(XS,YS,Y2OUT,MN,X,Y)
        WPROF=Y
        RETURN
      ENDIF
      IF(Z.LT.ZN2(1)) THEN
...
C     $ C3STL,S3STL,IYR,DAY,DF,DFA,DFC,APD,APDF,APDFC,APT,SLT
      ...
      COMMON/LTCOMPr/TLL,CSTL,SSTL,C2STL,S2STL,C3STL,S3STL
      COMMON/LTCOMPi/NSVL
      COMMON/LGCOMPr/XLL,CLONG,SLONG,C2LONG,S2LONG
      COMMON/LGCOMPi/NGVL
      SAVE
...
C      CALCULATE L=M CASE AND L=M+1
   ...
   30 CONTINUE
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE WSPLINE(X,Y,N,YP1,YPN,Y2)
...
C        Y2: OUTPUT ARRAY OF SECOND DERIVATIVES
   ...
   12 CONTINUE
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE WSPLINT(XA,YA,Y2A,N,X,Y)
...
C        Y: OUTPUT VALUE
    ...
    1 CONTINUE
      ...
      ENDIF
      H=XA(KHI)-XA(KLO)
      IF(H.EQ.0) WRITE(6,*) 'BAD XA INPUT TO WSPLINT'
      A=(XA(KHI)-X)/H
      B=(X-XA(KLO))/H
      Y=A*YA(KLO)+B*YA(KHI)+
