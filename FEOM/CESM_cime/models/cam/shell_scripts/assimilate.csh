...
switch ("`hostname`")
   case ys*:
      ...
      set BASEOBSDIR = /glade/p/image/Observations/NNRA+ACARS+GPS_local2013
      # set BASEOBSDIR = /glade/p/image/Observations/GPS/local
...
set SAVE_NTH_SETS = `grep SAVE_EVERY_NTH   ${BOGUSSETUP}.original | grep xmlchange | sed -e "s/=/ /"`
# Make st_archive archive all of the REMAINING restart sets,
# Rather than every Nth ones of the REMAINING ones.
./xmlchange DOUT_S_SAVE_EVERY_NTH_RESTART_FILE_SET=1
...
# RUNDIR, and st_archive will never have to deal with them either.
#-------------------------------------------------------------------------
...
# which causes all the files to be deleted.
if ($#log_list >= 3) then
   ...
   if ($#re_list < 3) then
      echo "Not enough restart sets, even though there are $#log_list log files"
      exit 14
   endif
   ...
   # by changing day_o_month for days < 10.
   # It seems like there must be a more elegent way, but I can't figure out a printf way.
   set octal = `echo $day_o_month | cut -c 1`
   if ($octal == "0") set day_o_month = `echo $day_o_month | cut -c 2`
   set sec_o_day   = $rm_date[$sec]

   if ($SAVE_NTH_SETS[3] >= 4) then
      @ save_mth_days = $SAVE_NTH_SETS[3] / 4
   else
      echo "DOUT_S_SAVE_EVERY_NTH_RESTART_FILE_SET must be >= 4 for 6 hourly assimilations"
      exit 17
   endif

   if ( $sec_o_day !~ '00000' || \
       ($sec_o_day =~ '00000' && $day_o_month % $save_mth_days != 0) ) then
      ...
      # Note that *cpl.ha.* is retained, and any h#, #>1.
      #        $CASE                          DD          -SSSSS
      $REMOVE  $rm_date[1]*.{r,rs,rh0,h0,i}*${day_o_month}-${sec_o_day}* &

      # Remove most log files: *YYMMDD-HHMMSS
      echo '$rm_log['$#rm_log']='$rm_log[$#rm_log]
      $REMOVE  *$rm_log[$#rm_log]  &
   endif
