...
switch ("`hostname`")
   case ys*:
      ...
      # set BASEOBSDIR = /glade/p/image/Observations/GPS/local-cosmic2013
...
# Add 1 more to the log_list test because the restart set used to start this will 
# be counted: there will be 3 restarts when there are only 2 cesm.log files,
# which would cause ALL the files to be deleted.
if ($#log_list >= 3) then
   ...
   set re = 3
   while ($re <= $#re_list) 
      set rm_date = `echo $re_list[$re] | sed -e "s/-/ /g;s/\./ /g;"`
      @ day = $#rm_date - 2
      @ sec = $#rm_date - 1
      set day_o_month = $rm_date[$day]
      set sec_o_day   = $rm_date[$sec]
   
      if ($SAVE_NTH_SETS >= 4) then
      ...
      # Always remove the CAM .r. files, since we always use the .i. files to do a hybrid start.
...
# Be sure that the removal of unneeded restart sets is finished.
wait
