...
# KDR This incorporates some changes from cesm1_5_beta02
# and changes needed for DART to run effectively.
...
# KDR There are 2 hard links for each restart file: rest/$dname and $component/rest.
# Each restart is copied to the HPSS twice.  Prevent that by removing the $component/rest 
# before the hsi call.
    rm -rf $DOUT_S_ROOT/*/rest

    # check if files on st_archive should be saved or not
# KDR; DOUT_L_SAVE_ALL_ON_DISK == "FALSE", 
#      but it did not make any space on scratch.
# ! That's because it didn't remove the $CASE.locked hard links (file names)
#   for the same files.
# Add rm_locked mechanism
    saveFlag="-dPR"
    rm_locked="true"
    ...
    if [ "$rm_locked" == 'true' ]; then
       DOUT_locked=`dirname $DOUT_S_ROOT`
       DOUT_locked=${DOUT_locked}.locked/archive
       if [ "$DOUT_locked" != "" ] && [ "$DOUT_locked" != " " ] ; then
          rm -rf $DOUT_locked/*
       fi
    fi
