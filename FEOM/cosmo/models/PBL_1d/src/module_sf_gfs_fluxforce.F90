...
!-------------------------------------------------------------------
   SUBROUTINE SF_GFS_fluxforce(U3D,V3D,T3D,QV3D,P3D,                &
...
!        QSURF(I)=QSFC(I,J)
      ...
      CALL PROGTM_fluxforce(IM,KM,PS,U1,V1,T1,Q1,                       &
                  SHELEG,TSKIN,QSURF,                                   &
...
!         QFX(I,J)=AMAX1(QFX(I,J),0.)
         ...
         FLHC(i,j)=hfx(i,j)/(THGB(I)-THX(I))
         FLQC(i,j)=qfx(i,j)/(QSFC(I,J)-Q1(I))
!         FLQC(i,j)=FLHC(i,j)/CPM(I,J)
!        FLHC(i,j)=CPM(I,J)*RHO1(I)*CHS(I,J)
  ...
  END SUBROUTINE SF_GFS_FLUXFORCE

!-------------------------------------------------------------------

      SUBROUTINE PROGTM_fluxforce(IM,KM,PS,U1,V1,T1,Q1,               &
     &                  SHELEG,TSKIN,QSURF,                             &
...
!@@@@
      ...
      DO I = 1, IM
         rhox = psurf(i) / (rd * tvs(i))
         molength=-ustar(i)**3*THV1(I)/&
              &(ca*g*(hfx(i)/(cp*rhox)+ RVRDM1*tvs(i)*qfx(i)/rhox))
         ...
         IF (zeta <  0.) THEN
            ...
            pm(i)= 2.*LOG(.5*(1.+x))+LOG(.5*(1.+x*x))-2.*ATAN(x)+&
                 &.5*con_pi
            ...
            pm10(i)=2.*LOG(.5*(1.+x))+LOG(.5*(1.+x*x))-2.*ATAN(x)+&
                 &.5*con_pi
         ELSE
            pm(i)=-b_stable*(zeta-c_stable/d_stable)*&
                 &EXP(-d_stable*zeta)-&
                 &a_stable*zeta-b_stable*c_stable/d_stable
            
            ph(i)=-b_stable*(zeta-c_stable/d_stable)*&
                 &EXP(-d_stable*zeta)-&
                 &(SQRT(1.+a_stable*b_stable*zeta))**3-&
                 &b_stable*c_stable/d_stable+1.
            ph2(i)=-b_stable*(zeta2-c_stable/d_stable)*&
                 &EXP(-d_stable*zeta2)-&
                 &(SQRT(1.+a_stable*b_stable*zeta2))**3-&
                 &b_stable*c_stable/d_stable+1.
            pm10(i)=-b_stable*(zeta10-c_stable/d_stable)*&
                 &EXP(-d_stable*zeta10)-&
                 &a_stable*zeta10-b_stable*c_stable/d_stable
         ENDIF

         rb(i)=zeta*(LOG(z1(i)/z_t)-ph(i))/(LOG(z1(i)/z_o)-pm(i))**2
...
!##DG  ENDIF
      ...
      RETURN
      END SUBROUTINE PROGTM_fluxforce
!
