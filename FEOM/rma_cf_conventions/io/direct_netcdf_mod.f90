...
COPIES: do copy = 1, state_ens_handle%my_num_copies
   ...
   if (query_write_copy(copy)) then
      ...
      if(file_exist(netcdf_filename_out)) then
         ...
         call nc_write_global_att_clamping(ncfile_out, domain)
...
call nc_write_global_att_clamping(ncfile_out, dom_id, from_scratch=.true.)
...
! attributes that are only restart files, mean and sd files
if ( copy_number <= ENS_SD_COPY ) then
   call  nc_write_variable_att_clamping(ncFileID, filename, ncVarID, domid, varid)
   
   if ( copy_number == ENS_MEAN_COPY ) then
...
if( copy_number == PRIOR_INF_COPY         .or. &
    copy_number == PRIOR_INF_SD_COPY      .or. &
    copy_number == POST_INF_COPY          .or. &
    copy_number == POST_INF_SD_COPY       .or. &
    copy_number == SPARE_PRIOR_INF_MEAN   .or. &
    copy_number == SPARE_PRIOR_INF_SPREAD .or. &
    copy_number == SPARE_POST_INF_MEAN    .or. &
    copy_number == SPARE_POST_INF_SPREAD ) then
...
real(r8) :: clamp_val
character(len=NF90_MAX_NAME) :: clamp_min
...
if ( do_io_clamping(domid, varid) ) then
   ...
   if ( clamp_val /= MISSING_R8 ) then
      ...
      call nc_check(nf90_put_att(ncFileID,ncVarID,'DART_clamp_max','NA'),&
                   'nc_write_variable_att_clamping','DART_clamp_max'//trim(filename))
   endif
   ...
   if ( clamp_val /= MISSING_R8 ) then
      ...
      call nc_check(nf90_put_att(ncFileID,ncVarID,'DART_clamp_min','NA'),&
                   'nc_write_variable_att_clamping','DART_clamp_min'//trim(filename))
   endif
...
subroutine nc_write_global_att_clamping(ncFileID, domid, from_scratch)
