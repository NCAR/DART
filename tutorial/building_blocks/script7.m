%% DART:script7  In methods so far, any higher order moment info in prior ensemble is discarded
...
% Plot a random sample perfect mean and variance posterior
x_posterior = normrnd(posterior_mean, posterior_var, 5, 1);
x_posterior = (x_posterior - mean(x_posterior)) * sqrt(posterior_var / var(x_posterior)) + posterior_mean;
y_posterior = y_prior + 0.1;
h_plot = plot(x_posterior, y_posterior, 'b*')
set(h_plot, 'markersize', 18)
set(h_plot, 'linewidth', 3)
text(-2.0, 0.2, 'Random Posterior Ensemble', 'fontsize', 24);
print -depsc s07f05.eps;
