...
ens_size = 40;
% Specify a number of groups, too
num_groups = 1
% Generate total ensemble randomly with no correlation
...
for i = 1:121
   ...
   for j = 1:num_groups
      ...
      grp_bot = (j - 1) * grp_size + 1;
      grp_top = j * grp_size;

      % Compute prior obs variance
      prior_obs_var = var(obs(grp_bot:grp_top, 1));
      prior_obs_mean = mean(obs(grp_bot:grp_top, 1));
      ...
      posterior_obs(grp_bot:grp_top, 1) = (obs(grp_bot:grp_top, 1) - mean(obs)) * sqrt(update_var / 1.0) + new_mean;
      obs_inc(grp_bot:grp_top, 1) = posterior_obs(grp_bot:grp_top,1) - obs(grp_bot:grp_top, 1);
   
      % Compute the regression coefficient
      s_correl = corrcoef(state(grp_bot:grp_top, 1), obs(grp_bot:grp_top, 1));
      s_var(1) = var(obs(grp_bot:grp_top, 1));
      s_var(2) = var(state(grp_bot:grp_top, 1));
      s_covar = s_correl(1, 2) * sqrt(s_var(1)) * sqrt(s_var(2));
      reg_coef(j) = s_covar / s_var(1);
   
   end
   
   % Compute regression factor, then update state
   if num_groups == 1
      reg_factor = 1
   else
      sum_reg_reg = 0.0;
      sum_reg2 = sum(reg_coef .* reg_coef);
      for ii = 1:num_groups
         for jj = ii+1:num_groups
            sum_reg_reg = sum_reg_reg + reg_coef(ii)*reg_coef(jj);
         end
      end
      reg_factor = 2 * sum_reg_reg / (sum_reg2 * (num_groups - 1));
   end
   if(reg_factor < 0.0) 
      reg_factor = 0.0;
   end
   ...
   for j = 1:num_groups
      ...
      grp_bot = (j - 1) * grp_size + 1;
      grp_top = j * grp_size;
      post_state(grp_bot:grp_top, 1) = state(grp_bot:grp_top, 1) + reg_coef(j) * obs_inc(grp_bot:grp_top, 1) * reg_factor;
   end


   output_interval = 20;

   % Stuff done only at output interval ------------------------
   ...
   % End of stuff done only at output interval -------------
