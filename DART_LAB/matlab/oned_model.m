...
function oned_model_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = oned_model_OutputFcn(hObject, eventdata, handles) 
...
% Inflate the model
   ...
   prior_spread = mean(abs(ens_new - mean(ens_new)));
   plot([handles.time_step - 1 + 0.1, handles.time_step - 0.1], ...
      [handles.spread, prior_spread], 'r');
   ...
   for n_tick = 1:handles.ens_size
      hold on;
      hg_prior = plot([ens_new(n_tick), ens_new(n_tick)], ...
   ...
   post_spread = mean(abs(new_ens - mean(new_ens)));
   subplot(handles.r2);
   plot([handles.time_step - 0.1, handles.time_step + 0.1], ...
      [handles.spread, post_spread], 'r');
   ...
   set(hg_like, 'color', 'red', 'LineWidth', 2);
