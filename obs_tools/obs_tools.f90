...
double precision :: conv_state_to_obs(num_obs)
...
integer, intent(in) :: total_num_obs, n_state, n_obs, state_list(n_state), &
   obs_list(n_obs)
type (obs_def_type), intent(in) :: obs_def(total_num_obs)
! Should we be using full covariance for obs yet???
double precision, intent(in) :: obs_variance(total_num_obs)
...
double precision, intent(in) :: coef(num_state)
