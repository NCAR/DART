...
while($batch <= $nbatch)
   foreach node ( `cat $PBS_NODEFILE` )
      ...
      rsh $node "csh $PBS_O_WORKDIR/assim_region.csh $PBS_O_WORKDIR $element /scratch/local/kdr_dartcam " &

      echo rsh $node \
         "csh $PBS_O_WORKDIR/assim_region.csh $PBS_O_WORKDIR $element /scratch/local/kdr_dartcam " &
