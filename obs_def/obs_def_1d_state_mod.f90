...
if(debug) print*, 'location for int ', i, 'is ', x
   ...
   call interpolate(state, location2, KIND_RAW_STATE_VARIABLE, val, istatus)
...
subroutine get_expected_power(state, location, powkey, val, istatus)
 real(r8),            intent(in)  :: state(:)
 type(location_type), intent(in)  :: location
...
call interpolate(state, location, KIND_RAW_STATE_VARIABLE, val, istatus)
