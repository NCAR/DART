...
double precision, allocatable :: obs_inc(:), ens_inc(:)
double precision, allocatable :: obs(:), obs_variance(:)
double precision :: sx, s_x2, err_var

! Storage for getting indices of nearest state variables for obs
integer, allocatable :: close_state_list(:)
integer :: num_close
...
allocate(x(model_size), ens(model_size, ens_size), ens_mean(model_size), &
   ...
   ens_obs(ens_size), obs_inc(ens_size))
...
! Add the ensemble increments onto the prior ensembles
               ens(j, :) = ens(j, :) + ens_inc
