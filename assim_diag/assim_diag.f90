...
type, public :: diag_type
   ...
   double precision :: total_rms_mean, mean_total_rms, mean_bias
   double precision :: sum_rms_mean, sum_mean_rms
   integer, pointer :: bin(:, :), var_list(:)
   double precision, pointer :: rms_mean(:), mean_rms(:), bias(:), sq_bias(:)
end type diag_type
...
d%ens_size = ens_size
d%num_var = num_var
allocate(d%bin(num_var, 0:ens_size), d%var_list(num_var), d%rms_mean(num_var), &
...
d%bin = 0
d%total_rms_mean = 0.0
d%mean_total_rms = 0.0
d%mean_bias = 0.0
d%sum_rms_mean = 0.0
d%sum_mean_rms = 0.0
d%rms_mean = 0.0
...
type (diag_type), intent(inout) :: d
double precision, intent(in) :: x(:, :), truth(:), ens_mean(:)
double precision :: squared_dif
integer :: i, j, ind, num

! Increment the number of obs times
d%count = d%count + 1
...
do i = 1, d%num_var
   ind = d%var_list(i)
   d%rms_mean(i) = d%rms_mean(i) + dabs(ens_mean(ind) - truth(ind))
...
! Notes from 31 August, 2001) relative to prediction from ensemble.
   ...
   d%sq_bias(i) = d%sq_bias(i) + (ens_mean(ind) - truth(ind))**2 / &
      squared_dif
...
d%sum_rms_mean = d%sum_rms_mean + sqrt(sum((ens_mean - truth)**2) / d%model_size)
do j = 1, d%ens_size
   d%sum_mean_rms = d%sum_mean_rms + sqrt(sum((x(:, j) - truth)**2) / d%model_size)
end do
...
double precision, dimension(0:d%ens_size) :: expected, num_in_bin
double precision :: df, chsq, chsq_prob, desired, trms_mean, tmean_rms, &
   tbias, tsq_bias
integer :: i, j
...
write(*, *) 'new rms of ensemble mean ', real(d%total_rms_mean / d%count)
write(chan, *) 'new rms of ensemble mean ', real(d%total_rms_mean / d%count)
write(*, *) 'new ensemble mean of rms ', real(d%mean_total_rms / (d%count * d%ens_size))
write(chan, *) 'new ensemble mean of rms ', real(d%mean_total_rms / (d%count * d%ens_size))
! Compute the ratio of the rms of mean / mean rms to the desired ratio
desired = sqrt((d%ens_size + 1) / (2. * d%ens_size))
write(*, *) 'normalized ratio is ', real((d%total_rms_mean / (d%mean_total_rms / d%ens_size)) / desired)
write(chan, *) 'normalized ratio is ', real((d%total_rms_mean / (d%mean_total_rms / d%ens_size)) / desired)

! Compute global means of these two quantities
write(*, *)
write(*, *) 'rms of ensemble mean is ', real(d%sum_rms_mean / (d%count))
write(chan, *) 'rms of ensemble mean is ', real(d%sum_rms_mean / (d%count))
write(*, *) 'mean of rms is ', real(d%sum_mean_rms / (d%ens_size * d%count))
write(chan, *) 'mean of rms is ', real(d%sum_mean_rms / (d%ens_size * d%count))

! Compute the ratio of the rms of mean / mean rms to the desired ratio
desired = sqrt((d%ens_size + 1) / (2. * d%ens_size))
write(*, *) 'normalized ratio is ', real((d%sum_rms_mean / (d%sum_mean_rms / d%ens_size)) / desired)
write(chan, *) 'normalized ratio is ', real((d%sum_rms_mean / (d%sum_mean_rms / d%ens_size)) / desired)

write(*, *) 'mean bias is ', real(d%mean_bias / (d%model_size * d%count))
write(chan, *) 'mean bias is ', real(d%mean_bias / (d%model_size * d%count))

! Compute chi-square for truth bins
expected = (d%count * 1.0) / (d%ens_size + 1.0)
do j = 1, d%num_var
   num_in_bin = dble(d%bin(j, :))
...
do i = 1, d%num_var
   trms_mean = d%rms_mean(i) / d%count
   tmean_rms = d%mean_rms(i) / (d%count * d%ens_size)
   tbias = d%bias(i) / d%count
   tsq_bias = d%sq_bias(i) / d%count
