...
character(len=256) :: roms_input_file  ! set with get_model_analysis_filename() if needed
...
if (test1thru > 2) then
   ...
   call get_model_restart_filename(roms_input_file)

   write(*,*)'Reading file from  '//trim(roms_input_file)
   write(*,*)'Writing data into '//trim(output_file)

   call restart_file_to_sv(roms_input_file, statevector, model_time)
...
if (test1thru > 3) then
   ...
   if ( .not. file_exist(dart_input_file) ) then
      ...
      write(string2,*) "Run 'roms_to_dart' to generate ",trim(dart_input_file)
...
character(len=256) :: ncfilename,txtfilename
