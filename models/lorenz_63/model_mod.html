...
<P>
DART interface module for the Lorenz-63 model. The 16 public interfaces
...
<TR><TD> deltat </TD>
    <TD> real(r8) </TD>
    <TD>Non-dimensional timestep.  This is mapped to the 
dimensional timestep specified by time_step_days and time_step_seconds.
</TD></TR>

<TR><TD> time_step_days </TD>
    <TD> integer </TD>
    <TD>Number of days for dimensional timestep, mapped to deltat.
</TD></TR>

<TR><TD> time_step_seconds </TD>
    <TD> integer </TD>
    <TD>Number of seconds for dimensional timestep, mapped to deltat.
</TD></TR>

<TR><TD> solver </TD>
    <TD> character(8) </TD>
    <TD>The name of the solver to use. 'RK2', the default, is a two-step 
        Runge-Kutta used in the original Lorenz 63 paper. 'RK4' is the 
        only other option, which uses the four-step classic Runge-Kutta
        method.
</TD></TR>
</TBODY> 
</TABLE>
...
state vector. The location defines where the state variable is located. For
lorenz-63, the variable 1 (x) is at 0.0, the variable 2 (y) at 1/3, and the 
variable 3 (z)
at 2/3 on a cyclic [0, 1] domain. The variable type is always returned as 1.
</P>
...
location in [0, 1] by linear interpolation. Variable locations are at
0, 1/3 and 2/3 in the cyclic [0, 1] domain. So, a value of 1/6 is an
average of the x and y variables. The type variable is not used here
and istatus is always returned as 0 (successful interpolation).
</P>
...
Values chosen here are 0.1 for all three state variables.
</P>
...
<H2>REFERENCES</H2>
<ol>
   <li>Lorenz, E.N., 1963: Deterministic nonperiodic flow.
       <span style="font-style: italic;">J. Atmos. Sci.</span>,
       <span style="font-weight: bold;">20</span>, 130-141.</li>
</ol>
