...
double precision, parameter :: max_close_distance = 0.175
!double precision, parameter :: max_close_distance = 1.00
!double precision, parameter :: max_close_distance = 0.00011
...
double precision :: x, x_min, x_max, dist(max_list)
integer :: ind_min, ind_max, i, model_size, index(max_list), list_temp(max_list)
...
do i = 1, num
   dist(i) = dabs(x - (list(i) - 1.0) / model_size)
   if(dist(i) > 0.5) dist(i) = 1.0 - dist(i)
