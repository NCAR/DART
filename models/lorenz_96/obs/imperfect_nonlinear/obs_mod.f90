...
!  real(r8), parameter :: max_close_distance =  0.11_r8
!  real(r8), parameter :: max_close_distance =  1.00_r8
!  real(r8), parameter :: max_close_distance = 0.255_r8
...
ind_min = ceiling(x_min * model_size - 1)
ind_max = floor(x_max * model_size - 1)
num     = ind_max - ind_min + 1
if(num > model_size - 1) num = model_size - 1

ind_min = modulo(ind_min, model_size - 1)

! Generate index list
do i = 1, num
   list(i) = modulo(ind_min + i - 1, model_size - 1) + 1
end do

! Add on the free state variable to be close to obs 1 to num
if(obs_num <= num) then
   num = num + 1
   list(num) = model_size 
end if

! Check for overflow and die
...
!==========================================================================
! end module lorenz_96/obs/imperfect_nonlinear/obs_mod.f90
!==========================================================================
