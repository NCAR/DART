...
# Create directories
dirs=(
    $RUN_DIR $TEMPLATE_DIR $OBSPROC_DIR $OUTPUT_DIR $ICBC_DIR
...
executables=(
    advance_time filter pert_wrf_bc obs_diag obs_sequence_tool
...
SEC_DIR=assimilation_code/programs/gen_sampling_err_table/work
...
# Put scripts in rundir
$COPY ${SHELL_SCRIPTS_DIR}/add_bank_perts.ncl ${RUN_DIR} || exit 8
$COPY ${SHELL_SCRIPTS_DIR}/new_advance_model.sh ${RUN_DIR} || exit 9

# Place modified input.nml in rundir
