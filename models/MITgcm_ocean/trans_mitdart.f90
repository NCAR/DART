...
open(5,file="MODE.txt",status="old")
read(5,*)RWFLAG
...
!----------------------------------------------------------------------------
	...
	call check(nf90_def_dim(ncid=ncid, name="XG", &
	     len = Nx, dimid = XGDimID))
	call check(nf90_def_dim(ncid=ncid, name="XC", &
 	    len = Nx, dimid = XCDimID))
	call check(nf90_def_dim(ncid=ncid, name="YG", &
	     len = Ny, dimid = YGDimID))
	call check(nf90_def_dim(ncid=ncid, name="YC", &
	     len = Ny, dimid = YCDimID))
	call check(nf90_def_dim(ncid=ncid, name="ZC", &
	     len = Nz, dimid = ZCDimID))
...
!Fill the data
	...
	open(7,file='PSAL.data',form="UNFORMATTED",status='OLD',access='DIRECT',recl=4*Nx*Ny*Nz)
	read(7,rec=1)data_3d
	close(7)
	where (data_3d == 0.0_r4) data_3d = FVAL
	call check(nf90_put_var(ncid,SVarID,data_3d,start=(/1,1,1/)))

	open(7,file='PTMP.data',form="UNFORMATTED",status='OLD',access='DIRECT',recl=4*Nx*Ny*Nz)
	read(7,rec=1)data_3d
	close(7)
	where (data_3d == 0.0_r4) data_3d = FVAL
	call check(nf90_put_var(ncid,TVarID,data_3d,start=(/1,1,1/)))

	open(7,file='UVEL.data',form="UNFORMATTED",status='OLD',access='DIRECT',recl=4*Nx*Ny*Nz)
	read(7,rec=1)data_3d
	close(7)
	where (data_3d == 0.0_r4) data_3d = FVAL
	call check(nf90_put_var(ncid,UVarID,data_3d,start=(/1,1,1/)))

	open(7,file='VVEL.data',form="UNFORMATTED",status='OLD',access='DIRECT',recl=4*Nx*Ny*Nz)
	read(7,rec=1)data_3d
	close(7)
	where (data_3d == 0.0_r4) data_3d = FVAL
	call check(nf90_put_var(ncid,VVarID,data_3d,start=(/1,1,1/)))
 
	open(7,file='ETA.data',form="UNFORMATTED",status='OLD',access='DIRECT',recl=4*Nx*Ny)
	read(7,rec=1)data_2d
	close(7)
...
!Fill the data
	...
	open(7,file='PSAL.data',form="UNFORMATTED",status='UNKNOWN',access='DIRECT',recl=4*Nx*Ny*Nz)
	write(7,rec=1)data_3d
	close(7)
	...
	call check( nf90_get_att(ncid,varid,"_FillValue",FVAL))        
	where (data_3d == FVAL) data_3d = 0.0_r4
	open(7,file='PTMP.data',form="UNFORMATTED",status='UNKNOWN',access='DIRECT',recl=4*Nx*Ny*Nz)
	write(7,rec=1)data_3d
	close(7)
	...
	call check( nf90_get_att(ncid,varid,"_FillValue",FVAL))        
	where (data_3d == FVAL) data_3d = 0.0_r4
	open(7,file='UVEL.data',form="UNFORMATTED",status='UNKNOWN',access='DIRECT',recl=4*Nx*Ny*Nz)
	write(7,rec=1)data_3d
	close(7)
	...
	call check( nf90_get_att(ncid,varid,"_FillValue",FVAL))        
	where (data_3d == FVAL) data_3d = 0.0_r4
	open(7,file='VVEL.data',form="UNFORMATTED",status='UNKNOWN',access='DIRECT',recl=4*Nx*Ny*Nz)
	write(7,rec=1)data_3d
	close(7)
	...
	open(7,file='ETA.data',form="UNFORMATTED",status='UNKNOWN',access='DIRECT',recl=4*Nx*Ny)
	write(7,rec=1)data_2d
	close(7)
