...
if(file_exist('input.nml')) then
   ...
   if(io /= 0) then
      ...
      call error_handler(E_ERR, 'trans_dart_to_MITgcm:&assim_model_nml problem', &
   ...
   if(io /= 0) then
      ...
      read(iunit, '(A)') nml_string
      write(err_string, *) 'INVALID NAMELIST ENTRY: ', trim(adjustl(nml_string))
      call error_handler(E_ERR, 'trans_dart_to_MITgcm:&model_nml problem', &
                         err_string, source, revision, revdate)
   endif
...
call error_handler(E_MSG,'trans_dart_to_MITgcm','assim_model_nml values are',' ',' ',' ')
write(logfileunit, nml=assim_model_nml)
write(     *     , nml=assim_model_nml)

call error_handler(E_MSG,'trans_dart_to_MITgcm','model_nml values are',' ',' ',' ')
