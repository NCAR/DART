...
<br />
<em class="program">netcdf_to_gitm_blocks</em> is the program that
updates the GITM restart files (i.e. <em class=
"file">b?????.rst</em>) with the information from a DART
output/restart file (e.g. <em class=
...
"program">netcdf_to_gitm_blocks</em>:</p>
<ul>
...
"program">gitm_to_dart</em> and <em class=
"program">netcdf_to_gitm_blocks</em> are run. This makes it easy to
use a single 'static' name for the input and output filenames.
<em class="program">advance_model.csh</em> is responsibile for
linking the appropriate files to these static filenames.</p>
...
<p>And then manually run <em class=
"program">netcdf_to_gitm_blocks</em> on the result.</p>
</div>
...
'netcdf_to_gitm_blocks namelist description'>
<tr>
...
<tr><!--contents-->
<td valign="top">
netcdf_to_gitm_blocks_output_file   </td>
<!--  type  -->
...
<td>logical</td>
<!--descript-->
<td>If you are manually converting a DART initial conditions or
restart file this should be <em class="code">.false.</em>; these
files have a single timestamp describing the valid time of the
model state. If <em class="code">.true.</em>, TWO timestamps are
expected in the DART file header and <em class=
"file">DART_GITM_time_control.txt</em>) is created with the
settings appropriate to advance GITM to the time requested by
DART.</td>
</tr>
</table>
<br />
...
"program">netcdf_to_gitm_blocks</em> is repeated here.</p>
<table border="0" cellpadding="3" width="100%" summary=
