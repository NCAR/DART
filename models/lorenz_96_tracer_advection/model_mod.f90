...
do i=1,num_my_grid_points
    ...
    !Perturbing all source grid points
        ...
        ! If source is not perturbed it will not evolve in assimilation
        ! Following perturbations allow source estimation for basic cases if turned on
        do j = 1, ens_size
            !!!state_ens_handle%copies(j, i) = random_gaussian(random_seq, 2.5_r8, 2.5_r8)
            !!!if(state_ens_handle%copies(j, i) < 0.0_r8)state_ens_handle%copies(j, i) = 0.0_r8
        end do
    end if
end do
