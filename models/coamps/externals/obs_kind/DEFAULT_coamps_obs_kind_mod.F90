...
! Additional base variable types for COAMPS (TRW)
public :: KIND_EXNER_FUNCTION
...
! A list of raw variable types that must be used by observation kinds but can
! also be used by models to define the type of their variables for
! get_state_meta_data and for forward operator evaluation by models. Negative
! values are reserved for raw state variable types so that low order models
! don't have to use this module.
...
! New raw variable types for dealing with the DART COAMPS implementation
! Not only do we add new types in here, but also supply information
! to build an array that holds the text names so  we can do searches
! on these names/indices as well as for the derived types.
! -----------------------------------------------------------------
integer, parameter :: KIND_EXNER_FUNCTION              = 85

integer, parameter :: MAX_RAW_TYPES                    = 90

character(len=32), dimension(MAX_RAW_TYPES) :: RAW_NAMES
! -----------------------------------------------------------------
...
! TRW: Add in the initialization of the raw variable names array
do i = 1, MAX_RAW_TYPES
   RAW_NAMES(i) = ""
end do

! Populate the array - if you want to add a raw variable type up
! there, you have to add it here as well.  I hate how this scales,
! but it's a viable work-around until DART includes a true converter 
! Whoops! kind_raw_state_variable is -1, but we don't need the name anyway

!RAW_NAMES(KIND_RAW_STATE_VARIABLE)       = "KIND_RAW_STATE_VARIABLE"
RAW_NAMES(KIND_U_WIND_COMPONENT)         = "KIND_U_WIND_COMPONENT"
RAW_NAMES(KIND_V_WIND_COMPONENT)         = "KIND_V_WIND_COMPONENT"
RAW_NAMES(KIND_SURFACE_PRESSURE)         = "KIND_SURFACE_PRESSURE"
RAW_NAMES(KIND_TEMPERATURE)              = "KIND_TEMPERATURE"
RAW_NAMES(KIND_SPECIFIC_HUMIDITY)        = "KIND_SPECIFIC_HUMIDITY"
RAW_NAMES(KIND_PRESSURE)                 = "KIND_PRESSURE"
RAW_NAMES(KIND_VERTICAL_VELOCITY)        = "KIND_VERTICAL_VELOCITY"
RAW_NAMES(KIND_RAINWATER_MIXING_RATIO)   = "KIND_RAINWATER_MIXING_RATIO"
RAW_NAMES(KIND_DEW_POINT_TEMPERATURE)    = "KIND_DEW_POINT_TEMPERATURE"
RAW_NAMES(KIND_DENSITY)                  = "KIND_DENSITY"
RAW_NAMES(KIND_VELOCITY)                 = "KIND_VELOCITY"
RAW_NAMES(KIND_RADAR_REFLECTIVITY)       = "KIND_RADAR_REFLECTIVITY"
RAW_NAMES(KIND_1D_INTEGRAL)              = "KIND_1D_INTEGRAL"
RAW_NAMES(KIND_GRAUPEL_MIXING_RATIO)     = "KIND_GRAUPEL_MIXING_RATIO"
RAW_NAMES(KIND_SNOW_MIXING_RATIO)        = "KIND_SNOW_MIXING_RATIO"
RAW_NAMES(KIND_GPSRO)                    = "KIND_GPSRO"
RAW_NAMES(KIND_CLOUD_LIQUID_WATER)       = "KIND_CLOUD_LIQUID_WATER"
RAW_NAMES(KIND_CLOUD_ICE)                = "KIND_CLOUD_ICE"
RAW_NAMES(KIND_CONDENSATIONAL_HEATING)   = "KIND_CONDENSATIONAL_HEATING"
RAW_NAMES(KIND_VAPOR_MIXING_RATIO)       = "KIND_VAPOR_MIXING_RATIO"
RAW_NAMES(KIND_ICE_NUMBER_CONCENTRATION) = "KIND_ICE_NUMBER_CONCENTRATION"
RAW_NAMES(KIND_GEOPOTENTIAL_HEIGHT)      = "KIND_GEOPOTENTIAL_HEIGHT"
RAW_NAMES(KIND_POTENTIAL_TEMPERATURE)    = "KIND_POTENTIAL_TEMPERATURE"
RAW_NAMES(KIND_SOIL_MOISTURE)            = "KIND_SOIL_MOISTURE"

RAW_NAMES(KIND_GRAV_WAVE_DRAG_EFFIC)      = "KIND_GRAV_WAVE_DRAG_EFFIC"
RAW_NAMES(KIND_GRAV_WAVE_STRESS_FRACTION) = "KIND_GRAV_WAVE_STRESS_FRACTION"

RAW_NAMES(KIND_VORTEX_LON)               = "KIND_VORTEX_LON"
RAW_NAMES(KIND_VORTEX_LAT)               = "KIND_VORTEX_LAT"
RAW_NAMES(KIND_VORTEX_PMIN)              = "KIND_VORTEX_PMIN"
RAW_NAMES(KIND_VORTEX_WMAX)              = "KIND_VORTEX_WMAX"

RAW_NAMES(KIND_EXNER_FUNCTION)           = "KIND_EXNER_FUNCTION"
...
function get_raw_obs_kind_name(obs_kind_ind)
  ...
  get_raw_obs_kind_name = RAW_NAMES(obs_kind_ind)
...
function get_raw_obs_kind_index(obs_kind_name)
  ...
  do ii = 1, MAX_RAW_TYPES
     if(trim(adjustl(obs_kind_name)) == trim(adjustl(RAW_NAMES(ii))))&
          & then
        get_raw_obs_kind_index = ii
        return
     end if
  end do
