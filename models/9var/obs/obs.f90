...
use model_mod, only : balance_init, get_model_size

use loc_and_dist_mod, only : loc_type, get_dist, set_loc


private
public num_obs, obs_var, take_obs, ens_ics, state_to_obs, get_num_obs, &
   get_obs_m_rinv, max_num_pos_obs, num_obs_int, obs_var_int, take_obs_int, &
   init_obs, get_close_state, take_single_obs, obs_loc
...
do i = 1, num_obs
   ...
   obs_def(i)%coef(1) = 1.0
!!!   obs_def(i)%coef(1) = 1.0 * i
...
double precision :: obs_var_int(num_obs)
...
double precision :: take_obs_int(num_obs)
double precision, intent(in) :: x(:)
...
integer :: i, ob, sv, j, k
double precision :: obs_variance(num_obs)
