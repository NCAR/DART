...
private
public init_conditions, adv_1step, adv_true_state, &
   output, diag_output_index, advance, balance_init, &
   get_model_size, init_model, model_output, state_loc

integer, parameter :: model_size = 9

!  define model parameters
! c is sqrt(0.75)
double precision, private, parameter :: a(3) = (/1., 1., 3./), &
   b(3) = (/ -1.5, -1.5, 0.5 /), c = 0.8660254, f(3) = (/0.10, 0.0, 0.0/), &
   h(3) = (/-1.0, 0.0, 0.0 /), nu = 1.0 / 48.0, kappa = nu

!  first value for g is lorenz default
double precision, parameter :: g = 8.0
!  second value for g gives higher dimension attractor
!double precision, parameter :: g = 9.90

!  define the model time step
double precision, parameter :: deltat = 1.0 / 12.0

! Define output indices for diagnostics
integer :: diag_output_index(9)

! Define the location of the state variables in module storage
! This is used for distance dependence stuff in more general models
! but is currently just set to give 0 separation distance for all 9 vars
! here.
...
double precision, intent(out) :: dxxx(:)
double precision x(3), y(3), z(3), dx(3), dy(3), dz(3), rnum(9)
integer i, j, k
...
double precision, intent(out) :: pert(9)
...
double precision, intent(in) :: pert(9)
...
double precision, intent(out) :: xnew(9)
...
double precision :: x1(9), x2(9), dx(9)
integer i
...
implicit none
double precision, intent(in) :: xxx(9)
double precision, intent(out) :: init_xxx(9)
double precision x(3), y(3), z(3), dx(3), dy(3), dz(3), wkspce(3)
double precision lhs(3,3),rhs(3)
integer i, j, k, ifail
