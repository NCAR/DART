...
def setup_usgs_daily(
    ...
    if usgs_daily_config['identity_obs']:
        ocp = dart_compile.models__pywatershed__work.exes['create_identity_streamflow_obs']
    else:
    ...
    internal_patches = ['input_files', 'network_file']
    ...
    route_link_f = pathlib.Path(config['pywatershed']['domain_dir']+'/parameters_dis_seg_app.nc')

    if not route_link_f.is_file():
        (output_dir / route_link_f.name).symlink_to(route_link_f)
    input_nml[converter_nml]['network_file'] = route_link_f.name
    ...
    if usgs_daily_config['identity_obs']:
        ...
        # To DO: this line needs serious re-write as the channel restart file could be a text file or a list, unless we want to change that. 
        hydro_rst_file = pathlib.Path(config['initial_ens']['from_filter']['input_nml']['filter_nml']['input_state_file_list']['channel_restart_file_list.txt'])
        (output_dir / hydro_rst_file.name).symlink_to(hydro_rst_file)
        input_nml['model_nml']['domain_order'] = 'channel'
        ...
        (output_dir /'parameters_dis_seg_app.nc').symlink_to(domain_file)
    ...
    print(config_file)
    print(output_dir / 'config_file.yaml')
