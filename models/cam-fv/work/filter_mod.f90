...
!*********************************
! TEMPORARILY MAKE ALL TRACERS AND Q NON-NEGATIVE FOR TESTING CAM bounded quantities
allocate(my_state_indx(state_ens_handle%my_num_vars))
call get_my_vars(state_ens_handle, my_state_indx)
write(*, *) 'my_num_vars ', state_ens_handle%my_num_vars
do j = 1, state_ens_handle%my_num_vars
   call get_state_meta_data(my_state_indx(j), my_state_loc, my_state_kind)
   if(my_state_kind == QTY_SPECIFIC_HUMIDITY .or. my_state_kind == QTY_CLOUD_LIQUID_WATER .or. &
      my_state_kind == QTY_CLOUD_ICE) then
      do i = 1, ens_size
         state_ens_handle%copies(i, j) = max(0.0_r8, state_ens_handle%copies(i, j))
      enddo
   endif
end do
!*********************************
