...
function invoke_diag(dart_output_file, style, metric, var, lev)
    ...
    if strcmp(style, 'profile') == 1
        plot_rmse_xxx_profile(dart_output_file, metric, 'obsname', var, 'verbose', false);
        
    else
        plot_rmse_xxx_evolution(dart_output_file, metric, 'obsname', var, 'level', lev, 'verbose', false);
