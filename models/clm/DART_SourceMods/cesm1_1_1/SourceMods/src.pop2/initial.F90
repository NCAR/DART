...
! /glade/p/cesm/cseg/collections/cesm1_1/models/ocn/pop2/source/initial.F90
...
!
! !REVISION HISTORY:
!  SVN:$Id: initial.F90 39674 2012-08-24 18:14:03Z mlevy@ucar.edu $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   logical (log_kind), public ::  &! context variables
      ...
      ldata_assim,                &! T ==> pop is being run in data assimilation mode !POPDART added by AK Sept 21,2012
...
!-----------------------------------------------------------------------
   ...
   ldata_assim = .false. !POPDART added by AK Sept 21,2012
   ...
   if (my_task == master_task) then
       ...
       select case (init_ts_option)
         case ('ccsm_continue','restart', 'ccsm_branch', 'ccsm_hybrid') 
            !*****POPDART added by AK Sept 21,2012*********
            if (trim(init_ts_suboption) == 'data_assim' ) then
                ldata_assim = .true.
                write(stdout,*) "AK: POPDART: Assume restarts followed an assimilation" 
            endif
            !**********************
   ...
   call broadcast_scalar(ldata_assim          , master_task) !POPDART added by AK sept 21,2012
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then  ! TJH DEBUG on top of AK Sept 21 2012 
      write(stdout,*) "DARTmessage: The init_ts_option    is ", trim(init_ts_option)
      write(stdout,*) "DARTmessage: The init_ts_suboption is ", trim(init_ts_suboption)
      write(stdout,*) "DARTmessage: The ldata_assim value is ", ldata_assim
   endif
...
!-----------------------------------------------------------------------
   ...
   case ('ccsm_continue', 'restart')
      ...
      !******POPDART added by AK Sept 11,2013*******************************
      !! If ldata_assim is true then the two time slices of POP states have 
      !! been modified by restart.F90:DART_modify_restart() to be identical.  
      !! Therefore, we must force the first step after data assimilation to 
      !! be forward euler. This happens when first_step = .true.     
       
      if (ldata_assim) then
         first_step   = .true.
         if (my_task == master_task) &
            write(stdout,*) 'DART modification ... forward euler timestep.'
      else
         if (my_task == master_task) &
            write(stdout,*) 'DART report ... standard timestep.'
      endif

      !******POPDART added by AK Sept 21,2012*******************************
      !! added the ldata_assim flag that changes read_restart
      call read_restart(init_ts_file,lccsm_branch,lccsm_hybrid, &
                        init_ts_file_fmt, errorCode, ldata_assim=ldata_assim)
