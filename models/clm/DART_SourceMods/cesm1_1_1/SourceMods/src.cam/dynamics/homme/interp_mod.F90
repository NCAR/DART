...
! /glade/p/cesm/cseg/collections/cesm1_1_1/models/atm/cam/src/dynamics/homme/interp_mod.F90
...
contains
  ...
  subroutine add_interp_attributes(file)
    ...
    if(itmp == 0) then
       ierr = pio_put_att(file, PIO_GLOBAL, 'interp_type', 'homme basis functions')
    else if(itmp == 1) then
  ...
  subroutine write_interpolated_scalar(File, varid, fld, numlev, data_type, decomp_type) 
    use pio, only : file_desc_t, io_desc_t, var_desc_t, pio_write_darray, iosystem_desc_t, pio_initdecomp, pio_freedecomp, pio_setdebuglevel
  ...
  ! KDR BUGFIX: allow write_interpolated_YYY to call shr_pio_getiosys with 
  ! 'ATM####' instead of 'ATM'.  #### is the instance number.
  ...
  ! KDR 'ATM' doesn't work for multi-instance runs.
