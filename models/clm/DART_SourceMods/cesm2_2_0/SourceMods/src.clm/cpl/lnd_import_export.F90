...
! DART note: this file started life as release-cesm2.2.01 :
! /glade/work/thoar/CESM/my_cesm_sandbox/components/clm/src/cpl/mct/lnd_import_export.F90
!
! Its only purpose is to change any unphysical downward negative fluxes from the forcing files.
! Some of the preprocessed fluxes in DS199.1 have numerical artifacts that leave them slightly negative.
! These should have been replaced on the RDA, but may exist for users who are using old files.
...
!  TJH  1/31/18 Some of the preprocessed fluxes have numerical artifacts that leave them slightly negative
       ...
       if ( atm2lnd_inst%forc_lwrad_not_downscaled_grc(g) <= 0.0_r8 )then
          write(iulog,*) 'WARNING: Longwave down =',atm2lnd_inst%forc_lwrad_not_downscaled_grc(g) ,' gridcell index = ', g           
          atm2lnd_inst%forc_lwrad_not_downscaled_grc(g) = tiny(1.0_r8)
       end if
