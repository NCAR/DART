...
! /glade/p/cesm/cseg/collections/cesm1_2_1/models/ocn/pop2/source/forcing.F90
...
!******POPDART AK ALICIAK... hardcoding the qsw crit to 

qsw_eps = -0.5e-1_r8 !ALICIAK

!POPDART, this was done because negative sw values were in the 
!the CPLHIST files from CAMDART and were crashing POP
!relaxing the critical value allowed the integration to continue.

if (my_task == master_task) then
write(stdout,*) "THE QSW_CRIT HAS BEEN CHANGED FOR ASSIMILATION" !ALICIAK
endif
!************************************************
...
!-----------------------------------------------------------------------
   ...
   if (ANY(SHF_QSW < qsw_eps)) then
      write(6,*) "AK AT END OF set_surface forcing"    !ALICIAK POPDART AK
      write(6,*) "AK THE minval is: ", MINVAL(SHF_QSW) !ALICIAK POPDART AK
      write(6,*) "AK THE minloc is: ", MINLOC(SHF_QSW) !ALICIAK POPDART AK
