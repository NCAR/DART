...
! /glade/p/cesm/cseg/collections/cesm1_2_1/models/ocn/pop2/source/restart.F90
...
! !USES:
   ...
   use operators, only: div,grad !!POPDART added by AK on Sept 21,2012
   ...
   use overflows
   use overflow_type
   use global_reductions !!POPDART added by AK on Sept 21,2012
...
! !INTERFACE:
 ...
 subroutine read_restart(in_filename,lccsm_branch,lccsm_hybrid, &
                         in_restart_fmt, errorCode, ldata_assim ) !POPDART added by AK on Sept 21,2012
...
! !INPUT PARAMETERS:
 ...
 logical (POP_logical), intent(in), optional :: &
      ldata_assim   ! flag if continuation after DART data assimilation  !POPDART added by AK on Sept 21,2012
...
!****POPDART added by AK on Sept 21,2012

if (present(ldata_assim)) then
   if (ldata_assim) then
      if (my_task == master_task) then
         write(stdout,*) 'read_restart: calling DART_modify_restart because ldata_assim = ',ldata_assim
         call POP_IOUnitsFlush(POP_stdout)
         call POP_IOUnitsFlush(stdout)
      endif
      call DART_modify_restart
   endif
else
   if (my_task == master_task) then
      write(stdout,*) 'read_restart: ldata_assim not present.'
   endif
endif
...
!***********************************************************************
!BOP
! !IROUTINE: DART_modify_restart
! !INTERFACE:

 subroutine DART_modify_restart

! !DESCRIPTION:
!  This routine modifies POP restart variables as needed
!  when initializing POP from restart files which are altered by
!  the DART data assimilation code (init_ts_suboption == 'data_assim').
!
! !REVISION HISTORY:
!  same as module
!
...
!-----------------------------------------------------------------------
   ...
   real (POP_r8), dimension(nx_block,ny_block) :: &
      WORK1,WORK2          ! local work space, single-block

   real (POP_r8), dimension(nx_block,ny_block,max_blocks_clinic) :: &
      WORK3, WORK4         ! local work space, multi-block

   real (POP_r8) ::           &
      psurf_hor_ave,       &! area-weighted mean of PSURF
      psurf_hor_area,      &! total area of PSURF
      psurf_mean            ! mean of PSURF
...
!-----------------------------------------------------------------------
!
!  recalculate UBTROP_CUR and VBTROP_CUR as vertical integrals
!  of UVEL_CUR and VVEL_CUR
!	
!  !!!NEEDS MODS FOR OVERFLOWS!!!
!
!-----------------------------------------------------------------------
   WORK3 = c0  ! initialize sums
   WORK4 = c0

   if (partial_bottom_cells) then
      do k = 1,km
         WORK3 = WORK3 + UVEL(:,:,k,curtime,:)*DZU(:,:,k,:)
         WORK4 = WORK4 + VVEL(:,:,k,curtime,:)*DZU(:,:,k,:)
      enddo
   else
      do k = 1,km
         WORK3 = WORK3 + UVEL(:,:,k,curtime,:)*dz(k)
         WORK4 = WORK4 + VVEL(:,:,k,curtime,:)*dz(k)
      enddo
   endif

   UBTROP(:,:,curtime,:) = WORK3*HUR    ! normalize by dividing by depth
   VBTROP(:,:,curtime,:) = WORK4*HUR    ! normalize by dividing by depth

!-----------------------------------------------------------------------
!
!  ensure zero horizontal mean PSURF_CUR, seperately in marginal seas
!	and open ocean.
!
!-----------------------------------------------------------------------
   do n = 0,num_regions
      ...
      WORK4 = c0
      if (n.eq.0) then
        WORK3 = merge(TAREA*PSURF(:,:,curtime,:),c0, REGION_MASK > 0)
        WORK4 = merge(TAREA,c0, REGION_MASK > 0)
      else
        if (region_info(n)%marginal_sea) then
           WORK3 = merge(TAREA*PSURF(:,:,curtime,:), c0, &
               REGION_MASK == region_info(n)%number)
           WORK4 = merge(TAREA, c0, &
               REGION_MASK == region_info(n)%number)
        endif
      endif

      psurf_hor_ave  = global_sum(WORK3, distrb_clinic, field_loc_center)
      psurf_hor_area = global_sum(WORK4, distrb_clinic, field_loc_center)
      if (psurf_hor_area > c0) then
         psurf_mean = psurf_hor_ave/psurf_hor_area
      else
         psurf_mean = c0
      endif
      ...
      if (n.eq.0) then
         if (my_task == master_task) then
         write(stdout,*)'(DART_modify_restart): region_id = ',0, & 
                 ' psurf_mean = ', psurf_mean
         end if
      else
         if (my_task == master_task) then
        write(stdout,*)'(DART_modify_restart): region_id = ', &
                 region_info(n)%number, ' psurf_mean = ', psurf_mean
         endif
      endif
      call POP_IOUnitsFlush(POP_stdout) ; call POP_IOUnitsFlush(stdout)

      where (WORK4 > c0)
         PSURF(:,:,curtime,:) = PSURF(:,:,curtime,:) - psurf_mean
      endwhere
...
!-----------------------------------------------------------------------
!
!  recompute GRADPX_CUR and GRADPY_CUR using new PSURF_CUR
!
!-----------------------------------------------------------------------
   do iblock = 1,nblocks_clinic
      this_block = get_block(blocks_clinic(iblock),iblock)
...
!-----------------------------------------------------------------------
   do iblock = 1,nblocks_clinic
      ...
      PGUESS(:,:,iblock) = PSURF(:,:,curtime,iblock)
...
!-----------------------------------------------------------------------
   do iblock = 1,nblocks_clinic
        ...
        PSURF(:,:,oldtime,iblock) = PSURF(:,:,curtime,iblock)
        GRADPX(:,:,oldtime,iblock) = GRADPX(:,:,curtime,iblock)
        GRADPY(:,:,oldtime,iblock) = GRADPY(:,:,curtime,iblock)
        do k = 1,km
          UVEL(:,:,k,oldtime,iblock) = UVEL(:,:,k,curtime,iblock)
          VVEL(:,:,k,oldtime,iblock) = VVEL(:,:,k,curtime,iblock)
          do n = 1,2
            TRACER(:,:,k,n,oldtime,iblock) = TRACER(:,:,k,n,curtime,iblock)
          enddo
        enddo
...
!EOC
 ...
 end subroutine DART_modify_restart
...
!-----------------------------------------------------------------------
   ...
   if (check_time_flag(restart_flag) .or.   &
      ...
      (nsteps_this_interval == nsteps_per_interval)) ) then
