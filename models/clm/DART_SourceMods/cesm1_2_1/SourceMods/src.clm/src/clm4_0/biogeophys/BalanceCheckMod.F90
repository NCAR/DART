...
! /glade/p/cesm/releases/cesm1_2_1/models/lnd/clm/src/clm4_0/biogeophys/BalanceCheckMod.F90
...
! !USES:
    ...
    use subgridAveMod
    use clm_time_manager , only : get_step_size, get_nstep, is_first_restart_step
...
!-----------------------------------------------------------------------
    ...
    if ( found .and. (.not. is_first_restart_step()) ) then ! TJH
       ...
       if ((ctype(indexc) .eq. icol_roof .or. ctype(indexc) .eq. icol_road_imperv .or. &
          ...
          call endrun('clm urban model is stopping - water balance error is greater than 1.e-1')
          ...
          write(iulog,*)'qflx_snwcp_ice   = ',qflx_snwcp_ice(indexc)
          write(iulog,*)'clm model is stopping'
          call endrun('clm model is stopping - water balance error is greater than .10')
       end if
    end if
    ...
    if ( found .and. (.not. is_first_restart_step()) ) then ! TJH
       ...
       if (abs(errh2osno(indexc)) > 0.1_r8 .and. (nstep > 2) ) then
          ...
          write(iulog,*)'clm model is stopping'
          call endrun('clm model is stopping - snow balance error is greater than .10')
       end if
    end if
    ...
    if ( found  .and. (nstep > 2) .and. (.not. is_first_restart_step() ) ) then ! TJH
       ...
       write(iulog,*)'clm model is stopping'
       call endrun('BalanceCheck: solar radiation balance error')
    end if
    ...
    if ( found  .and. (nstep > 2) .and. (.not. is_first_restart_step() ) ) then ! TJH
       ...
       write(iulog,*)'clm model is stopping'
       call endrun('BalanceCheck: longwave energy balance error')
    end if
    ...
    if ( found  .and. (nstep > 2) .and. (.not. is_first_restart_step() ) ) then ! TJH
       ...
       write(iulog,*)'clm model is stopping'
       call endrun('BalanceCheck: surface flux energy balance error')
    end if
    ...
    if ( found .and. (.not. is_first_restart_step() ) ) then ! TJH
       ...
       if (abs(errsoi_col(indexc)) > .10_r8 .and. (nstep > 2) ) then
          write(iulog,*)'clm model is stopping'
          call endrun('BalanceCheck: soil balance error')
       end if
    end if
