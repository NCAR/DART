...
! /glade/p/cesmdata/releases/cesm1_2_1/models/lnd/clm/src/clm4_0/biogeophys/SnowHydrologyMod.F90
!
! NOTE: It includes a modified snow grain radius computation documented in bugzilla
!       report 1934.
    ...
    do fc = 1, num_snowc
       ...
       if (msno > 1) then
          if (dzsno(c,1) > 0.02_r8) then
             ...
             !mgf++ 20140225: Mass-weighted combination of radius  ...  bugzilla 1934
             !rds(c,2) = rds(c,1) ! (combo)
             rds(c,2) = (rds(c,2)*(swliq(c,2)+swice(c,2)) + &
                         rds(c,1)*(zwliq+zwice))/(swliq(c,2)+swice(c,2)+zwliq+zwice)
             !mgf-- bugzilla 1934
       ...
       if (msno > 2) then
          if (dzsno(c,2) > 0.05_r8) then
             ...
             rds(c,3) = (rds(c,3)*(swliq(c,3)+swice(c,3)) + &
                         rds(c,2)*(zwliq+zwice))/(swliq(c,3)+swice(c,3)+zwliq+zwice)
             !mgf-- bugzilla 1934
       ...
       if (msno > 3) then
          if (dzsno(c,3) > 0.11_r8) then
             ...
             rds(c,4) = (rds(c,4)*(swliq(c,4)+swice(c,4)) + &
                         rds(c,3)*(zwliq+zwice))/(swliq(c,4)+swice(c,4)+zwliq+zwice)
             !mgf-- bugzilla 1934
       ...
       if (msno > 4) then
          if (dzsno(c,4) > 0.23_r8) then
             ...
             rds(c,5) = (rds(c,5)*(swliq(c,5)+swice(c,5)) + &
                         rds(c,4)*(zwliq+zwice))/(swliq(c,5)+swice(c,5)+zwliq+zwice)
             !mgf-- bugzilla 1934
