...
! /glade/p/cesm/releases/cesm1_2_1/models/lnd/clm/src/clm4_5/biogeochem/CNBalanceCheckMod.F90
...
! !USES:
   use clmtype
   use clm_time_manager, only: get_step_size, is_first_restart_step
!
! !ARGUMENTS:
   implicit none
   integer, intent(in) :: lbc, ubc        ! column bounds
...
!-----------------------------------------------------------------------
   ...
   if ( err_found .and. (.not. is_first_restart_step()) ) then ! TJH
      c = err_index
      ...
      call endrun('column carbon balance')
   end if
...
!-----------------------------------------------------------------------
   ...
   if ( err_found .and. (.not. is_first_restart_step()) ) then ! TJH
      ...
      call endrun('column nitrogen balance error')
   end if
