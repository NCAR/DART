...
! /glade/p/cesmdata/cseg/collections/cesm1_2_1/models/atm/cam/src/chemistry/mozart/mo_usrrxt.F90
...
!
             ...
             do m=1,nspec_amode(l)
               if ( index_tot_mass(l,m) > 0 ) tot_mass = tot_mass + mmr(i,k,index_tot_mass(l,m))
               if ( index_chm_mass(l,m) > 0 ) chm_mass = chm_mass + mmr(i,k,index_chm_mass(l,m))
             end do
             if ( tot_mass > 0._r8 ) then
               sad_mode(i,k,l) = chm_mass/tot_mass * mmr(i,k,num_idx(l))*rho_air*pi*dgnumwet(i,k,l)**2*exp(2*alnsg_amode(l)**2)  ! m^2/m^3
