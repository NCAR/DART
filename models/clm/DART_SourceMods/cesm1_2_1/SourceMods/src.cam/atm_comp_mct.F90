...
! /glade/p/cesm/cseg/collections/cesm1_2_1/models/atm/cam/src/cpl_mct/atm_comp_mct.F90
!
! This atm_comp_mod.F90 forces CAM to use initial conditions file while 
! all other model components use restart files. This allows us to run 
...
! avoid lagging the ocean by a day. The time is also read from the coupler. 
! -- TJH Tue Mar  4 13:57:49 MST 2014
...
!================================================================================
  ...
  subroutine atm_init_mct( EClock, cdata_a, x2a_a, a2x_a, NLFilename )
    ...
    integer :: currenttod       ! current time of day
    ...
    if (masterproc) write(iulog,*)'TJH atm_init_mct: first_time is ',first_time
    ...
    if (first_time) then
       ...
       if (dart_mode) then
          ! TJH : force the atm into an initial run while everyone else is a restart
          ! TJH : This allows us to (potentially) use POP in the restart mode
          ! TJH : huge step towards coupled assimilation with coupled models.
          starttype = trim(seq_infodata_start_type_start)
       end if
       ...
       call seq_timemgr_EClockGetData(EClock, &
                                      ...
                                      curr_ymd=CurrentYMD, curr_tod=CurrentTOD, &
                                      calendar=calendar )

       ! In DART mode we allow the coupler to be running in a restart mode,
       ! but force CAM into an initial mode.  To do this CAM's start time must
       ! be set to the current time from the driver's clock.
       if (dart_mode) then
          if (masterproc) write(iulog,*)'TJH timemgr ref   ',ref_ymd, ref_tod
          if (masterproc) write(iulog,*)'TJH timemgr start ',start_ymd, start_tod
          start_ymd = CurrentYMD
          start_tod = CurrentTOD
          if (masterproc) write(iulog,*)'TJH reset   to    ',start_ymd, start_tod
       end if
       ...
       if (masterproc) write(iulog,*)'TJH StepNo check. StepNo is ',StepNo
...
!
  subroutine atm_read_srfrest_mct( EClock, cdata_a, x2a_a, a2x_a)
    ...
    if (dart_mode) then
       write(str_year ,'(i4.4)') yr_spec
       write(str_month,'(i2.2)') mon_spec
       write(str_day  ,'(i2.2)') day_spec
       write(str_sec  ,'(i5.5)') sec_spec
       fname_srf_cam = trim(caseid) //".cam"// trim(inst_suffix) //".rs."// str_year //"-"// str_month //"-"// str_day //"-"// str_sec //".nc"
       call getfil(fname_srf_cam, fname_srf_cam)
    else
