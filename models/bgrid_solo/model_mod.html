...
<TR><TD>dt_bias</TD>
    ...
    <TD> Allows a simple mechanism to simulate model error. If dt_bias is non-zero, the assimilation 
         ...
         one time step for the internal bgrid clock (for instance dt_atmos = 3600, dt_bias = 3600), and filter with
         another (dt_atmos = 3000, and dt_bias = 3600) model error is simulated.
        </TD>
</TR>

<TR><TD>state_variables(:,2)</TD>
    <TD>character(len=129)</TD>
    <TD>Strings that identify the bgrid_solo variables that should be part of the DART
        state vector. The first column is the netCDF variable name, the second column
        is the corresponding DART quantity.
        </TD>
</TR>

<TR><TD>template_file</TD>
    <TD>character(len=256)</TD>
    <TD>This is the name of the file that specifies the resolution of the variables 
        DART uses to create the DART state vector. 
        If <em class=code>template_file&nbsp;=&nbsp;"null"</em> the 
        <em class=code>&amp;bgrid_cold_start_nml</em> namelist variables
        are used to specify the resolution. The actual input filenames for
        <em class=program>filter</em> and
        <em class=program>perfect_model_obs</em> come from their respective namelists.
        The resolutions in the file specified in <em class=code>template_file</em> must 
        match the resolutions of the variables in the input filenames. To start an experiment with a 
        new model resolution, set template_file to "null" and set the resolutions in bgrid_cold_start_nml.
        </TD>
</TR>


<TR><TD colspan=3><hr></TD></TR>
<TR><TD colspan=3>The following values are set in the bgrid_cold_start_nml</TD></TR>

<TR><TD>nlon</TD>
    <TD>integer</TD>
    <TD> The number of longitudes on the model grid.
        </TD>
</TR>

<TR><TD>nlat</TD>
    <TD>integer</TD>
    <TD> The number of latitudes on the model grid.
        </TD>
</TR>

<TR><TD>nlev</TD>
    <TD>integer</TD>
    <TD> The number of model levels.
        </TD>
</TR>

<TR><TD>equal_vertical_spacing</TD>
    <TD>logical</TD>
    <TD> Model levels are equally spaced in pressure if true. 
        </TD>
</TR>
...
<P>
The Held-Suarez forcing details can be modified with the hs_forcing_nml namelist using the documentation in
Held and Suarez (1994). 
<P>Model dynamics can be adjusted with the bgrid_core_driver_nml following
the documentation in the references and internal documentation in the bgrid code. 
...
<P>Know that these modules have their own dependencies</P>
...
<P><!-- just to make the 'top' widget work --></P>
...
<TR><TD colspan=2>Files used or created by <em class=program>perfect_model_obs</em></TD>
<TR><TD colspan=2>&nbsp;</TD>

<TR><TH align=left>filename</TH>
    <TH align=left>purpose</TH></TR>
<TR><TD valign=top><em class=file>input.nml</em></TD>
    <TD valign=top>to read the namelist specifying the run-time control</TD></TR>
<TR><TD valign=top><em class=file>perfect_input.nc</em></TD>
    <TD valign=top>the initial model state (if not cold-start)</TD></TR>
<TR><TD valign=top><em class=file>obs_seq.in</em></TD>
    <TD valign=top>the metadata for the desired synthetic observations</TD></TR>
<TR><TD valign=top><em class=file>perfect_output.nc</em></TD>
    <TD valign=top>the time-history of the model state - the True State</TD></TR>
<TR><TD valign=top><em class=file>obs_seq.out</em></TD>
    <TD valign=top>the synthetic observations created from the True State</TD></TR>
<TR><TD valign=top><em class=file>dart_log.[out,nml]</em></TD>
    <TD valign=top>run-time output</TD></TR>

<TR><TD colspan=2>&nbsp;</TD>
<TR><TD colspan=2>Files used or created by <em class=program>filter</em></TD>
<TR><TD colspan=2>&nbsp;</TD>

<TR><TH align=left>filename</TH>
    <TH align=left>purpose</TH></TR>

<TR><TD valign=top><em class=file>filter_input_list.txt</em></TD>
    <TD valign=top>The file specifying the filename(s) of the initial ensemble.
        The default filename for the initial ensemble is <em class=file>filter_input.nc</em> 
    </TD></TR>

<TR><TD valign=top><em class=file>obs_seq.out</em></TD>
    <TD valign=top>the observations to be assimilated</TD></TR>
<TR><TD valign=top><em class=file>preassim.nc</em></TD>
    <TD valign=top>the ensemble information after prior inflation but before assimilation</TD></TR>
<TR><TD valign=top><em class=file>postassim.nc</em></TD>
    <TD valign=top>the ensemble information after assimilation but before posterior inflation</TD></TR>

<TR><TD valign=top><em class=file>filter_output_list.txt&nbsp;&nbsp;&nbsp;</em></TD>
    <TD valign=top>the file specifying the filename(s) of the time-evolution of the ensemble
        after all inflations and assimilation - the 'posterior'.
        The default filename for the output ensemble is <em class=file>filter_output.nc</em> 
    </TD></TR>

<TR><TD valign=top><em class=file>obs_seq.final</em></TD>
    <TD valign=top>the original observations and (optionally) the expected observations 
        from the ensemble of model states. This may contain both the prior and posterior
        expected observation values.</TD></TR>
<TR><TD valign=top><em class=file>dart_log.[out,nml]</em></TD>
    <TD valign=top>run-time output</TD></TR>
...
<!--==================================================================-->
<!-- Cite references, if need be/possible.                            -->
<!--==================================================================-->
...
<ol>
<li>
<a href="http://journals.ametsoc.org/doi/full/10.1175/JCLI-3223.1">
Anderson, J. L. and Coauthors, 2004</a>: The new GFDL global atmosphere and land model AM2-LM2: 
Evaluation with prescribed SST simulations. J. Climate, 17, 4641-4673.
<li>
<a href="http://journals.ametsoc.org/doi/full/10.1175/JAS3510.1">
Anderson, J. L., Wyman, B., Zhang, S. & Hoar, T., 2005</a>: Assimilation of surface pressure 
observations using an ensemble filter in an idealized global atmospheric prediction system, 
J. Atmos. Sci., 62, 2925-2938.
<li>
<a href="http://journals.ametsoc.org/doi/abs/10.1175/1520-0477(1994)075%3C1825%3AAPFTIO%3E2.0.CO%3B2">
Held, I. M., and M. J. Suarez, 1994</a>: A proposal for the intercomparison
