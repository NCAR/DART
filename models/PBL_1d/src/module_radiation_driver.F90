...
! !INTERFACE:
      SUBROUTINE radiation_driver( itimestep                     &
      ,JULDAY,JULIAN,JULYR                                       &
      ,GMT,DT,XTIME,XLAT,XLONG,RADT                              &
      ,RA_CALL_OFFSET,STEPRA,NPHS,soltopfrac                     &
      ,ra_lw_physics,ra_sw_physics                               &
      ,ALBEDO,EMISS,XICE,XLAND,VEGFRA                            &
      ,SNOW,WARM_RAIN                                            &
      ,TSK,T8W,T,P8W,P,PI,RHO,Z,DZ8W                             &
      ,HBOT,HTOP,HBOTR,HTOPR,ICLOUD                              &
      ,GLW,GSW                                                   &
      ,RSWTOA,RLWTOA                                             &
      ,RTHRATEN,RTHRATENLW,RTHRATENSW                            &
      ,SWDOWN,SWDOWNC                                            &
      ,TAUCLDC,TAUCLDI                                           &
      ,ACFRCV,ACFRST,CFRACH,CFRACL,CFRACM                        &
      ,CUPPT,CZMEAN,NCFRCV,NCFRST                                &
...
! *** add new modules of schemes here
   ...
   USE module_ra_sw
!   USE module_ra_gsfcsw  ! not implemented in this version (RT)
   USE module_ra_rrtm
!   USE module_ra_cam     ! not implemented in this version (RT)
!   USE module_ra_gfdleta ! not implemented in this version (RT)
...
! set in <a href=set_scalar_indices_from_config.html>set_scalar_indices_from_config</a> 
...
!   INTEGER, INTENT(IN)            :: lw_physics, sw_physics !now local vars (see below)...RT
   CHARACTER(len=15) :: ra_lw_physics, ra_sw_physics
...
! LOCAL  VAR
   ...
   INTEGER            :: lw_physics, sw_physics !now local vars...RT
...
!-------------------------------------------------
   REAL, OPTIONAL, INTENT(INOUT) :: DECLIN_URB  !urban
...
!   if (lw_physics_i .eq. 0 .and. sw_physics_i .eq. 0)         return !commented out ...RT
...
!     3rd condition added (STEPRA.EQ.1), with radt set to 0 in namelist, to allow call to radiation every time step ...RT
      Radiation_step: IF (itimestep .eq. 1 .or. mod(itimestep,STEPRA) .eq. 1 + ra_call_offset .or. STEPRA .eq. 1) THEN

! CAM-specific additional radiation frequency - cam_abs_freq_s (=21600s by default)
!RT     STEPABS = nint(cam_abs_freq_s/(dt*STEPRA))*STEPRA
!RT    IF (itimestep .eq. 1 .or. mod(itimestep,STEPABS) .eq. 1 + ra_call_offset) THEN
!RT       doabsems = .true.
!RT     ELSE
!RT       doabsems = .false.
!RT     ENDIF
...
! Calculate constant for short wave radiation
     ...
     SOLCON = SOLCON*(1.0-soltopfrac) ! attenuation to account for a model top not at TOA ...RT

     if(present(DECLIN_URB))DECLIN_URB=DECLIN  ! urban

     lwrad_cldfra_select: SELECT CASE(ra_lw_physics)
...
!RT     IF ( PRESENT ( CLDFRA ) .AND.                           &
!RT          PRESENT(F_QC) .AND. PRESENT ( F_QI ) ) THEN
! Call to cloud fraction routine based on Randall 1994 (Hong Pan 1998)

!RT   CALL cal_cldfra2(CLDFRA,qv,qc,qi,qs,                     &
!RT                   F_QV,F_QC,F_QI,F_QS,t,p,                &
!RT                   F_ICE_PHY,F_RAIN_PHY,                   &
...
!pjj/cray  Cray X1 cannot print from threaded region
     ...
     lwrad_select: SELECT CASE(ra_lw_physics)
        ...
        CASE ('GFDLLWSCHEME')
           WRITE( wrf_err_message , * )'This LW option is not supported' !RT
!RT             lw_physics = 99
!RT             CALL wrf_debug (100, 'CALL gfdllw')

!RT             IF ( PRESENT(F_QV) .AND. PRESENT(F_QC) .AND.                     &
!RT                  PRESENT(F_QS) .AND. PRESENT(qs)   .AND.                     &
!RT                  PRESENT(qv)   .AND. PRESENT(qc)   ) THEN
!RT               IF ( F_QV .AND. F_QC .AND. F_QS) THEN
!RT                 gfdl_lw  = .true.
!RT                 CALL ETARA(                                        &
!RT                  DT=dt,XLAND=xland                                 &
!RT                 ,P8W=p8w,DZ8W=dz8w,RHO_PHY=rho,P_PHY=p,T=t         &
!RT                 ,QV=qv,QW=qc_temp,QI=qi,QS=qs                      &
!RT                 ,TSK2D=tsk,GLW=GLW,RSWIN=SWDOWN,GSW=GSW            &
!RT                 ,RSWINC=SWDOWNC,CLDFRA=CLDFRA,PI3D=pi              &
!RT                 ,GLAT=glat,GLON=glon,HTOP=htop,HBOT=hbot           &
!RT                 ,HBOTR=hbotr, HTOPR=htopr                          &
!RT                 ,ALBEDO=albedo,CUPPT=cuppt                         &
!RT                 ,VEGFRA=vegfra,SNOW=snow,G=g,GMT=gmt               &
!RT                 ,NSTEPRA=stepra,NPHS=nphs,ITIMESTEP=itimestep      &
!RT                 ,XTIME=xtime,JULIAN=julian                         &
!RT                 ,COSZ_URB2D=COSZ_URB2D  ,OMG_URB2D=omg_urb2d       &
!RT                 ,JULYR=julyr,JULDAY=julday                         &
!RT                 ,GFDL_LW=gfdl_lw,GFDL_SW=gfdl_sw                   &
!RT                 ,CFRACL=cfracl,CFRACM=cfracm,CFRACH=cfrach         &
!RT                 ,ACFRST=acfrst,NCFRST=ncfrst                       &
!RT                 ,ACFRCV=acfrcv,NCFRCV=ncfrcv                       &
!RT                 ,RSWTOA=rswtoa,RLWTOA=rlwtoa,CZMEAN=czmean         &
!RT                 ,THRATEN=rthraten,THRATENLW=rthratenlw             &
!RT                 ,THRATENSW=rthratensw                              &
!RT                 ,IDS=ids,IDE=ide, JDS=jds,JDE=jde, KDS=kds,KDE=kde &     
!RT                 ,IMS=ims,IME=ime, JMS=jms,JME=jme, KMS=kms,KME=kme &
!RT                 ,ITS=its,ITE=ite, JTS=jts,JTE=jte, KTS=kts,KTE=kte &
!RT                                                                    )
!RT               ELSE
!RT                 CALL wrf_error_fatal('Can not call ETARA (1a). Missing moisture fields.')
!RT               ENDIF
!RT             ELSE
!RT               CALL wrf_error_fatal('Can not call ETARA (1b). Missing moisture fields.')
!RT             ENDIF
        CASE ('CAMLWSCHEME')
           WRITE( wrf_err_message , * )'This LW option is not supported' !RT
!RT             lw_physics = 3
!RT             CALL wrf_debug(100, 'CALL camrad lw')
!RT             IF(cam_abs_dim1 .ne. 4 .or. cam_abs_dim2 .ne. kde .or.  &
!RT                paerlev .ne. 29 .or. levsiz .ne. 59 )THEN
!RT               WRITE( wrf_err_message , * ) &
!RT'set paerlev=29, levsiz=59, cam_abs_dim1=4, and cam_abs_dim2=number of levels (e_vert) in physics namelist for CAM radiation'
!RT               CALL wrf_error_fatal ( wrf_err_message )
!RT             ENDIF
!RT             IF ( PRESENT( OZMIXM ) .AND. PRESENT( PIN ) .AND. &
!RT                  PRESENT(M_PS_1) .AND. PRESENT(M_PS_2) .AND.  &
!RT                  PRESENT(M_HYBI0) .AND. PRESENT(AEROSOLC_1)    &
!RT                  .AND. PRESENT(AEROSOLC_2) ) THEN
!RT             CALL CAMRAD(RTHRATENLW=RTHRATEN,RTHRATENSW=RTHRATENSW,    &
!RT                     SWUPT=SWUPT,SWUPTC=SWUPTC,                        &
!RT                     SWDNT=SWDNT,SWDNTC=SWDNTC,                        &
!RT                     LWUPT=LWUPT,LWUPTC=LWUPTC,                        &
!RT                     LWDNT=LWDNT,LWDNTC=LWDNTC,                        &
!RT                     SWUPB=SWUPB,SWUPBC=SWUPBC,                        &
!RT                     SWDNB=SWDNB,SWDNBC=SWDNBC,                        &
!RT                     LWUPB=LWUPB,LWUPBC=LWUPBC,                        &
!RT                     LWDNB=LWDNB,LWDNBC=LWDNBC,                        &
!RT                     SWCF=SWCF,LWCF=LWCF,OLR=OLR,CEMISS=CEMISS,        &
!RT                     TAUCLDC=TAUCLDC,TAUCLDI=TAUCLDI,COSZR=COSZR,      &
!RT                     GSW=GSW,GLW=GLW,XLAT=XLAT,XLONG=XLONG,            &
!RT                     ALBEDO=ALBEDO,t_phy=t,TSK=TSK,EMISS=EMISS         &
!RT                    ,QV3D=qv                                           &
!RT                    ,QC3D=qc                                           &
!RT                    ,QR3D=qr                                           &
!RT                    ,QI3D=qi                                           &
!RT                    ,QS3D=qs                                           &
!RT                    ,QG3D=qg                                           &
!RT                    ,F_QV=f_qv,F_QC=f_qc,F_QR=f_qr                     &
!RT                    ,F_QI=f_qi,F_QS=f_qs,F_QG=f_qg                     &
!RT                    ,f_ice_phy=f_ice_phy,f_rain_phy=f_rain_phy         &
!RT                    ,p_phy=p,p8w=p8w,z=z,pi_phy=pi,rho_phy=rho,        &
!RT                     dz8w=dz8w,                                        &
!RT                     CLDFRA=CLDFRA,XLAND=XLAND,XICE=XICE,SNOW=SNOW,    &
!RT                     ozmixm=ozmixm,pin0=pin,levsiz=levsiz,             &
!RT                     num_months=n_ozmixm,                              &
!RT                     m_psp=m_ps_1,m_psn=m_ps_2,aerosolcp=aerosolc_1,   &
!RT                     aerosolcn=aerosolc_2,m_hybi0=m_hybi0,             &
!RT                     paerlev=paerlev, naer_c=n_aerosolc,               &
!RT                     cam_abs_dim1=cam_abs_dim1, cam_abs_dim2=cam_abs_dim2, &
!RT                     GMT=GMT,JULDAY=JULDAY,JULIAN=JULIAN,DT=DT,XTIME=XTIME,DECLIN=DECLIN,  &
!RT                     SOLCON=SOLCON,RADT=RADT,DEGRAD=DEGRAD,n_cldadv=3  &
!RT                   ,abstot_3d=abstot,absnxt_3d=absnxt,emstot_3d=emstot &
!RT                   ,doabsems=doabsems                               &
!RT                 ,IDS=ids,IDE=ide, JDS=jds,JDE=jde, KDS=kds,KDE=kde &     
!RT                 ,IMS=ims,IME=ime, JMS=jms,JME=jme, KMS=kms,KME=kme &
!RT                 ,ITS=its,ITE=ite, JTS=jts,JTE=jte, KTS=kts,KTE=kte &
!RT                                                                    )
...
!RT             ENDIF
        CASE DEFAULT
             ...
             WRITE( wrf_err_message , * ) 'The longwave option does not exist: ra_lw_physics = ', ra_lw_physics
...
!                   ,COSZ_URB2D=COSZ_URB2D ,OMG_URB2D=omg_urb2d        & !urban
        ...
        CASE ('GSFCSWSCHEME')
           WRITE( wrf_err_message , * )'This LW option is not supported' !RT
!RT             sw_physics = 2
!RT             CALL wrf_debug(100, 'CALL gsfcswrad')
!RT             CALL GSFCSWRAD(                                           &
!RT                     RTHRATEN=rthraten,GSW=gsw,XLAT=xlat,XLONG=xlong   &
!RT                    ,ALB=albedo,T3D=t,P3D=p,P8W3D=p8w,pi3D=pi          &
!RT                    ,DZ8W=dz8w,RHO_PHY=rho                             &
!RT                    ,CLDFRA3D=cldfra                                   &
!RT                    ,GMT=gmt,CP=cp,G=g                                 &
!RT                    ,COSZ_URB2D=COSZ_URB2D ,OMG_URB2D=omg_urb2d            & !urban
!RT                    ,JULDAY=julday,XTIME=xtime                         &
!RT                    ,DECLIN=declin,SOLCON=solcon                       &
!RT                    ,RADFRQ=radt,DEGRAD=degrad                         &
!RT                    ,TAUCLDI=taucldi,TAUCLDC=taucldc                   &
!RT                    ,WARM_RAIN=warm_rain                               &
!RT#ifdef WRF_CHEM
!RT                    ,TAUAER300=tauaer300,TAUAER400=tauaer400           & ! jcb
!RT                    ,TAUAER600=tauaer600,TAUAER999=tauaer999           & ! jcb
!RT                    ,GAER300=gaer300,GAER400=gaer400                   & ! jcb
!RT                    ,GAER600=gaer600,GAER999=gaer999                   & ! jcb
!RT                    ,WAER300=waer300,WAER400=waer400                   & ! jcb
!RT                    ,WAER600=waer600,WAER999=waer999                   & ! jcb
!RT#endif
!RT                    ,IDS=ids,IDE=ide, JDS=jds,JDE=jde, KDS=kds,KDE=kde &     
!RT                    ,IMS=ims,IME=ime, JMS=jms,JME=jme, KMS=kms,KME=kme &
!RT                    ,ITS=its,ITE=ite, JTS=jts,JTE=jte, KTS=kts,KTE=kte &
...
!RT      DO k=kts,kte
!RT      DO i=its,ite
!RT         RTHRATEN(I,K,J)=RTHRATEN(I,K,J)+RTHRATENSW(I,K,J)
!RT      ENDDO
!RT      ENDDO
!RT      ENDDO
        ...
        CASE ('GFDLSWSCHEME')
           WRITE( wrf_err_message , * )'This LW option is not supported' !RT
!RT             sw_physics = 99
!RT             CALL wrf_debug (100, 'CALL gfdlsw')
!RT
!RT             IF ( PRESENT(F_QV) .AND. PRESENT(F_QC) .AND.                     &
!RT                  PRESENT(F_QS) .AND. PRESENT(qs)   .AND.                     &
!RT                  PRESENT(qv)   .AND. PRESENT(qc) ) THEN
...
!RT                                                                    )
!RT               ELSE
!RT                 CALL wrf_error_fatal('Can not call ETARA (2a). Missing moisture fields.')
!RT               ENDIF
!RT             ELSE
!RT               CALL wrf_error_fatal('Can not call ETARA (2b). Missing moisture fields.')
!RT             ENDIF

        CASE DEFAULT

             WRITE( wrf_err_message , * ) 'The shortwave option does not exist: ra_sw_physics = ', ra_sw_physics
             CALL wrf_error_fatal ( wrf_err_message )
   ...
   ENDIF Radiation_step
     ...
     accumulate_lw_select: SELECT CASE(ra_lw_physics)

     CASE ('CAMLWSCHEME')
!RT   IF(PRESENT(LWUPTC))THEN
!RT   !$OMP PARALLEL DO   &
!RT   !$OMP PRIVATE ( ij ,i,j,k,its,ite,jts,jte)

!RT   DO ij = 1 , num_tiles
!RT     its = i_start(ij)
!RT     ite = i_end(ij)
!RT     jts = j_start(ij)
!RT     jte = j_end(ij)
...
!RT        DO j=jts,jte
!RT        DO i=its,ite
!RT           ACLWUPT(I,J) = ACLWUPT(I,J) + LWUPT(I,J)*DT
!RT           ACLWUPTC(I,J) = ACLWUPTC(I,J) + LWUPTC(I,J)*DT
!RT           ACLWDNT(I,J) = ACLWDNT(I,J) + LWDNT(I,J)*DT
!RT           ACLWDNTC(I,J) = ACLWDNTC(I,J) + LWDNTC(I,J)*DT
!RT           ACLWUPB(I,J) = ACLWUPB(I,J) + LWUPB(I,J)*DT
!RT           ACLWUPBC(I,J) = ACLWUPBC(I,J) + LWUPBC(I,J)*DT
!RT           ACLWDNB(I,J) = ACLWDNB(I,J) + LWDNB(I,J)*DT
!RT           ACLWDNBC(I,J) = ACLWDNBC(I,J) + LWDNBC(I,J)*DT
!RT        ENDDO
!RT        ENDDO
!RT   ENDDO
!RT   !$OMP END PARALLEL DO
!RT   ENDIF
     CASE DEFAULT
     END SELECT accumulate_lw_select

     accumulate_sw_select: SELECT CASE(ra_sw_physics)

     CASE ('CAMSWSCHEME')
!RT   IF(PRESENT(SWUPTC))THEN
!RT   !$OMP PARALLEL DO   &
!RT   !$OMP PRIVATE ( ij ,i,j,k,its,ite,jts,jte)
...
!RT        DO j=jts,jte
!RT        DO i=its,ite
!RT           ACSWUPT(I,J) = ACSWUPT(I,J) + SWUPT(I,J)*DT
!RT           ACSWUPTC(I,J) = ACSWUPTC(I,J) + SWUPTC(I,J)*DT
!RT           ACSWDNT(I,J) = ACSWDNT(I,J) + SWDNT(I,J)*DT
!RT           ACSWDNTC(I,J) = ACSWDNTC(I,J) + SWDNTC(I,J)*DT
!RT           ACSWUPB(I,J) = ACSWUPB(I,J) + SWUPB(I,J)*DT
!RT           ACSWUPBC(I,J) = ACSWUPBC(I,J) + SWUPBC(I,J)*DT
!RT           ACSWDNB(I,J) = ACSWDNB(I,J) + SWDNB(I,J)*DT
!RT           ACSWDNBC(I,J) = ACSWDNBC(I,J) + SWDNBC(I,J)*DT
!RT        ENDDO
!RT        ENDDO
!RT   ENDDO
!RT   !$OMP END PARALLEL DO
