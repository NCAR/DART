...
character(len=256) :: grid_file, grid_format,  grid_type, &     ! CMB verified types
                      gridcpl_file, kmt_file, bathymetry_file, &
                      bathymetry_format, grid_atm, grid_ice, grid_ocn
integer :: kcatbound, nblyr, ncat, nfsd, nilyr, nslyr
logical :: close_boundaries, orca_halogrid, use_bathymetry
real(r8) :: dxrect, dyrect
...
! CMB stole code from ice_domain.F90
   ...
   logical (kind=log_kind) :: &
      ...
      add_mpi_barriers   ! added by MW for CICE6, she doesn't know what it does
...
!code from  /glade/u/home/cmip6/cesm_tags/release-cesm2.1.2/cime/src/drivers/mct/shr/seq_timemgr_mod.F90
!formerly from cesm1_5_beta06c/cime/driver_cpl/shr/seq_timemgr_mod.F90
   ...
   namelist /grid_nml/ bathymetry_file, bathymetry_format,&
                       close_boundaries,                  &
                       dxrect, dyrect,                    &
                       grid_atm, grid_ice, grid_ocn,      &
                       grid_file, grid_format, grid_type, &
                       gridcpl_file, kcatbound, kmt_file, &
                       nblyr, ncat, nfsd, nilyr, nslyr,   &
                       orca_halogrid, use_bathymetry

   namelist /domain_nml/ processor_shape,   &
                         distribution_type, &
                         distribution_wght, &
                         ew_boundary_type,  &
                         ns_boundary_type,  &
