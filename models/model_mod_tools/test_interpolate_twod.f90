...
setup_location = set_location(vals(1), vals(2))
...
call write_matlab_header(matunit, nx, ny, ens_size)
...
call write_matlab_data(matunit, nx, ny, field)
call write_matlab_footer_and_close(matunit)

! Write out the netCDF file for easy exploration.
call write_netcdf_output_and_close(ncid, ens_size, nx, ny, &
                                   interp_test_xrange, &
				   interp_test_yrange, &
                                   quantity_string, X, Y field)
...
subroutine write_matlab_header(matunit, nx, ny, ens_size)

integer, intent(in) :: matunit, nx, ny, ens_size
...
subroutine write_matlab_data(matunit, nx, ny, field)

integer, intent(in) :: matunit, nx, ny
...
write(matunit,'(''];'')')
write(matunit,'(''datmat = reshape(interptest,ny,nx,nens);'')')
write(matunit,'(''datmat = permute(datmat,[3,1,2]);'')')
...
subroutine write_netcdf_output_and_close(ncid, ens_size, nx, ny, &
                                         interp_test_xrange, &
                                         interp_test_yrange, &
                                         quantity_string, X, Y, field)

integer,  intent(in) :: ncid
integer,  intent(in) :: ens_size, nx, ny
