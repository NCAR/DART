...
for ii,year in enumerate(years):
    ...
    print('Copying year ' + str(year) + ' to ensemble number '+ zero_filled_ii)
    
    yearpath    = load_path + '/' + zero_filled_year + '-01-01-00000/' 

    # Define file paths
    popold  = yearpath + case_name + '.pop.r.'  + zero_filled_year + '-01-01-00000.nc'
    ovfold  = yearpath + case_name + '.pop.ro.' + zero_filled_year + '-01-01-00000'
    ciceold = yearpath + case_name + '.cice.r.' + zero_filled_year + '-01-01-00000.nc'
    
    popnew  = savepath + case_name + '.pop_'    + zero_filled_ii   + '.r.2010-01-01-00000.nc'
    ovfnew  = savepath + case_name + '.pop_'    + zero_filled_ii   + '.ro.2010-01-01-00000'
    cicenew = savepath + case_name + '.cice_'   + zero_filled_ii   + '.r.2010-01-01-00000.nc'
...
for ii,year in enumerate(years2):
    ...
    # Define file paths
    popold  = glob.glob(yearpath + case_name + '.pop.r.'  + zero_filled_year + '-01-01-00000.nc*')[0]
    ovfold  = glob.glob(yearpath + case_name + '.pop.ro.' + zero_filled_year + '-01-01-00000*')[0]
    ciceold = glob.glob(yearpath + case_name + '.cice.r.' + zero_filled_year + '-01-01-00000.nc*')[0]
    
    popnew  = savepath + case_name_out + '.pop_'  + zero_filled_ii + '.r.2010-01-01-00000.nc'
    ovfnew  = savepath + case_name_out + '.pop_'  + zero_filled_ii + '.ro.2010-01-01-00000'
    cicenew = savepath + case_name_out + '.cice_' + zero_filled_ii + '.r.2010-01-01-00000.nc'
        
    # Copy over, decompressing as necessary

    if popold[-3:]=='.gz':
        print('Decompressing and copying year ' + str(year) + ' to ensemble number '+ zero_filled_ii)
    else:
        print('Copying year ' + str(year) + ' to ensemble number '+ zero_filled_ii)
