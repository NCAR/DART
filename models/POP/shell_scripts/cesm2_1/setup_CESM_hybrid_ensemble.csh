...
# states are specified by data files. The initial states comes from a
...
# before the multi-instance capacity was developed and the naming convention
# decided. Consequently, there is a lot of manipulation of the 'instance'
...
# ==============================================================================
# Set CASE to the combination of the compset, resolution, num_instances  and an
...
setenv CASE                 ${case_string}.${resolution}_e${num_instances}${extra_string}
...
# In an ensemble run, a lot of data is created. The short-term archiver
# runs serially and may require a significant amount of time.
# ==============================================================================
...
# ==============================================================================
# Modifying namelist templates for each instance.
# ==============================================================================

echo ""
echo "Modifying namelist templates for each instance."
echo ""
...
while ($inst <= $num_instances)
   ...
   echo "ice_ic = '${RUNDIR}/${refcase}.cice${inst_string}.r.${reftimestamp}.nc'" >> $fname
...
if ( \${CONTINUE_RUN} == TRUE ) then
   ...
   # These are not needed, so remove the clutter
   ${REMOVE} rpointer.atm_????
   ${REMOVE} rpointer.ice_????
   ${REMOVE} rpointer.ocn_????.tavg
   ${REMOVE} rpointer.drv
   ${REMOVE} rpointer.rof
