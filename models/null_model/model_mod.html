...
a Gaussian, or combinations of the two.  Namelist controls can set the width
of the Gaussian and change both the model advance method and the expected observation
...
<TR><TD>delta_t     </TD>
    <TD>real(r8)    </TD>
    <TD>Internal model timestep parameter.
</TD></TR>

<TR><TD>time_step_days    </TD>
    <TD>integer           </TD>
    <TD>Minimum model advance time in days.
</TD></TR>

<TR><TD>time_step_seconds </TD>
    <TD>integer           </TD>
    <TD>Minimum model advance time in seconds.
</TD></TR>

<TR><TD>noise_amplitude </TD>
    <TD>real(r8)        </TD>
    <TD>If greater than 0.0 sets the standard deviation of the added
Gaussian noise during the model advance.
</TD></TR>

<TR><TD>advance_method </TD>
    <TD>character(64)  </TD>
    <TD>Controls the model advance method.  The default is 'simple' timestepping.
A 4-step Runga Kutta method can be selected with the string 'rk'.
</TD></TR>

<TR><TD>interpolation_method </TD>
    <TD>character(64)  </TD>
    <TD>Controls how the expected value of an observation is computed.  The default
is 'standard' which uses a linear interpolation between the two surrounding model points.
Other options include 'square' which returns the square of the computed value,
'opposite_side' which adds on a value from the opposite side of the cyclical domain, and
'average' which averages 15 points to get the expected value.  Model size should be > 15
...
<TR><TD>&nbsp;</TD><TD><A HREF="#shortest_time_between_assimlations">shortest_time_between_assimlations</A></TD></TR>
...
<P>
 NOTE: NOT UPDATED YET FOR THE MANHATTAN RELEASE. 
 The details of these interfaces will be generated by Doxygen.
 For now, see the code.
</P>
