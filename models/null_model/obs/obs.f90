...
!double precision, parameter :: max_close_distance = 0.10
double precision, parameter :: max_close_distance = 0.00010
!double precision, parameter :: max_close_distance = 0.20
...
double precision :: x_pert(size(x))
integer :: i, j
...
do i = 1, size(x)
   x_pert(i) = x(i) + 0.01 * random_gaussian(ens_seq, dble(0.0), dble(1.0))
end do
...
! May want the constant to always be the same as the obs_var.
      as(i, j) = x_pert(i) + 0.01 * random_gaussian(ens_seq, dble(0.0), dble(1.0))       
   end do
end do
