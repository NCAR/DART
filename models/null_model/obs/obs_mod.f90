...
!real(r8), parameter :: max_close_distance = 0.10_r8
 real(r8), parameter :: max_close_distance = 0.00010_r8
!real(r8), parameter :: max_close_distance = 0.20_r8
...
do i = 1, num_obs
   ...
   if(loc < 0.0_r8 .or. loc > 1.0_r8) then
...
! function obs_var()
!
! Defines the observational error variance. 
! Eventually may need to be generalized to covariance.
...
do i = 1, size(x)
   x_pert(i) = x(i) + 0.01_r8 * random_gaussian(ens_seq, 0.0_r8, 1.0_r8)
end do
...
do i = 1, size(x)
   do j = 1, size(as, 2)
      ...
      as(i, j) = x_pert(i) + 0.01_r8 * random_gaussian(ens_seq, 0.0_r8, 1.0_r8)       
   end do
end do
...
call get_loc(obs_loc(obs_num), x)    ! Get location of this ob
model_size = get_model_size()
