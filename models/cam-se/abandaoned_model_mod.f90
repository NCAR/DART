...
use    cam_common_code_mod, only : scale_height, HIGH_TOP_TABLE, LOW_TOP_TABLE, std_atm_table_len, &
                                   std_atm_hgt_col, std_atm_pres_col, load_low_top_table, load_high_top_table, &
                                   store_std_atm_tables, free_std_atm_tables, high_top_threshold, &
                                   is_surface_field, ref_model_top_pressure, ref_surface_pressure, ref_nlevels, &
                                   init_globals, ref_model_top_pressure, ref_surface_pressure, ref_nlevels, &
                                   cam_1d_array, cam_grid, grid_data, get_close_state, are_damping, ramp_end, &
                                   discarding_high_obs, no_assim_above_height, no_assim_above_level, &
                                   no_assim_above_pressure, vertical_localization_type, above_ramp_start, &
                                   v_above, v_down, v_difference, higher_is_smaller, convert_vert_one_obs, &
                                   convert_vertical_obs, obs_vertical_to_height, obs_vertical_to_level, &
                                   obs_vertical_to_pressure, obs_vertical_to_scaleheight, &
...
!jla integer :: vertical_localization_type = VERTISPRESSURE
...
!jla logical :: higher_is_smaller

!jla! things related to damping at the model top
!jlalogical  :: are_damping = .false.
!jlareal(r8) :: ramp_end         ! fixed top of ramp; the start (bottom) varies
!jlalogical  :: discarding_high_obs = .false.
!jlareal(r8) :: no_assim_above_height    = -1.0_r8 
!jlareal(r8) :: no_assim_above_level     = -1.0_r8 
!jlareal(r8) :: no_assim_above_pressure  = -1.0_r8 
...
! The common module needs to know about no_normalization_of_scale_heights
...
subroutine jlaconvert_vertical_obs(ens_handle, num, locs, loc_qtys, loc_types, &
...
end subroutine jlaconvert_vertical_obs

!--------------------------------------------------------------------

subroutine jlaobs_vertical_to_pressure(ens_handle, location, my_status)
...
end subroutine jlaobs_vertical_to_pressure

!--------------------------------------------------------------------

subroutine jlaobs_vertical_to_height(ens_handle, location, my_status)
type(ensemble_type), intent(in)    :: ens_handle
type(location_type), intent(inout) :: location
integer,             intent(out)   :: my_status
...
end subroutine jlaobs_vertical_to_height

!--------------------------------------------------------------------

subroutine jlaobs_vertical_to_level(ens_handle, location, my_status)
type(ensemble_type), intent(in)    :: ens_handle
type(location_type), intent(inout) :: location
integer,             intent(out)   :: my_status
...
end subroutine jlaobs_vertical_to_level

!--------------------------------------------------------------------

subroutine jlaobs_vertical_to_scaleheight(ens_handle, location, my_status)
type(ensemble_type), intent(in)    :: ens_handle
type(location_type), intent(inout) :: location
integer,             intent(out)   :: my_status
...
end subroutine jlaobs_vertical_to_scaleheight

!--------------------------------------------------------------------

subroutine jlaconvert_vert_one_obs(ens_handle, loc, otype, vert_type, status1)
type(ensemble_type), intent(in)    :: ens_handle
...
end subroutine jlaconvert_vert_one_obs
...
function jlaabove_ramp_start(test_value, gc, obs_type, ramp_end, total_dist, extra_dist)
...
end function jlav_down

!--------------------------------------------------------------------
! returns difference of a and b
! (doesn't depend on the vertical_localization_type)

pure function jlav_difference(a, b)
real(r8), intent(in) :: a, b
real(r8) :: jlav_difference

jlav_difference = abs(a - b)

end function jlav_difference
...
subroutine jla_get_close_state(gc, base_loc, base_type, locs, loc_qtys, loc_indx, &
                           num_close, close_ind, dist, ens_handle)
...
end subroutine jla_get_close_state
