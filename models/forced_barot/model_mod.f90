...
double precision :: lat(lat_max), lon(num_lon)
...
complex :: dt(0:num_fourier, 0:num_spherical, 3)
integer :: i
...
!  compute spectral vort
vsp = compute_laplacian(psisp, 1)
!  compute dT
dt(:, :, 1) = fordt(vsp)
!  do forward time step
call ftstpwv(vsp, vold, dt(:, :, 1), delta_t)
!do n = 0, num_fourier
!   do m = 0, n
do n = 0, min(num_fourier, spherical_lim)
   do m = 0, min(n, fourier_lim)
...
!  quit here if only one step
if(nsteps == 1) return

!  next do second leapfrog step; compute dt
dt(:, :, 2) = fordt(vsp)
...
!  do loop of adams-bashforth 3rd order steps
do i = 3, nsteps
!  compute dt in rotating storage
   index = (i - 1) - (i - 1) / 3 * 3 + 1
   dt(:, :, index) = fordt(vsp)
!  compute the timestep using the rotating dt storage
   call adams3(vsp, vold, dt, index, delta_t)
do n = 0, min(num_fourier, spherical_lim)
   do m = 0, min(n, fourier_lim)
...
! Copy into result
forwrd = compute_laplacian(vsp, -1)

do n = 0, min(num_fourier, spherical_lim)
   do m = 0, min(n, fourier_lim)
      if(m <= fourier_lim .and. n <= spherical_lim) then
!         forwrd(m, n-m) = force(m, n-m)
! Used for no forcing model at any resolution
...
complex, intent(in), dimension(0:num_fourier, 0:num_spherical) :: vsp
complex :: fordt(0:num_fourier, 0:num_spherical)
complex, dimension(0:num_fourier, 0:num_spherical) :: qsp, psisp, current_state
complex, dimension(0:num_fourier, 0:num_spherical) :: diffusion
real :: dtph(num_lon, lat_max), dif2_coef
integer :: i, n, m

!  get spectral psi (inverse laplacian)
psisp = compute_laplacian(vsp, -1)
!  get absolute vorticity
qsp = vsp
qsp(0, 1) = qsp(0, 1) + 2. * omega * sqrt(2./3.)

!  COMPUTE PHYSICAL SPACE DT
dtph = jacob(psisp, qsp)
...
! Add in the timescale normalization
diffusion = diffusion / (86400 * dif_days)
...
! Get rid of the factor of cos2
call divide_by_cos2(jacob)
...
! WARNING: The dynamical systems routines all use double precision,
...
do m = 1, 9
   diag_output_index(m) = (m - 1) * (lat_max / 9) + 1
end do
...
double precision, intent(in) :: x(model_size)
...
! Converts from double precision physical space flat array to physical
! space array
...
double precision :: barot_to_dp(model_size)
real :: phys(num_lon, lat_max)
integer :: i, j, index

! Transform to physical space
call trans_spherical_to_grid(psi, phys)

! Copy into dp array
barot_to_dp = grid_to_dp(phys)
...
real, intent(in) :: x(num_lon, lat_max)
double precision :: grid_to_dp(model_size)
integer :: i, j, index
...
double precision, intent(inout) :: x(model_size)
integer :: m, n
...
! Have real data every day
   real_time = real_time + delta_t * 48
...
! Convert to spectral, advance for 24 steps
psisp = dp_to_barot(x)
...
implicit none
double precision, intent(in) :: x(model_size)
double precision, intent(out) :: x_new(model_size)
integer, intent(in) :: nsteps
integer :: i
