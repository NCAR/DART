function [observation, openloop, forecast, analysis, exp] = HydroDARTdiags(dir_exps, obs, dir_ol, disp_res, plot_state)
...
[~, Nl] = netcdf.inqDim(ncid, 1); % # of links in the domain             
netcdf.close(ncid);
...
for e = 1:num_exps
    ...
    if plot_ol, ol.ensemble = double(ncread(ol.obs_epoc, 'observations')); end
...
openloop    = cell( 3, gauges.want.num          );
forecast    = cell(10, gauges.want.num, num_exps);
...
for i = 1:gauges.want.num
    ...
    for e = 1:num_exps
        ...
        if plot_ol, tmp.ens_mean_ol = ol.ensemble(2, find_obs); tmp.ens_sd_ol = ol.ensemble(4, find_obs); end  
        ...
        for j = 1:Nt
            ...
            elseif ~isnan(ens_time(j+1))
                ...
                if plot_ol, ens_mean_ol(j) = NaN; ens_sd_ol(j) = NaN; end

            end
        end
        clear tmp
        
        flood(i, e) = find(obs_val == nanmax(obs_val), 1);
    ...
    if num_exps > 1
        ...
        for e = 1:num_exps
            ...
            if num_exps == 2
                ...
                strY = [ gauges.want.names(o, :), ' | ID: ', num2str(gauges.want.OID(o)) ];

                set(gca, 'FontSize', 16, 'XLim', [xticks(1), xticks(end)], 'XTick', xticks, 'XTickLabel', xtickslabel, 'Ylim', [0 limsy(2)])
                ylabel(strY, 'FontSize', 18, 'Interpreter', 'none', 'FontWeight', 'bold')

                if plot_ol
                    L = legend([ob_a, ob_r, en_f(1), en_a(1), op, mf , ...
                                ma, so, sf, sa], 'Used Obs'          , ...
                                sprintf('Rejected Obs: %.2f%%'       , 100-observation{4, o, e}/observation{3, o, e}*100), ...
                                'Prior Members', 'Posterior Members' , ...
                                sprintf('Open Loop, RMSE: %.2f'      , nanmean(openloop{2, o})), ...
                                sprintf('Prior Mean, RMSE: %.2f'     , nanmean(forecast{2, o, e})), ...
                                sprintf('Posterior Mean, RMSE: %.2f' , nanmean(analysis{2, o, e})), ...
                                sprintf('Open loop Spread, avg: %.2f', nanmean(openloop{3, o})), ... 
                                sprintf('Prior Spread, avg: %.2f'    , nanmean(forecast{4, o, e})), ...
                                sprintf('Posterior Spread, avg: %.2f', nanmean(analysis{4, o, e})), ...
                                'Location', 'NorthEast');
                else
                ...
                set(L, 'Interpreter', 'none', 'FontSize', 10) 
                title(L, {'Experiment', dir_exps{e}}, 'FontSize', 12)

                str2 = 'Statistics Time-Series (Hydrograph)';
                ...
                title(['Exp: ', dir_exps{e}], 'FontSize', 16, 'FontWeight', 'bold', 'Interpreter', 'none')
        ...
        set(L, 'Interpreter', 'none', 'Box', 'off', 'FontSize', 12)
        ...
        plot(current, ones(1, Nt), '--', 'Color', gY)
        
        limsy = get(gca, 'YLim');
        
        set(gca, 'FontSize', 16, 'XLim', [xticks(1), xticks(end)], 'XTick', xticks, 'XTickLabel', xtickslabel, 'Ylim', [0.8 limsy(2)-0.1])
        ylabel('Inflation', 'FontSize', 18)
        
        L = legend([i_pr_m, i_po_m], ...
                sprintf('Prior Inflation Mean, avg: %.2f'  , nanmean(forecast{9,  o, e})), ...
                sprintf('Posterior Inflation Mean, avg: %.2f'  , nanmean(analysis{8,  o, e})), ...
                'Location', 'NorthEast');
        ...
        if plot_state && o == gauges.want.num
            ...
            title({['Experiment: ' dir_exps{e}],'Stream Flow: Time-Avg. Prior Mean'}, 'FontSize', 14, 'Interpreter', 'none')

            subplot(222)
            plot_connections(Xs1_f, nc(e).routelink, get(gca, 'position'), 'cms', tiny_flow_s)
            title({['Experiment: ' dir_exps{e}],'Stream Flow: Time-Avg. Prior Spread'}, 'FontSize', 14, 'Interpreter', 'none')

            subplot(223)
            if bucket
                Xm2_f = mean(exp(e).pr.state.x2 , 2); 
                plot_connections(Xm2_f, nc(e).routelink, get(gca, 'position'), 'mm', tiny_flow_b)
                title({['Experiment: ' dir_exps{e}],'Bucket: Time-Avg. Prior Mean'}, 'FontSize', 16, 'Interpreter', 'none')
            else
                plot_connections(Xm1_a, nc(e).routelink, get(gca, 'position'), 'cms', tiny_flow_s)
                title({['Experiment: ' dir_exps{e}],'Stream Flow: Time-Avg. Posterior Mean'}, 'FontSize', 16, 'Interpreter', 'none')
            end
            ...
            if bucket
                ...
                title({['Experiment: ' dir_exps{e}],'Bucket: Time-Avg. Prior Spread'}, 'FontSize', 16, 'Interpreter', 'none')
            else
            ...
            title({['Experiment: ' dir_exps{e}],'Stream Flow: DA Increment (Prior-Posterior)', ...
            ...
            if bucket
                ...
                title({['Experiment: ' dir_exps{e}],'Bucket: DA Increment (Prior-Posterior)', ...
                       ['Event: ' longtime(event, :)]}, 'FontSize', 16, 'Interpreter', 'none')
            else
