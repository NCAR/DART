...
! quantity for the other way of measuring elevation
! contrast this with geopotential height
integer, parameter, public :: &
    QTY_GEOMETRIC_HEIGHT            = 59

! quantities for limited satellite radiances (jason o.)
! more generic radiances/brightness temperature info
! can be found at quantity 290
integer, parameter, public :: &
    QTY_INFRARED_RADIANCE           = 60, &
    QTY_INFRARED_BRIGHT_TEMP        = 61, &
    QTY_LANDMASK                    = 62

! quantity for unstructured grids
integer, parameter, public :: &
    QTY_EDGE_NORMAL_SPEED           = 63

! quantity for cloud liquid water path
...
integer, parameter, public :: &
    ...
    QTY_VERTLEVEL                   = 88, &
    QTY_MICROWAVE_BRIGHT_TEMP       = 89    ! this was used for a limited implementation,
                                            ! more general quantities are found at 290
...
!! Reserve kind numbers 151 to 250 for chemistry types.
!! DO NOT USE numbers between 151-250 without talking to the DART group, please?

integer, parameter, public :: &
    QTY_O3                          = 151, &
    QTY_CO                          = 153, &
    QTY_NO                          = 155, &
    QTY_NO2                         = 156, &  ! Nitrogen dioxide
    QTY_N2O                         = 157     ! JLS Nitrous oxide, for RTTOV

! more chemistry kinds (Jerome Barre)
integer, parameter, public :: &
    QTY_CO01                        = 161, &
    QTY_CO02                        = 162, &
    QTY_CO03                        = 163
...
!! Reserving LAND quantities from 301 to 399
...
!! Reserving CICE quantities from 401 to 499
...
! WACCM
integer, parameter, public :: &
  QTY_ION_O_MIXING_RATIO            = 465, &
  QTY_ATOMIC_H_MIXING_RATIO         = 466

integer, parameter, public :: &
  QTY_ABSOLUTE_HUMIDITY             = 467

!! Reserving satellite quantities for radiances from 501 to 599
! Directly-observed (L1) satellite quantities
! and supporting fields for RTTOV
integer, parameter, public :: &
  QTY_RADIANCE                      = 501, &  ! L1 radiance (mW/cm^-1/sr/m^2)
  QTY_BRIGHTNESS_TEMPERATURE        = 502, &  ! L1 brightness temperature (K)
  QTY_BI_DIRECTIONAL_REFLECTANCE    = 503, &  ! L1 bi-directional reflectance (BDRF, unitless)
  QTY_SURFACE_TYPE                  = 504, &  ! land = 0, sea = 1, seaice = 2
  QTY_WIND_FETCH                    = 505, &  ! Wind fetch, m
  QTY_WATER_TYPE                    = 506, &  ! fresh = 0, ocean = 1
  QTY_FOAM_FRAC                     = 507, &  ! Fraction of foam on ocean surface (0-1)
  QTY_INSOLUBLE_AER                 = 508, &  ! Insoluble aerosol OPAC aerosol (INSO)
  QTY_H2O_SOLUBLE_AER               = 509, &  ! Soluble aerosol OPAC aerosol (WASO)
  QTY_SOOT                          = 510, &  ! Soot aerosol OPAC aerosol (SOOT)
  QTY_SEASALT_ACCUM                 = 511, &  ! Sea salt (accumulation mode) OPAC aerosol (SSAM)
  QTY_SEASALT_COARSE                = 512, &  ! Sea salt (coarse) OPAC aerosol (SSCM)
  QTY_MINERAL_NUCLEUS               = 513, &  ! Mineral (nucleus) OPAC aerosol (MINM)
  QTY_MINERAL_ACCUM                 = 514, &  ! Mineral (accumulation mode) OPAC aerosol (MIAM)
  QTY_MINERAL_COARSE                = 515, &  ! Mineral (coarse mode) OPAC aerosol (MICM)
  QTY_MINERAL_TRANSPORTED           = 516, &  ! Mineral (transported mode) OPAC aerosol (MITR)
  QTY_SULPHATED_DROPS               = 517, &  ! Sulphated droplets OPAC aerosol (SUSO)
  QTY_VOLCANIC_ASH                  = 518, &  ! Volcanic ash OPAC aerosol (VOLA)
  QTY_NEW_VOLCANIC_ASH              = 519, &  ! New volcanic ash OPAC aerosol (VAPO)
  QTY_ASIAN_DUST                    = 520, &  ! Asian dust OPAC aerosol (ASDU)
  QTY_BLACK_CARBON                  = 521, &  ! Black carbon CAMS aerosol (BCAR)
  QTY_DUST_BIN1                     = 522, &  ! Dust bin 1 CAMS aerosol (DUS1)
  QTY_DUST_BIN2                     = 523, &  ! Dust bin 2 CAMS aerosol (DUS2)
  QTY_DUST_BIN3                     = 524, &  ! Dust bin 3 CAMS aerosol (DUS3)
  QTY_AMMONIUM_SULPHATE             = 525, &  ! Ammonium sulphate CAMS aerosol (SULP)
  QTY_SEA_SALT_BIN1                 = 526, &  ! Sea salt bin 1 CAMS aerosol (SSA1)
  QTY_SEA_SALT_BIN2                 = 527, &  ! Sea salt bin 2 CAMS aerosol (SSA2)
  QTY_SEA_SALT_BIN3                 = 528, &  ! Sea salt bin 3 CAMS aerosol (SSA3)
  QTY_HYDROPHILIC_ORGANIC_MATTER    = 529, &  ! Hydrophilic organic matter CAMS aerosol (OMAT)
  QTY_CLOUDWATER_DE                 = 530, &  ! Cloud liquid water effective diameter (microns)
  QTY_CLOUD_ICE_DE                  = 531, &  ! Cloud ice effective diameter (microns)
  QTY_COLUMN_CLOUD_FRAC             = 532, &  ! Simple cloud fraction (0-1)
  QTY_CLOUD_TOP_PRESSURE            = 533
...
integer, parameter :: max_defined_quantities = 533
...
obs_kind_names(135) = obs_kind_type(QTY_2M_VAPOR_MIXING_RATIO       , 'QTY_2M_VAPOR_MIXING_RATIO')
...
obs_kind_names(465) = obs_kind_type(QTY_ION_O_MIXING_RATIO          , 'QTY_ION_O_MIXING_RATIO')
obs_kind_names(466) = obs_kind_type(QTY_ATOMIC_H_MIXING_RATIO       , 'QTY_ATOMIC_H_MIXING_RATIO')

obs_kind_names(467) = obs_kind_type(QTY_ABSOLUTE_HUMIDITY           , 'QTY_ABSOLUTE_HUMIDITY')

obs_kind_names(501) = obs_kind_type(QTY_RADIANCE                    , 'QTY_RADIANCE')
obs_kind_names(502) = obs_kind_type(QTY_BRIGHTNESS_TEMPERATURE      , 'QTY_BRIGHTNESS_TEMPERATURE')
obs_kind_names(503) = obs_kind_type(QTY_BI_DIRECTIONAL_REFLECTANCE  , 'QTY_BI_DIRECTIONAL_REFLECTANCE')
obs_kind_names(504) = obs_kind_type(QTY_SURFACE_TYPE                , 'QTY_SURFACE_TYPE')
obs_kind_names(505) = obs_kind_type(QTY_WIND_FETCH                  , 'QTY_WIND_FETCH')
obs_kind_names(506) = obs_kind_type(QTY_WATER_TYPE                  , 'QTY_WATER_TYPE')
obs_kind_names(507) = obs_kind_type(QTY_FOAM_FRAC                   , 'QTY_FOAM_FRAC')
obs_kind_names(508) = obs_kind_type(QTY_INSOLUBLE_AER               , 'QTY_INSOLUBLE_AER')
obs_kind_names(509) = obs_kind_type(QTY_H2O_SOLUBLE_AER             , 'QTY_H2O_SOLUBLE_AER')
obs_kind_names(510) = obs_kind_type(QTY_SOOT                        , 'QTY_SOOT')
obs_kind_names(511) = obs_kind_type(QTY_SEASALT_ACCUM               , 'QTY_SEASALT_ACCUM')
obs_kind_names(512) = obs_kind_type(QTY_SEASALT_COARSE              , 'QTY_SEASALT_COARSE')
obs_kind_names(513) = obs_kind_type(QTY_MINERAL_NUCLEUS             , 'QTY_MINERAL_NUCLEUS')
obs_kind_names(514) = obs_kind_type(QTY_MINERAL_ACCUM               , 'QTY_MINERAL_ACCUM')
obs_kind_names(515) = obs_kind_type(QTY_MINERAL_COARSE              , 'QTY_MINERAL_COARSE')
obs_kind_names(516) = obs_kind_type(QTY_MINERAL_TRANSPORTED         , 'QTY_MINERAL_TRANSPORTED')
obs_kind_names(517) = obs_kind_type(QTY_SULPHATED_DROPS             , 'QTY_SULPHATED_DROPS')
obs_kind_names(518) = obs_kind_type(QTY_VOLCANIC_ASH                , 'QTY_VOLCANIC_ASH')
obs_kind_names(519) = obs_kind_type(QTY_NEW_VOLCANIC_ASH            , 'QTY_NEW_VOLCANIC_ASH')
obs_kind_names(520) = obs_kind_type(QTY_ASIAN_DUST                  , 'QTY_ASIAN_DUST')
obs_kind_names(521) = obs_kind_type(QTY_BLACK_CARBON                , 'QTY_BLACK_CARBON')
obs_kind_names(522) = obs_kind_type(QTY_DUST_BIN1                   , 'QTY_DUST_BIN1')
obs_kind_names(523) = obs_kind_type(QTY_DUST_BIN2                   , 'QTY_DUST_BIN2')
obs_kind_names(524) = obs_kind_type(QTY_DUST_BIN3                   , 'QTY_DUST_BIN3')
obs_kind_names(525) = obs_kind_type(QTY_AMMONIUM_SULPHATE           , 'QTY_AMMONIUM_SULPHATE')
obs_kind_names(526) = obs_kind_type(QTY_SEA_SALT_BIN1               , 'QTY_SEA_SALT_BIN1')
obs_kind_names(527) = obs_kind_type(QTY_SEA_SALT_BIN2               , 'QTY_SEA_SALT_BIN2')
obs_kind_names(528) = obs_kind_type(QTY_SEA_SALT_BIN3               , 'QTY_SEA_SALT_BIN3')
obs_kind_names(529) = obs_kind_type(QTY_HYDROPHILIC_ORGANIC_MATTER  , 'QTY_HYDROPHILIC_ORGANIC_MATTER')
obs_kind_names(530) = obs_kind_type(QTY_CLOUDWATER_DE               , 'QTY_CLOUDWATER_DE')
obs_kind_names(531) = obs_kind_type(QTY_CLOUD_ICE_DE                , 'QTY_CLOUD_ICE_DE')
obs_kind_names(532) = obs_kind_type(QTY_COLUMN_CLOUD_FRAC           , 'QTY_COLUMN_CLOUD_FRAC')
obs_kind_names(533) = obs_kind_type(QTY_CLOUD_TOP_PRESSURE          , 'QTY_CLOUD_TOP_PRESSURE')
