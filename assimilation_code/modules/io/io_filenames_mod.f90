...
if ( nf90_get_att(ncFile, ncVarID, att_string, ret_spvalINT) == NF90_NOERR ) then
   if (ret_spvalINT /= ret_spvalINT) then ! ret_spval is NaN
      if (.not. (spvalINT /= spvalINT)) then ! spval is not NaN, so they are not the same
         ...
         call error_handler(E_ERR, 'check_attribute_value_INT', msgstring, source)
      end if
      return
   endif
