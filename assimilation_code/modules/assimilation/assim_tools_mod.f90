...
! Testing bounded rhf declarations
...
if ((obs_var == 0.0_r8) .and. (prior_var == 0.0_r8)) then
   ...
   else if(filter_kind == 888) then
      ! Initial test is for L96, has approximate climatological bounds
      is_bounded(1) = .true.
      bound(1) = -12
      is_bounded(2) = .true.
      bound(2) = 14
      call obs_increment_bounded_norm_rhf(ens, ens_size, prior_var, obs, obs_var, obs_inc, &
         is_bounded, bound)
   else
...
subroutine obs_increment_bounded_norm_rhf(ens, ens_size, prior_var, obs, obs_var, obs_inc, &
   is_bounded, bound)
!------------------------------------------------------------------------
integer,  intent(in)  :: ens_size
real(r8), intent(in)  :: ens(ens_size), prior_var, obs, obs_var
...
real(r8) :: s_ens(ens_size), ens_like(ens_size), post(ens_size)
...
! Compute likelihood for sorted ensemble members: assumes truncated normal obs error
do i = 1, ens_size
   ens_like(i) = get_truncated_normal_like(s_ens(i), obs, obs_var, is_bounded, bound)
end do

! Compute the mean likelihood in each interior interval (bin)
do i = 1, ens_size - 1
   region_likelihood(i) = (ens_like(i) + ens_like(i + 1)) / 2.0_r8
end do

! Likelihoods for outermost regions (bounded or unbounded); just outermost ensemble like
region_likelihood(0) = ens_like(1)
region_likelihood(ens_size) = ens_like(ens_size)
