...
!> basic metadata for most observation types. Radiance observations, by
!> necessity, have their metadata read in order to create temporary observation
!> types that differentiate the channels and platforms, etc.
!> Unique metadata to all other observation types is not converted.
