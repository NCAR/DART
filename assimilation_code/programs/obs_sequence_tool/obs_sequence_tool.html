...
<TR><TD>remove_precomputed_FO_values</TD>
    <TD>character(len=32), dimension(500)</TD>
    <TD>The (case-insensitive) array of observation type names whose 
...
persists, subject to the constraints of <em class=code>keep_types</em>
...
<li><a href="#precomputed">Precomputed Forward Observation Values</a>
</ul>
...
<A NAME="precomputed"></A>
<h3>Precomputed Forward Observation Values</h3>
<P>
Precomputed Forward Observation Values are the result of an external program
...
the <em class=code>GSI2DART</em> observation converter).
By default, any observation with precomputed forward observation (FO) values 
will have those values simply pass through <em class=code>obs_sequence_tool</em>
just like any other piece of metadata.  If the precomputed forward observation 
...
<strong>Important</strong>: observations resulting from 
<em class=code>perfect_model_obs</em> are <strong>not</strong> 
