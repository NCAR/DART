...
<a name="TOP" id="TOP"></a>
<h1>Jamaica</h1>
<h2>DART Jamaica release documentation</h2>
<div class="attention">
<div class="title">
<p>Attention</p>
</div>
<p>Jamaica is a prior release of DART. Its source code is available via the
<a href="https://github.com/NCAR/DART/tree/Jamaica">DART repository on Github</a>.
...
<strong>real</strong> (as well as synthetic) observations to allow
increased understanding of all three. The DART programs have been
compiled with several (many?) Fortran 90 compilers and run on linux
compute-servers, linux clusters, OSX laptops/desktops, SGI Altix
clusters, supercomputers running AIX ... a pretty broad range,
really. You should definitely read the <a href=
"#customizations">Customizations</a> section.</p>
...
<p>The Jamaica release has been tested on several supercomputers,
including the NCAR IBM machines 'bluevista' and 'blueice', the SGI
Altix machines 'columbia' and 'origin'. There are
<strong>many</strong> more changes - much more thoroughly described in the
Jamaica_diffs_from_I document. What follows here is only meant to draw
attention to the fact that experienced users should really read the change
document.</p>

<p>The most visible changes in the Jamaica release are as follows.</p>

<h3 class="indent1">Changes in the inflation strategies.</h3>
<p class="indent1">The changes in the inflation mechanism now
provide the ability to perform inflation in observation-space,
fixed-state-space, or spatially-varying-state-space in the prior
and/or posterior with either a deterministic or stochastic
algorithm.</p>

<h3 class="indent1">Fundamental changes for parallel behavior</h3>
<p class="indent1">The <strong>optional</strong> use of MPI
required a complete rewrite of many of the routines that previously
governed attempts at parallel computations. This resulted in the
removal of the <em class="code">async = 3</em> method of achieving
parallel assimilations and model advances. As a consequence,
<em class="program">assim_region.csh</em>, <em class=
"program">advance_ens.csh</em>, and <em class=
"program">filter_server.csh</em> no longer exist.</p>

<p>Some assimilation strategies previously controlled by <em class=
"program">assim_tools_nml</em>, (namely, whether to perform a
parallel assimilation, how many domains to use, and a few others)
have been replaced by a new parallel assimilation algorithm.
Associated inflation computations are documented in the manuscript
by Anderson and Collins that is to appear in the <cite>Journal of
Atmospheric and Oceanic Technology</cite> and in section 22 of the
tutorial. Namelist entries <em class="code">do_parallel,
num_domains</em>, and <em class="code">parallel_command</em> no
longer exist.</p>

<p>The (optional) MPI implementation allows for very large state
vectors - some model configurations (i.e. state vectors too large
to fit in core) are now possible. And <strong>yes</strong>, it
scales rather nicely, thank you. There are references throughout
the documents and code to MPI - keep in mind all the MPI interfaces
are in one module <em class="file">mpi_utilities_mod.f90</em>. If
you are NOT using MPI - there is a simple equivalent module named
<em class="file">null_mpi_utilities_mod.f90</em> which is the
default.</p>

<p>Using MPI greatly increases the performance for large models and/or
large numbers of observations. It also greatly
<strong>decreases</strong> the shell script complexity. The use of
MPI is enabled by giving <em class="program">mkmf_filter</em> an
optional argument <em class="unix">-mpi</em>.</p>

<h3 class="indent1">Vertical localization</h3>

<p class="indent1">It is now possible to perform localization in
the vertical in 3D models, which necessitated a change in the
<em class="code">model_mod</em> interface. The <em class=
"program">model_mod:get_close_states()</em> and <em class=
"program">model_mod:get_num_close_states()</em> routines have been
replaced by "pass-through" interfaces to routines in <em class=
"program">location_mod</em>. An optional interface <em class=
"program">model_mod:ens_mean_for_model()</em> is required only by
some models when performing localization in the vertical. This
routine provides the ensemble mean so that the same model state is
used when computing the vertical localization.</p>
<h3 class="indent1">3D plots of observation locations &amp;
type</h3>
<p class="indent1">It's more than just eye candy. It is also
possible to plot (in 3D) the locations of observations by
processing an observation sequence through <em class=
"program">obs_diag</em> and running the resulting output file
through the matlab function <em class=
"program">plot_observation_locations</em>.</p>
<!--==================================================================-->
...
<p>DART has been tested on many Fortran compilers and platforms.
Our philosophy seems to be to burn away the 'impurities' by
compiling on any architecture we can get our hands on. We
completely buy into the use of the Fortran 'kind' strategy to
declare variable types rather than relying on compiler
autopromotion flags - which are a BAD idea in my opinion. There are
some models in DART that come from outside developers and _may_
contain code segments that require the use of some special compiler
flags - we are not so draconian as to mandate the use of the
Fortran 'kind'. In general, the low-order models and all common
portions of the DART code compile very cleanly.</p>
...
development environment for DART is highly skewed to these
machines. We do most of our development on a small linux
workstation and a mac laptop running OSX 10.4.9 - but have an
extensive test network. I've never built nor run DART on a Windows
machine - so I don't even know if it's possible. If you have run it
(under Cygwin?) please let me know how it went -- I'm curious.</p>

<h2 class="indent1">What's nice to have</h2>
<ul>
<li><strong>ncview</strong>: DART users have used
<a href=
...
"http://www.mathworks.com/">Matlab®</a> scripts, designed to
produce graphical diagnostics from DART netCDF output files are
also part of the DART project.</li>
<li><strong>MPI</strong>: The latest release of
the DART system includes an MPI option. MPI stands for 'Message
Passing Interface', and is both a library and run-time system that
enables multiple copies of a single program to run in parallel,
exchange data, and combine to solve a problem more quickly. The
latest release of DART does *NOT* require MPI to run; the default
build scripts do not need nor use MPI in any way. However, for
larger models with large state vectors and large numbers of
...
multiple ensembles of your model fit comfortably (in time and
memory space) on a single processor, you need read no further about
MPI.</li>
</ul>
...
Compiler</a>, <a href="http://gcc.gnu.org/fortran">GNU Fortran 95
Compiler ("gfortran")</a>, <a href="http://www.absoft.com">Absoft
Fortran 90/95 Compiler (Mac OSX)</a>. Since recompiling the code is
a necessity to experiment with different models, there are no
binaries to distribute.</p>
<!--==================================================================-->
...
<p>This release of the <a href="https://github.com/NCAR/DART/releases/tag/v6.0.0">
...
named <em class="file">DART</em> and will be approximately 221.7 Mb.
...
<hr>
<h3>Customizing the build scripts -- Overview.</h3>
...
<h2 class="indent1">Building and Customizing the 'mkmf.template'
file</h2>
...
<h3 class="indent2">NETCDF</h3>
<p class="indent1">Modifying the <em class="code">NETCDF</em> value
should be relatively straightforward. Change the string to reflect the location
of your netCDF installation containing <em class="file">netcdf.mod</em> and
...
data assimilation. If you have correctly configured your <em class=
"file">mkmf.template</em>, you can run anything in the
tutorial.</p>
<!--==================================================================-->
...
<p>There are eight <em class="file">mkmf_</em><em class=
...
<li><em class="program">merge_obs_seq</em>, and</li>
...
to compile anything -- <em class="program">make</em> does that:</p>
...
vice-versa. Since this is a specialty routine - we will not cover
its use in this document.</td>
</tr>
<tr>
<td valign="top"><em class="program">wakeup_filter</em></td>
...
<h2 class="indent1">1. Integrate the L63 model for a 'long'
time.</h2>
...
<h3 class="indent1">1.1 Create an observation set definition.</h3>
...
<h3 class="indent1">1.2 Create an observation sequence
definition.</h3>
...
<h3>1.3 Initialize the model onto the attractor.</h3>
...
<h3 class="indent1">Generating the ensemble</h3>
