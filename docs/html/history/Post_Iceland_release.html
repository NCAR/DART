...
<body>
<a name="TOP" id="TOP"></a>
<h1>DART "Iceland release" Documentation</h1>
...
<!--==================================================================-->
<h1>DART "post-Iceland release" Documentation</h1>
<!--==================================================================-->
...
section.<br>
<br>
DART employs a modular programming approach to apply an Ensemble
...
described in detail in the tutorial directory of the package.<br>
<br>
DART ultimately creates a few netCDF files containing the model
...
<p><a href="/DAReS/DART/DART_download">The latest software release
-- "post_iceland".</a></p>
<p>The <b>post_iceland</b> release provides several new models and
has a greatly expanded capability for <b>real</b> observations
which required a fundamentally different implementation of the
low-level routines. It is now required to run a preprocessor on
several of the program units to construct the source code files
which will be compiled by the remaining units. Due to the
potentially large number of observations types possible and for
portability reasons, the preprocessor is actually a F90 program
that uses the namelist mechanism for specifying the observation
types to be included. This also prevents having a gory set of
compile flags that is different for every compiler. One very clever
colleague also 'built a better mousetrap' and figured out how to
effectively and robustly read namelists, detect errors, and
generate meaningful error messages. HURRAY!</p>
<p>The post_iceland release has also been tested with more
compilers in an attempt to determine non-portable code elements. It
is my experience that the largest impediment to portable code is
the reliance on the compiler to autopromote <tt>real</tt> variables
to one flavor or another. <a href=
...
approximately 189 Mb. Compiling the code in this tree (as is
usually the case) will necessitate much more space.</p>
<div class="unix">gunzip <em class=
"file">DART_post_iceland.tar.gz</em><br>
tar -xvf <em class="file">DART_post_iceland.tar</em><br></div>
...
<div class="unix">\rm -f ../../../obs_def/obs_def_mod.f90<br>
\rm -f ../../../obs_kind/obs_kind_mod.f90<br>
./preprocess<br>
ls -l ../../../obs_def/obs_def_mod.f90<br>
ls -l ../../../obs_kind/obs_kind_mod.f90<br></div>
...
worry about the highlighted stuff)</p>
...
"changed">'RAW_STATE_VARIABLE' /</em><br>
<br>
&amp;assim_model_nml<br>
