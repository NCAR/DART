...
<title>DART "Guam release" Documentation</title>
...
<a name="TOP" id="TOP"></a>
<h1>Guam</h1>
<h2>DART Guam release documentation</h2>
<div class="attention">
<div class="title">
<p>Attention</p>
</div>
<p>Guam is a prior release of DART. Its source code is available via the
<a href="https://github.com/NCAR/DART/tree/Guam">DART repository on Github</a>.
...
<p>The Guam release provides several new models and has a
fundamentally different implementation of the low-level routines
that will now operate on a much broader class of observations. As
such, the first two program units have changed somewhat. The
strategy of these two programs units is still the same; declare the
characteristics of the observations and then declare the temporal
nature of the observations. These are still destined to be run as a
back-end to some sort of GUI-driven interface, so virtually no work
has been done to change the level of user interaction required to
run these programs.</p>
<p>The Guam release has also been tested with more compilers in an
attempt to determine non-portable code elements. It is my
...
autopromotion in different ways</a>. Using the F90 "kind" allows for
much more flexible code, in that the use of interface procedures is
possible only when two routines do not have identical sets of input
arguments -- something that happens when the compiler autopromotes
32bit reals to 64bit reals, for example.</p>
...
system, so please email me (Tim Hoar) thoar 'at' ucar 'dot' edu
(trying to cut down on the spam).</p>
...
Linux/x86 platforms with the <a href=
"http://www.intel.com/software/products/compilers/flin">Intel
Fortran Compiler 7.1 for Linux</a>, which is free for individual
scientific use. It has also been built and successfully run with
the <a href="http://www.pgroup.com">Portland Group Fortran
Compiler</a> (5.02), and again with the Intel 8.0.034 compiler.
Since recompiling the code is a necessity to experiment with
...
interface to the library which are available through the <em class=
...
<p>DART also uses the <strong>very</strong> common <a href=
...
as described in the <a href="#compiling">compiling</a> section.</p>
<!--==================================================================-->
...
<p>This release of the <a href="https://github.com/NCAR/DART/releases/tag/v3.0.0">
...
named <em class="file">DART</em> and will be approximately 28.6 Mb.
...
example namelist is called <em class="file">input.nml.mkmf</em>, so
as not to clash with any exising <em class="file">input.nml</em>
that may exist in that directory.</p>
...
"file">mkmf.template.pgi</em>) is needed: (note that only the first
few lines are shown here).</p>
...
"code">FFLAGS</em> and <em class="code">LIBS</em>, which indicate
where to look for the netCDF F90 modules and the location of the
netCDF and udunits libraries.</p>
...
location of the netCDF modules <em class="file">netcdf.mod</em> and
<em class="file">typesizes.mod</em>. Change the <em class=
"code">/usr/local/netcdf/include</em> string to reflect the
location of your modules. The low-order models can be compiled
without the <em class="code">-r8</em> switch, but the <em class=
"file">bgrid_solo</em> model cannot.</p>

<h4 class="indent2">LIBS</h4>
<p class="indent1">Modifying the <em class="code">LIBS</em> value
...
Change the <em class="code">/usr/local/udunits-1.11.7/lib</em>
string to reflect the location of your <em class=
"file">libudunits.a</em>.</p>
...
"program">create_obs_fixed_network_seq</em>, <em class=
...
Fortran90 modules used by this program.</p>
...
example).<br>
<br>
The only function of the <em class="program">mkmf_</em><em class=
"italic">xxxxxx</em> script is to generate a <em class=
"file">Makefile</em> and an <em class="file">input.nml.mkmf</em>
file. It is not supposed to compile anything:</p>
<pre>
<code>
...
"file">input.nml.create_obs_sequence_default</em> file. The second
command results in the compilation of a series of Fortran90 modules
which ultimately produces an executable file: <em class=
"program">create_obs_sequence</em>. Should you need to make any
changes to the <em class="file">DART/mkmf/mkmf.template</em>, you
will need to regenerate the <em class="file">Makefile</em>. A
series of object files for each module compiled will also be left
...
the other three programs needed to work with L63 in DART as
follows:</p>
...
<p>The result (hopefully) is that four executables now reside in
...
observations</a><br>
by running the filter; diagnostic output is generated.<br>
<br></li>
</ol>
<a name="integrate" id="integrate"></a>
<h3 class="indent1">1. Integrate the L63 model for a 'long'
time.</h3>
<p><em class="program">perfect_model_obs</em> integrates the model
for all the times specified in the 'observation sequence
definition' file. To this end, begin by creating an 'observation
sequence definition' file that spans a long time. Creating an
'observation sequence definition' file is a two-step procedure
involving <em class="program">create_obs_sequence</em> followed by
<em class="program">create_fixed_network_seq</em>. After they are
both run, it is necessary to integrate the model with <em class=
"program">perfect_model_obs</em>.</p>
...
being 3600 seconds. By declaring we have 1000 observations taken
once per day, we create an observation sequence definition spanning
24000 'model' timesteps; sufficient to spin-up the model onto the
attractor. Finally, enter a name for the 'observation sequence
definition' file. Note again: there are no observation values
present in this file. Just an observation type, location, time and
the error characteristics. We are going to populate the observation
sequence with the <em class="program">perfect_model_obs</em>
program.</p>
<div class="unix">
<pre>
...
namelist was created <em class="file">input.nml.mkmf</em> that
contains all of the namelist input for the executable. If you
followed the example, each namelist was saved to a unique name. We
must now rename and edit the namelist file for <em class=
"program">perfect_model_obs</em>. Copy <em class=
"file">input.nml.perfect_model_obs</em> to <em class=
...
advance the model state from the end of the first 24,000 steps to
the end of an additional 24,000 steps and place the final state in
<em class="file">perfect_restart</em>. The rest of the namelists in
...
<p class="indent1">is done with the program <em class=
"program">filter</em>, which also uses the Fortran90 namelist
mechanism for input. It is now necessary to copy the <em class=
"file">input.nml.filter</em> namelist to <em class=
"file">input.nml</em> or you may simply insert the <em class=
"code">filter_nml</em> namelist into the existing <em class=
...
"file">input.nml</em> that has all the namelist blocks in it.</p>
...
<tr>
<td><em class="code">num_output_ens_members</em></td>
...
the <em class="file">DART/matlab</em> directory. They are reliant
on the public-domain <a href=
...
necessary to prepend the location of the DART/matlab scripts to the
matlabpath. Keep in mind the location of the netcdf operators on
your system WILL be different from ours ... and that's OK.</p>
<div class="unix">
<pre>
...
model)</h2>
<p>One of the common problems with ensemble filters is filter
...
weight to observations then they should be given. The result can be
...
the matlab script <em class="program">plot_bins</em>.<br>
<br>
