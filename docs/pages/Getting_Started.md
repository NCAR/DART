...
support efficient operational DA applications. DART is a software
...
* [More detailed DART setup instructions](#DetailedSetup)
    ...
    *  [Download the DART software into the expected source tree](#installing).
...
assumes that you are a sophisticated user who understands all of the necessary
issues without further introduction. If this level of detail is too sparse for
you, please refer to [More detailed DART setup instructions](#DetailedSetup).
...
"Download ZIP" to download the latest release of DART. Unzip this file in your
desired location. *IMPORTANT:* this ZIP file will contain a read-only copy of
DART. If you plan to contribute to DART, you will need to fork the DART
repository to eventually contribute your changes. See
[Contributing to DART](#ContributeToDart) for more information.

Alternatively, if you are interested in running *bgrid_solo*, *cam-fv*, or
testing the *NCEP/prep_bufr* observation converter, you will need to download a
different archive file. There are several large files that are needed to run
...
Download the appropriate tar file and untar it into your desired location.
Ignore any warnings about
```
tar: Ignoring unknown extended header keyword
```
As above, this directory will be a read-only copy of DART.

For either ZIP or tar file, take note of the directory you installed into
(referred to as *DARTHOME* below), and go into the `build_templates` subdirectory and copy over the closest `mkmf.template._compiler.system_` file into `mkmf.template`.
...
*IMPORTANT:* this *netCDF* library must have been compiled with the same
compiler (including version) that you will use to compile DART and also must
include the F90 interfaces. In practice this means that even if you have a
...
*NOTE:* The 'mpif90' command you use must have been built with the same
version of the compiler and *netCDF* you are using.
...
4.  [Download the DART software into the expected source tree](#installing).
...
*DISCLAIMER:* we have tried to make the DART code as portable as possible,
but we do not have access to all compilers on all platforms, so unfortunately
we cannot guarantee that the code will work correctly on your particular system.
We are genuinely interested in your experience building the system, so we
welcome you to send us an email with your experiences at dart @ ucar .edu,
which we will endeavor to incorporate into future versions of this guide.
...
and `typesizes.mod` modules. *IMPORTANT*: different compilers sometimes create
these modules with different filenames (filenames on Unix are case-sensitive),
and sometimes they are not **both** installed into the expected directory. It
is required that both modules be present. The normal place would be in the
...
instructions for building the library or installing from an RPM  may be found
...
Go to https://github.com/NCAR/DART, click "Clone or download", and click
"Download ZIP" to download the latest release of DART. Unzip this file in your
desired directory, which we refer to as DARTHOME in this document. *IMPORTANT:*
This ZIP file will contain a read-only copy of DART. If you plan to contribute
to DART, you will need to fork the DART repository to eventually contribute
your changes. See [Contributing to DART](#ContributeToDart) for more information.
...
will require a large amount of additional disk space (up to the 1 Gb required
for DART), so be aware of any disk quota restrictions before continuing.
...
*Makefile*) and an example namelist `input.nml._**program**_default` with the
...
| LIBS     | the libraries passed to *FC* (or *MPIFC*) during compilation. Note you may need to change this if the netCDF library `libnetcdf` is not in the standard location under the `lib` subdirectory of *NETCDF*. |
...
customization. However, modifying this file will be required if you wish to add
your model to DART. See [How do I run DART with my model?](#RunWithMyModel) for
more information.
...
simple ODE model with only three variables. Of course DART can support models
with many orders of magnitude more variables than three, but if you can compile
and run ONE of the low-order models, you should be able to compile and run ANY
of the low-order models. The higher-order models take require more specialized
setup to go along with the increased model complexity, but the complexity is
typically introduced by the model itself rather than DART. For this reason,
here we focus solely on the Lorenz 63 model here. If so desired, see
[The Lorenz 63 model: what is it and why should we care?](#Lorenz63) for more
information on this simple yet surprisingly relevant model.
...
build the executables. *NOTE:* some higher-order models have many more than
seven programs; for example, the Weather Research and Forecasting (WRF) model,
which is run operationally around the world to predict regional weather, has 28
...
directory. *The most common problem* is that the netCDF libraries and include
files (particularly `typesizes.mod`) are not found. Find them, edit the
`DARTHOME/build_templates/mkmf.template` to point to their location, recreate the `Makefile`, and try again.
...
more about what this "physically" means for this low-order model.
...
*REQUEST:* In the case that the above instructions had one or more issues that
either did not work for you as intended or were confusing, please contact the
DART software development team at dart @ ucar .edu . We value your input to
make getting started as smooth as possible for new DART users!
...
have a spread due to instrument noise, etc. Mapping each of the ensemble
members to the observations with a function *h* and performing **Bayes' rule**
will give an update to the posterior distribution, called here the *state
increment*. Adding the state increment to the ensemble members will give the
new *analysis* at time *t*<sub>*k*+1</sub>. This process can then be repeated
for each set of observations as many times as necessary.
...
usually many more ensemble members, typically in the range of 20-1000
depending on the application. Each member **x**<sub>i</sub> can have
...
observations are assumed to be functions of the model state and provide "real
world" checks against the model forecast. A forward operator *h*
maps between **x**<sub>i</sub> and **y**<sub>j</sub>, giving an "expected
observation" of the *j*<sup>th</sup> observation for the *i*<sup>th</sup>
...
example radar returns) may be a model that is quite complicated.

In practice, observations are not always 100% reliable. The observations
...
ensemble filter algorithm (the most typically used algorithm is a
**Square-Root Ensemble Kalman Filter**; see
...
estimated correlations may become unreliable at distant locations due to
sampling error. Thus, various tricks such as **covariance localization** may be
employed to improve the quality of estimated relationships and increase skill
in prediction. Furthermore, the ensemble spread may sometimes be deemed "too
small" or "too large" by various criteria, in which case a multiplicative or
additive **inflation** or **deflation**, respectively, may be applied. Despite
these caveats, in practice the ensemble method is usually far more accurate and
less error-prone than the main alternative of manually specifying uncertainty
by some model, and it is certainly less labor-intensive to develop.
...
By default, the `model_mod.f90` follows the Lorenz 63 paper to use the Runge-
Kutta 2 scheme (otherwise known as the midpoint scheme) to advance the model.
...
invariant), so RK2 can be written as follows, again following the Lorenz 63
...
This time-step was equal to 3600 seconds, or one hour, in dimensional time.
This is also set in the namelist above. The Lorenz 63 model observation file
included with the DART repository uses observations of all three state
...
So as an exercise, let's make a nicer-looking plot using the computational power
available today, which even on the most humble of computers is many times
greater than what Lorenz had in 1963. Let's first change the non-dimensional
timestep to 0.001, which will correspond to a dimensional timestep of 360
seconds (6 minutes). Change the `input.nml` file in
...
Note the changes to `deltat` and `time_step_seconds`. Also note that we do not
need to recompile as the purpose of namelists file is to pass run-time
parameters to a Fortran program without recompilation.

#### Updating the observation sequence (single copy)

Let's now generate the DART observation files with the updated timestep. In a
typical large-scale application, the user will provide observations to DART in
a standardized format called the *Observation Sequence* file (see
[How do I run DART with my model?](#RunWithMyModel) for more information).
However, for the low-order models such as Lorenz 63, included with DART are
helpful interactive programs to generate these observation sequence files for
...
For this file, we will not have any additional copies of the data, nor will
we have any quality control fields. So use **0** for both.
...
exercise is to refine the time step and number of observations by a factor of  10.
...
*NOTE:* as we specified in `set_def.out`, there are 3 observations per time
step, so a total of 6000 observations will be generated.

Also *NOTE*: the Lorenz 63 model dimensional time-step is related to the
observational time *only* through this mechanism. In other words, `deltat` in
the namelist could relate to virtually any dimensional time step if the
...
the observations. Once noise is added, the output is written to `obs_seq.out`.
...
`filter` for Lorenz 63 has three stages: the *preassim* stage, where the
ensemble is generated and the file `preassim.nc`, which contains the pre-
assimilation model trajectories for all the ensemble members, is written; the
*analysis* stage, where the data assimilation is conducted. The post-
assimilation model trajectories for all the ensemble members will be written to
`analysis.nc`; and the *output* stage, which writes the file `obs_seq.final`
containing the actual observations as assimilated plus the ensemble forward-
operator expected values and any quality-control values.
...
favorite language could write a fully functional DA system in a manner of weeks
...
devastatingly complex it will become. Writing a parallel DA system that can
efficiently utilize multiple cores with MPI is not straight-forward, and adding
...
of core DA code, not to mention the head-aches involved in supporting multiple
...
has already gone through this multi-year project, and the code is available for
you to use out-of-the-box. For more information on how DART was written
(and continues to be developed), see [DART's design philosophy](#dartDesign).
...
applicable) and mature and robust DA can automatically be utilized. A  
...
[How do I run DART with my model?](#RunWithMyModel) for more information.
5. DART **makes it easy to test the new observations** in order to test the
...
engineers, data assimilation mathematicians, and data assimilation scientists
all work together to continually improve DART and support user needs. See the
[About us](About_Us.md) for more information about the DART team.
...
It is possible to run DART in **Observation System Simulation Experiment (OSSE)**
mode. In OSSE mode, a perfect "true" model trajectory is created, and
...
It is also possible to run a more realistic **Observation System Experiment
(OSE)** experiment in DART. In an OSE, there is no perfect model truth, which
...
member may take up 100s of Mb or more of disk space, and it is therefore
inefficient to combine all of the very large NetCDF files together. Instead,
each ensemble member is stored in a **separate file**. The initialization,
prior, and posterior ensemble members are each stored in their own file by
default. *NOTE:* the prior would be the same as the initialization if not for
using inflation. It is also possible to store the posterior files before
and after inflation.
...
just before applying DA; `analysis.nc`, which contains the posterior state for
all the ensemble members just after applying DA; `filter_output.nc`, which
is the final posterior that could be used to restart the OSSE process; and
`obs_seq.final`, which adds the forward-calculated expected values *h*(**x**)
for each observation. The `obs_seq.final` file can be analyzed and binned by
the `obs_diag` program, producing the file `obs_diag_output.nc` which can be
used as input to the Matlab DART diagnostic files.
...
`obs_seq.out` file is ultimately the user's responsibility. Here, the option to
...
*IMPORTANT*: the decision to store ensemble members as separate files and
whether to run an OSSE or OSE are independent. An OSSE can be run with multiple
files and an OSE can be run with combined ensemble members.
...
3. Many users of DART are not software development professionals and appreciate simple code.
...
DART offers numerous **filter algorithms** that can be used. These determine
...
DART also offers the ability to correct for sampling errors. This behavior
can be turned on or off via the *sampling_error_correction* in `input.nml` under the "assim_tools_nml" section. The following
covariance localization options are available (set by *select_localization* in
`input.nml` under the "cov_cutoff_nml" section):

| Loc #    | Localization type                 | References  |
| :------- | :----------                       | :---------- |
|        1 | Gaspari-Cohn eq. 4.10             | **Gaspari, G. and Cohn, S. E.** (1999). Construction of correlation functions in two and three dimensions. *Q.J.R. Meteorol. Soc.*, 125: 723-757. https://doi.org/10.1002/qj.49712555417 |
|        2 | Box-car                           | None |
...
DART's localization and sampling error correction algorithm is described in
...
behaviors if appropriate. For more complex models, additional steps outside of
Fortran may be required, especially if the model is run on a supercomputing
queueing system. Refer to your model's documentation for more information if
necessary.
...
| 4. **static_init_model()**  | *Initialize* the model. The procedure for doing this will depend on how complex the model is; see below for suggestions for implementation. | `default_model_mod` / `models/utilities` | Does nothing. |
...
as a reference. Here we provide some further advice on how to add this kind of model to
DART.

The primary consideration with a simple model is how you will store the state.
If you have only a *single type of variable* in your state vector (in other
words, all variables can be compared directly as part of the same field), here
are some hints on how to implement your initialization and meta data routines:
...
`path_names_model_mod_check` to your `DARTHOME/models` subdirectory. You might
find it helpful to consult another model matching your model type (simple or
...
generic *observation quantities*. The role of the various obs_def input files 
...
second column is the generic observation quantity and must match the list of 
known quantities in `obs_kind_mod.f90`. (To add a new quantity, the 
`DARTHOME/assimilation_code/modules/observations/DEFAULT_obs_kind_mod.F90` file 
must be edited to include a unique integer identifier for the quantity.) The 
third column must be the keyword COMMON_CODE which tells the preprocess program 
to automatically generate all necessary interface code for this type. 
...
*IMPORTANT:* as the code in these sections will be combined with the code of 
other observation definitions, the majority of the forward operator code here 
should be defined inside a separate module to keep variables and other private 
subroutines from colliding with unrelated routines and variables in other 
forward operator files. In other words, the code in the sections below should 
...
| :----------    | :---------                    |
|   state_handle | A handle to access the state vector (including all ensemble members) | 
|   ens_size     |  the number of ensemble members to do at once (between 1 and total ensemble size) | 
|   copy_indices | the indicies the ensemble members (between 1 and total ensemble size) |
|   location     | observation location |
|   obs_type_ind | the index of the specific type of observation |
...
| obs_val        | The observation value, if needed (read-write, but in general do not change obs_val here). |
| is_ascii       | Logical to indicate how the file was opened, formatted or unformatted (read-only) | 
...
| key 	         | The index observation number in this sequence (read-only) |
| is_ascii 	     | A logical value to indicate how the file was opened, formatted or unformatted (read-only) |
...
for no credit. While this seems somewhat paranoid, some users feel they cannot
be too careful in this regards. For these users, an additional step can
temporarily "hide" the sensitive code.
...
limit in mind if you are a free user.
