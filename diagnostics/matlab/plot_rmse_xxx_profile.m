...
% kdr add units to x-label
% basename = ReturnBase(x.allvarnames{i});
% if (basename == 'GPSRO_REFRACTIVITY')
%    units = '(refr.-1)*10^6'
% elseif (findstr('SPECIFIC_HUMIDITY',basename)
%    units = 'g/kg'
% endif (findstr('TEMPERATURE',basename)
%    units = 'K'
% endif (findstr('WIND',basename)
%    units = 'm/s'
% MORE options for chemicals, fluxes, ...
% end
% kdr end
