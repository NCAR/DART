...
! Read in the set_def_list
set_def_list = read_set_def_list(in_unit)

! Initialize a sequence
seq = init_obs_sequence(max_obs, 0)
...
! Would want to output some information about this obs_def, too, should come from obs_def_set level
...
20 continue
   ...
   read(*, *) option
! Should also allow both regular and irregular for same set, but too much work for now
   if(option == 1) then
...
! Loop to put these obs in the list
      ...
      read(*, *) days, seconds
! I know I shouldn't use goto's, but I can never remember the break syntax
      if(days < 0) goto 10

! Set the time
      this_time = set_time(seconds, days)

! Input this on a long list for later sorting (fixed storage for now should be changed)
      index = index + 1
      set_index(index) = i
      if(index > max_obs) then
         write(*, *) 'Number of observations exceeds max_obs'
